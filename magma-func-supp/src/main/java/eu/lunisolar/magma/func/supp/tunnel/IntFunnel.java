...
/**
 * IntFunnel is a finalized and reusable for of tunnel. In order to reduce GC one side (target/output) was reduced to some type of container
 * (not specific instance, but specific type).
 *
 * IntFunnel contains method that allow to push and/or transform some set of values of type [E] (entry values) to a container of
 * values of type [int].
 * 
 * IntFunnel the operations are organized in a way that those operations do not need to maintain a state while traversing the values. The main purpose
...
public class IntFunnel<C, E extends aType> {

	private final TeConsumer<C, E> tunnel;

	private IntFunnel(TeConsumer<C, E> tunnel) {
		this.tunnel = tunnel;
	}

	public static <C, E extends aType> IntFunnel<C, E> intFunnel(TeConsumer<C, E> tieFunction) {
		return new IntFunnel(tieFunction);
	}

	public <S> IntFunnel<C, E> push(C target, IndexedRead<S, E> ia, S source) {
		tunnel.genericForEach(target, ia, source);
		return this;
	}

	public <S, I> IntFunnel<C, E> push(C target, SequentialRead<S, I, E> sa, S source) {
