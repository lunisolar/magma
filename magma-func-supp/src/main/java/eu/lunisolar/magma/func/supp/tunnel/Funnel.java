...
/**
 * Funnel is a finalized and reusable for of tunnel. In order to reduce GC one side (target/output) was reduced to some type of container
 * (not specific instance, but specific type).
 *
 * Funnel contains method that allow to push and/or transform some set of values of type [E] (entry values) to a container of
 * values of type [T].
 * 
 * Funnel the operations are organized in a way that those operations do not need to maintain a state while traversing the values. The main purpose
...
public class Funnel<C, E extends aType> {

	private final TeConsumer<C, E> tunnel;

	private Funnel(TeConsumer<C, E> tunnel) {
		this.tunnel = tunnel;
	}

	public static <C, E extends aType> Funnel<C, E> funnel(TeConsumer<C, E> tieFunction) {
		return new Funnel(tieFunction);
	}

	public <S> Funnel<C, E> push(C target, IndexedRead<S, E> ia, S source) {
		tunnel.genericForEach(target, ia, source);
		return this;
	}

	public <S, I> Funnel<C, E> push(C target, SequentialRead<S, I, E> sa, S source) {
