...
public interface OptFltTrait<SELF extends OptFltTrait<SELF>> extends FluentTrait<SELF>, aValue<aFloat>, CheckFltTrait<SELF>, FilterFltSingleTrait<SELF>, IsFltTrait<SELF>, DoIfFltSingleTrait<SELF>, UseFltSingleTrait<SELF>, UniMapFltTrait<SELF> {
	...
	default <K1> SELF ifPresentOrWith(K1 a1, @Nonnull LObjFltConsumer<? super K1> action, @Nonnull LConsumer<K1> emptyAction) {
		Null.nonNullArg(action, "action");
		if (isPresent()) {
			action.accept(a1, get());
		} else {
	...
	default <K1, K2> SELF ifPresentOrWith(K1 a1, K2 a2, @Nonnull LBiObjFltConsumer<? super K1, ? super K2> action, @Nonnull LBiConsumer<K1, K2> emptyAction) {
		Null.nonNullArg(action, "action");
		if (isPresent()) {
			action.accept(a1, a2, get());
		} else {
