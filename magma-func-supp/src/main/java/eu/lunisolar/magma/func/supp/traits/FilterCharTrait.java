...
public interface FilterCharTrait<SELF extends FilterCharTrait<SELF>> extends FluentTrait<SELF> {
	...
	/** Variant 'obj.filter(..., (...) -> { ..long multiline definition.. })' */
	default @Nonnull <V1> SELF filterWith(V1 with, @Nonnull LObjCharPredicate<? super V1> predicate) {
		return filter(a -> predicate.test(with, a));
	}

	/** Variant 'obj.filter(Is::equal, ...)' or 'opt.filter(Does::contain, ...)', etc.  */
	default @Nonnull <V1> SELF filterWith(@Nonnull LObjCharPredicate<? super V1> predicate, V1 with) {
