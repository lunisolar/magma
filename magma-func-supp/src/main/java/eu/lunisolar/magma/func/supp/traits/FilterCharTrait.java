...
public interface FilterCharTrait<SELF extends FilterCharTrait<SELF>> extends FluentTrait<SELF> {
	...
	/** Variant 'obj.filter(..., (...) -> { ..long multiline definition.. })' */
	default <V> SELF filter2Variant(V v, @Nonnull LObjCharPredicate.LCharObjPred<? super V> predicate) {
		return filter(a -> predicate.testCharObj(a, v));
	}

	/** Variant 'obj.filter(Is::equal, ...)' or 'opt.filter(Does::contain, ...)', etc.  */
	default <V> SELF filter2Variant(@Nonnull LObjCharPredicate.LCharObjPred<? super V> predicate, V v) {
