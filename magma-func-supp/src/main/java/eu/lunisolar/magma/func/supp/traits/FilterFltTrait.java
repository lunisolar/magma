...
public interface FilterFltTrait<SELF extends FilterFltTrait<SELF>> extends FluentTrait<SELF> {
	...
	/** Variant 'obj.filter(Is::equal, ...)' or 'opt.filter(Does::contain, ...)', etc.  */
	default @Nonnull <V1> SELF filterWithFlt(@Nonnull LObjFltPredicate<? super V1> predicate, V1 with1) {
		return filterWithFlt(with1, predicate);
	}
	...
	/** Variant 'obj.filter(Is::equal, ...)' or 'opt.filter(Does::contain, ...)', etc.  */
	default @Nonnull <V1, V2> SELF filterWith(@Nonnull LBiObjFltPredicate<? super V1, ? super V2> predicate, V1 with1, V2 with2) {
