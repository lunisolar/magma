...
/**
 *
 * @see {@link eu.lunisolar.magma.func.predicate.BiBytePredicateX}
 */
@FunctionalInterface
...
public interface BiBytePredicate extends MetaPredicate, PrimitiveCodomain<BiBytePredicate> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> BiBytePredicate wrapException(@Nonnull final BiBytePredicate other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (byte b1, byte b2) -> {
			try {
				return other.test(b1, b2);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> BiBytePredicate handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BiBytePredicate.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> BiBytePredicate handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BiBytePredicate.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
