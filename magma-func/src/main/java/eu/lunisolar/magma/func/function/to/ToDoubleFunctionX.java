...
/**
 * Throwing functional interface (lambda) ToDoubleFunctionX for Java 8.
 *
 * Type: function
 *
 * Domain (lvl: 1): T t
...
public interface ToDoubleFunctionX<T, X extends Exception> extends MetaFunction, PrimitiveCodomain<ToDoubleFunctionX<T, X>>, MetaThrowingInterface<X> { // NOSONAR
	...
	default DoubleSupplierX<X> capture(T t) {
		return () -> this.applyAsDouble(t);
	}

	public static <T, X extends Exception> ToDoubleFunctionX<T, X> constant(double r) {
		return (t) -> r;
	}
	...
	/** Wraps JRE instance. */
	@Nonnull
	public static <T, X extends Exception> ToDoubleFunctionX<T, X> wrapStd(final java.util.function.ToDoubleFunction<T> other) {
	...
	default ToDoubleFunction<T> shove() {
		ToDoubleFunctionX<T, RuntimeException> exceptionCast = (ToDoubleFunctionX<T, RuntimeException>) this;
	...
	@Nonnull
	public static <T, X extends Exception, E extends Exception, Y extends Exception> ToDoubleFunctionX<T, Y> wrapException(@Nonnull final ToDoubleFunctionX<T, X> other, Class<E> exception, DoubleSupplierX<X> supplier, ExceptionHandler<E, Y> handler) {
		return (T t) -> {
			try {
	...
	default <E extends Exception, Y extends Exception> ToDoubleFunctionX<T, Y> handle(Class<E> exception, ExceptionHandler<E, Y> handler) {
		...
		return ToDoubleFunctionX.wrapException(this, exception, null, (ExceptionHandler) handler);
	}
	...
	default <E extends Exception, Y extends Exception> ToDoubleFunctionX<T, Y> handle(Class<E> exception, DoubleSupplierX<X> supplier) {
		...
		return ToDoubleFunctionX.wrapException(this, exception, supplier, null);
	}
