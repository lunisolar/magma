...
public interface LObjIntDblPredicate<T> extends MetaPredicate, MetaInterface.NonThrowing, Codomain<aBool>, Domain3<a<T>, aInt, aDouble> { // NOSONAR
	 ...
	 */
	public static <T> M<T> memento(boolean initialValue, LObjIntDblPredicate<T> baseFunction, LLogicalBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LObjIntDblPredicate.D)
	 */
	final class M<T> implements LObjIntDblPredicate<T> {
		...
		private M(boolean lastValue, LObjIntDblPredicate<T> baseFunction, LLogicalBinaryOperator mementoFunction) {
