...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.from.ObjDoubleFunction}
 */
@FunctionalInterface
...
public interface ObjDoubleFunctionX<T, R, X extends Exception> extends MetaFunction, MetaThrowingInterface<X> { // NOSONAR
	...
	@Nonnull
	public static <T, R, X extends Exception, Y extends Exception> ObjDoubleFunctionX<T, R, Y> wrapException(@Nonnull final ObjDoubleFunctionX<T, R, X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (T t, double d) -> {
			try {
	...
	@Nonnull
	default <Y extends Exception> ObjDoubleFunctionX<T, R, Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ObjDoubleFunctionX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> ObjDoubleFunctionX<T, R, Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ObjDoubleFunctionX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
