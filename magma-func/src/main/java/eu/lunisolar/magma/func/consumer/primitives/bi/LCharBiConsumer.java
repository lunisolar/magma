...
/**
 * Non-throwing functional interface (lambda) LCharBiConsumer for Java 8.
 *
 * Type: consumer
 *
...
public interface LCharBiConsumer extends LCharBiConsumerX<RuntimeException>, MetaConsumer, MetaInterface.NonThrowing {
	...
	default String functionalInterfaceDescription() {
		return LCharBiConsumer.DESCRIPTION;
	}
	...
	@Nonnull
	default <E extends Exception, Y extends RuntimeException> LCharBiConsumer handle(Class<E> exception, ExceptionHandler<E, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LCharBiConsumer.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for any exception (including unchecked exception that might be different from X) will call handler function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> LCharBiConsumer handle(ExceptionHandler<Exception, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LCharBiConsumer.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
