...
public interface LTieBoolConsumer<T> extends MetaConsumer, MetaInterface.NonThrowing, TieConsumer<T, aBool>, Codomain<aVoid>, Domain3<a<T>, aInt, aBool> {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1, V2, V3> LTriConsumer<V1, V2, V3> unboxingCompose(@Nonnull final LFunction<? super V1, ? extends T> before1, @Nonnull final LToIntFunction<? super V2> before2, @Nonnull final LPredicate<? super V3> before3) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
		Null.nonNullArg(before3, "before3");
	...
	default LTieBoolConsumer<T> shoving() {

		return new LTieBoolConsumer<T>() {

			public void accept(T a1, int a2, boolean a3) {
				try {
					this.acceptX(a1, a2, a3);
				} catch (Throwable e) { // NOSONAR
					Handling.handleErrors(e);
			...
			public void acceptX(T a1, int a2, boolean a3) throws Throwable {
				LTieBoolConsumer.this.acceptX(a1, a2, a3);
			}
	...
	*/
	public static <C1, C2, C3, I3, T> int iterate(IndexedRead<C1, a<T>> ia1, C1 source1, IndexedRead<C2, aInt> ia2, C2 source2, SequentialRead<C3, I3, aBool> sa3, C3 source3, LTieBoolConsumer<? super T> consumer) {
		...
		var nextFunc3 = SA.boolSupplier(sa3);
		int i = 0;
