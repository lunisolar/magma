...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.IntToShortFunctionX}
 */
@FunctionalInterface
...
public interface IntToShortFunction extends MetaFunction, PrimitiveCodomain<IntToShortFunction> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> IntToShortFunction wrapException(@Nonnull final IntToShortFunction other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (int i) -> {
			try {
				return other.applyAsShort(i);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> IntToShortFunction handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return IntToShortFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> IntToShortFunction handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return IntToShortFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
