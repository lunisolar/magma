...
public interface LObjIntFltFunction<T, R> extends MetaFunction, MetaInterface.NonThrowing, Codomain<a<R>>, Domain3<a<T>, aInt, aFloat> { // NOSONAR
	...
	public static <M, K, V, R> R from(@Nonnull M container, LBiFunction<M, K, V> extractor, K key, int a2, float a3, @Nonnull LObjIntFltFunction<V, R> function) {
	...
	final class M<T, R> implements LObjIntFltFunction<T, R> {
		...
		public R currentApply(T a1, int a2, float a3) {
			R x1 = lastBaseValue;
		...
		public R currentBaseValue(T a1, int a2, float a3) {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1, V2, V3> LTriFunction<V1, V2, V3, R> unboxingCompose(@Nonnull final LFunction<? super V1, ? extends T> before1, @Nonnull final LToIntFunction<? super V2> before2, @Nonnull final LToFltFunction<? super V3> before3) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
		Null.nonNullArg(before3, "before3");
		return (v1, v2, v3) -> this.apply(before1.apply(v1), before2.applyAsInt(v2), before3.applyAsFlt(v3));
	...
	default LObjIntFltFunction<T, R> shoving() {

		return new LObjIntFltFunction<T, R>() {
			...
			public R applyX(T a1, int a2, float a3) throws Throwable {
				return LObjIntFltFunction.this.applyX(a1, a2, a3);
			}
