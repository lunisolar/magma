...
public interface LObjIntSrtPredicate<T> extends MetaPredicate, MetaInterface.NonThrowing, Codomain<aBool>, Domain3<a<T>, aInt, aShort> { // NOSONAR
	 ...
	 */
	public static <T> M<T> memento(boolean initialValue, LObjIntSrtPredicate<T> baseFunction, LLogicalBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LObjIntSrtPredicate.D)
	 */
	final class M<T> implements LObjIntSrtPredicate<T> {
		...
		private M(boolean lastValue, LObjIntSrtPredicate<T> baseFunction, LLogicalBinaryOperator mementoFunction) {
