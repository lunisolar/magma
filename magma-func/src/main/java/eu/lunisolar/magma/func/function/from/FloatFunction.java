...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.from.FloatFunctionX}
 */
@FunctionalInterface
...
public interface FloatFunction<R> extends MetaFunction { // NOSONAR
	...
	@Nonnull
	public static <R, X extends Exception, Y extends RuntimeException> FloatFunction<R> wrapException(@Nonnull final FloatFunction<R> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (float f) -> {
			try {
				return other.apply(f);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> FloatFunction<R> handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return FloatFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> FloatFunction<R> handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return FloatFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
