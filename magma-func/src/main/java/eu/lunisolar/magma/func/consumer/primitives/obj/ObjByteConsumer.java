...
/**
 * A consumer.
 * @see {@link eu.lunisolar.magma.func.consumer.primitives.obj.ObjByteConsumerX}
 */
@FunctionalInterface
...
public interface ObjByteConsumer<T> extends MetaConsumer {
	...
	@Nonnull
	public static <T, X extends Exception, Y extends RuntimeException> ObjByteConsumer<T> wrapException(@Nonnull final ObjByteConsumer<T> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (T t, byte b) -> {
			try {
				other.accept(t, b);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> ObjByteConsumer<T> handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
	...
	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> ObjByteConsumer<T> handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);
