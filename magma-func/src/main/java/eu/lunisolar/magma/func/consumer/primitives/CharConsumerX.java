...
/**
 * Throwing functional interface (lambda) CharConsumerX for Java 8.
 *
 * Type: consumer
 *
 * Domain (lvl: 1): char c
...
public interface CharConsumerX<X extends Exception> extends MetaConsumer, MetaInterface.Throwing<X> {
	...
	default ActionX<X> capture(char c) {
	...
	default CharConsumer shove() {
		CharConsumerX<RuntimeException> exceptionCast = (CharConsumerX<RuntimeException>) this;
	...
	@Nonnull
	public static <X extends Exception, E extends Exception, Y extends Exception> CharConsumerX<Y> wrapException(@Nonnull final CharConsumerX<X> other, Class<E> exception, ExceptionHandler<E, Y> handler) {
		return (char c) -> {
			try {
				other.accept(c);
			} catch (Exception e) {
	...
	@Nonnull
	default <E extends Exception, Y extends Exception> CharConsumerX<Y> handleX(Class<E> exception, ExceptionHandler<E, Y> handler) {
	...
	@Nonnull
	default <Y extends Exception> CharConsumerX<Y> handleX(ExceptionHandler<Exception, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);
