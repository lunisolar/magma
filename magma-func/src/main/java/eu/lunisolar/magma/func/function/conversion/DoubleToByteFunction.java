...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.DoubleToByteFunctionX}
 */
@FunctionalInterface
...
public interface DoubleToByteFunction extends MetaFunction, PrimitiveCodomain<DoubleToByteFunction> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> DoubleToByteFunction wrapException(@Nonnull final DoubleToByteFunction other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (double d) -> {
			try {
				return other.applyAsByte(d);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> DoubleToByteFunction handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return DoubleToByteFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> DoubleToByteFunction handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return DoubleToByteFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
