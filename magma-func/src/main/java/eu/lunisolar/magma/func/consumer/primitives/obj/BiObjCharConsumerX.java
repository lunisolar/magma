...
public interface BiObjCharConsumerX<T1, T2, X extends Exception> extends MetaConsumer, MetaInterface.Throwing<X> {
	...
	default BiObjCharConsumerX<T1, T2, RuntimeException> uncheck() {
		return (BiObjCharConsumerX) this;
	}
	...
	@Nonnull
	default <E extends Exception, Y extends Exception> BiObjCharConsumerX<T1, T2, Y> handleX(Class<E> exception, ExceptionHandler<E, Y> handler) {
	...
	@Nonnull
	default <Y extends Exception> BiObjCharConsumerX<T1, T2, Y> handleX(ExceptionHandler<Exception, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);
