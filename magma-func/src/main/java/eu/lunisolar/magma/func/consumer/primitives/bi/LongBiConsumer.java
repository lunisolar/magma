...
/**
 * A consumer.
 * @see {@link eu.lunisolar.magma.func.consumer.primitives.bi.LongBiConsumerX}
 */
@FunctionalInterface
@SuppressWarnings("UnusedDeclaration")
public interface LongBiConsumer extends MetaConsumer {
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> LongBiConsumer wrapException(@Nonnull final LongBiConsumer other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (long l1, long l2) -> {
			try {
				other.accept(l1, l2);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> LongBiConsumer handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
	...
	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> LongBiConsumer handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);
