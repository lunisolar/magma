...
/**
 *
 * @see {@link eu.lunisolar.magma.func.operator.binary.CharBinaryOperatorX}
 */
@FunctionalInterface
...
public interface CharBinaryOperator extends MetaOperator, PrimitiveCodomain<CharBinaryOperator> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> CharBinaryOperator wrapException(@Nonnull final CharBinaryOperator other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (char c1, char c2) -> {
			try {
				return other.applyAsChar(c1, c2);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> CharBinaryOperator handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return CharBinaryOperator.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> CharBinaryOperator handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return CharBinaryOperator.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
