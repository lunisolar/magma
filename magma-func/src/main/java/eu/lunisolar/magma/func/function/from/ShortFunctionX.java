...
/**
 * Throwing functional interface (lambda) ShortFunctionX for Java 8.
 *
 * Type: function
 *
...
public interface ShortFunctionX<R, X extends Exception> extends MetaFunction, MetaInterface.Throwing<X> { // NOSONAR
	...
	default SupplierX<R, X> capture(short s) {
		return () -> this.apply(s);
	}

	public static <R, X extends Exception> ShortFunctionX<R, X> constant(R r) {
		return (s) -> r;
	}
	...
	default ShortFunction<R> shove() {
		ShortFunctionX<R, RuntimeException> exceptionCast = (ShortFunctionX<R, RuntimeException>) this;
	...
	@Nonnull
	public static <R, X extends Exception, E extends Exception, Y extends Exception> ShortFunctionX<R, Y> wrapException(@Nonnull final ShortFunctionX<R, X> other, Class<E> exception, SupplierX<R, X> supplier, ExceptionHandler<E, Y> handler) {
		return (short s) -> {
			try {
	...
	@Nonnull
	default <E extends Exception, Y extends Exception> ShortFunctionX<R, Y> handleX(Class<E> exception, ExceptionHandler<E, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ShortFunctionX.wrapException(this, exception, null, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for any exception (including unchecked exception that might be different from X) will call handler function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> ShortFunctionX<R, Y> handleX(ExceptionHandler<Exception, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ShortFunctionX.wrapException(this, Exception.class, null, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call supplier and return default value instead for propagating exception.  */
	@Nonnull
	default <E extends Exception, Y extends Exception> ShortFunctionX<R, Y> handleX(Class<E> exception, SupplierX<R, X> supplier) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(supplier, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ShortFunctionX.wrapException(this, exception, supplier, null);
	}

	/** Wraps with exception handling that for any exception will call supplier and return default value instead for propagating exception.  */
	@Nonnull
	default <Y extends Exception> ShortFunctionX<R, Y> handleX(SupplierX<R, X> supplier) {
		Objects.requireNonNull(supplier, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ShortFunctionX.wrapException(this, Exception.class, supplier, null);
	}
