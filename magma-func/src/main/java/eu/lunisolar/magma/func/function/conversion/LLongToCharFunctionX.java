...
public interface LLongToCharFunctionX<X extends Throwable> extends MetaFunction, MetaInterface.Throwing<X> { // NOSONAR
	...
	String DESCRIPTION = "LLongToCharFunctionX: char doApplyAsChar(long a) throws X";

	char doApplyAsChar(long a) throws X;

	default char tupleApplyAsChar(LLongSingle args) throws X {
	...
	default char shovingDoApplyAsChar(long a) {
		return ((LLongToCharFunctionX<RuntimeException>) this).doApplyAsChar(a);
	}

	/** Function call that handles exceptions according to the instructions. */
	default <Y extends Throwable> char handlingDoApplyAsChar(long a, HandlingInstructions<Throwable, Y> handling) throws Y {
	...
	default char nonNullDoApplyAsChar(long a) throws X {
	...
	default LCharSupplierX<X> captureLongToCharFunc(long a) {
		return () -> this.doApplyAsChar(a);
	}
	...
	static <X extends Throwable> char call(long a, final @Nonnull LLongToCharFunctionX<X> lambda) throws X {
		Null.nonNullArg(lambda, "lambda");
		return lambda.doApplyAsChar(a);
	}

	static <X extends Throwable> char shoving(long a, final @Nonnull LLongToCharFunctionX<X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.shovingDoApplyAsChar(a);
	}

	static <X extends Throwable> char nesting(long a, final @Nonnull LLongToCharFunctionX<X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.nestingDoApplyAsChar(a);
	}

	static <X extends Throwable, Y extends Throwable> char handling(long a, final HandlingInstructions<Throwable, Y> handling, final @Nonnull LLongToCharFunctionX<X> lambda) throws Y {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	static <X extends Throwable> LLongToCharFunctionX<X> safe() {
		return Function4U::produceChar;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <X extends Throwable, Y extends Throwable> LSupplierX<LLongToCharFunctionX<X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <X extends Throwable> LLongToCharFunctionX<X> safe(final @Nullable LLongToCharFunctionX<X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <X extends Throwable, Y extends Throwable> LSupplierX<LLongToCharFunctionX<X>, Y> safeSupplier(final @Nullable LSupplierX<LLongToCharFunctionX<X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	@Nonnull
	default LLongToCharFunctionX<X> longToCharFuncComposeLong(@Nonnull final LLongUnaryOperatorX<X> before) {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V> LToCharFunctionX<V, X> longToCharFuncCompose(@Nonnull final LToLongFunctionX<? super V, X> before) {
		Null.nonNullArg(before, "before");
		return v -> this.doApplyAsChar(before.doApplyAsLong(v));
	}
