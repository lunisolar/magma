...
public interface LBiBoolFunctionX<R, X extends Throwable> extends MetaFunction, MetaInterface.Throwing<X> { // NOSONAR
	...
	static <R, X extends Throwable> R call(boolean a1, boolean a2, final @Nonnull LBiBoolFunctionX<R, X> lambda) throws X {
		Null.nonNullArg(lambda, "lambda");
		return lambda.doApply(a1, a2);
	}

	static <R, X extends Throwable> R shoving(boolean a1, boolean a2, final @Nonnull LBiBoolFunctionX<R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.shovingDoApply(a1, a2);
	}

	static <R, X extends Throwable> R nesting(boolean a1, boolean a2, final @Nonnull LBiBoolFunctionX<R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.nestingDoApply(a1, a2);
	}

	static <R, X extends Throwable, Y extends Throwable> R handling(boolean a1, boolean a2, final HandlingInstructions<Throwable, Y> handling, final @Nonnull LBiBoolFunctionX<R, X> lambda) throws Y {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	static <R, X extends Throwable> LBiBoolFunctionX<R, X> safe() {
		return Function4U::produce;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <R, X extends Throwable, Y extends Throwable> LSupplierX<LBiBoolFunctionX<R, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <R, X extends Throwable> LBiBoolFunctionX<R, X> safe(final @Nullable LBiBoolFunctionX<R, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <R, X extends Throwable, Y extends Throwable> LSupplierX<LBiBoolFunctionX<R, X>, Y> safeSupplier(final @Nullable LSupplierX<LBiBoolFunctionX<R, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	/** Permutation of LBiBoolFunctionX for method references. */
	@FunctionalInterface
	interface V1<R, X extends Throwable> extends LBiBoolFunctionX<R, X> {
		@Nullable
		R doApplyV1(boolean a2, boolean a1) throws X;

		@Override
		default R doApply(boolean a1, boolean a2) throws X {
