...
public interface LToCharFunction<T> extends MetaFunction, MetaInterface.NonThrowing, OFunction<T, aChar>, Codomain<aChar>, Domain1<a<T>> { // NOSONAR
	 ...
	 */
	public static <T> M<T> memento(char initialValue, LToCharFunction<T> baseFunction, LCharBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LToCharFunction.D)
	 */
	final class M<T> implements LToCharFunction<T> {
		...
		private M(char lastValue, LToCharFunction<T> baseFunction, LCharBinaryOperator mementoFunction) {
