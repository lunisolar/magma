...
public interface LTieByteFunction<T> extends MetaFunction, MetaInterface.NonThrowing, TieFunction<T, aByte>, Codomain<aInt>, Domain3<a<T>, aInt, aByte> { // NOSONAR
	 ...
	 */
	public static <T> M<T> memento(int initialValue, LTieByteFunction<T> baseFunction, LIntBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LTieByteFunction.D)
	 */
	final class M<T> implements LTieByteFunction<T> {
		...
		private M(int lastValue, LTieByteFunction<T> baseFunction, LIntBinaryOperator mementoFunction) {
