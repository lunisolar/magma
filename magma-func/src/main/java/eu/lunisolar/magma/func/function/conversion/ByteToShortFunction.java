...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.ByteToShortFunctionX}
 */
@FunctionalInterface
...
public interface ByteToShortFunction extends MetaFunction, PrimitiveCodomain<ByteToShortFunction> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> ByteToShortFunction wrapException(@Nonnull final ByteToShortFunction other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (byte b) -> {
			try {
				return other.applyAsShort(b);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> ByteToShortFunction handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ByteToShortFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> ByteToShortFunction handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ByteToShortFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
