...
/**
 * A consumer.
 * @see {@link eu.lunisolar.magma.func.consumer.primitives.obj.BiObjCharConsumerX}
 */
@FunctionalInterface
...
public interface BiObjCharConsumer<T1, T2> extends MetaConsumer {
	...
	@Nonnull
	public static <T1, T2, X extends Exception, Y extends RuntimeException> BiObjCharConsumer<T1, T2> wrapException(@Nonnull final BiObjCharConsumer<T1, T2> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (T1 t1, T2 t2, char c) -> {
			try {
				other.accept(t1, t2, c);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> BiObjCharConsumer<T1, T2> handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
	...
	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> BiObjCharConsumer<T1, T2> handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);
