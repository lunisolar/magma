...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.FloatToDoubleFunctionX}
 */
@FunctionalInterface
...
public interface FloatToDoubleFunction extends MetaFunction, PrimitiveCodomain<FloatToDoubleFunction> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> FloatToDoubleFunction wrapException(@Nonnull final FloatToDoubleFunction other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (float f) -> {
			try {
				return other.applyAsDouble(f);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> FloatToDoubleFunction handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return FloatToDoubleFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> FloatToDoubleFunction handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return FloatToDoubleFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
