...
public interface LObjDblFunction<T, R> extends MetaFunction, MetaInterface.NonThrowing, Codomain<a<R>>, Domain2<a<T>, aDouble> { // NOSONAR
	...
	final class M<T, R> implements LObjDblFunction<T, R> {
		...
		public R currentApply(T a1, double a2) {
			R x1 = lastBaseValue;
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1, V2> LBiFunction<V1, V2, R> unboxingCompose(@Nonnull final LFunction<? super V1, ? extends T> before1, @Nonnull final LToDblFunction<? super V2> before2) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
		return (v1, v2) -> this.apply(before1.apply(v1), before2.applyAsDbl(v2));
	}
	...
	default LObjDblFunction<T, R> shoving() {

		return new LObjDblFunction<T, R>() {
			...
			public R applyX(T a1, double a2) throws Throwable {
				return LObjDblFunction.this.applyX(a1, a2);
			}
