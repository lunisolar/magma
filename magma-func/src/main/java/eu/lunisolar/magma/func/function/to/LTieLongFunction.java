...
public interface LTieLongFunction<T> extends MetaFunction, MetaInterface.NonThrowing, TieFunction<T, aLong>, Codomain<aInt>, Domain3<a<T>, aInt, aLong> { // NOSONAR
	 ...
	 */
	public static <T> M<T> memento(int initialValue, LTieLongFunction<T> baseFunction, LIntBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LTieLongFunction.D)
	 */
	final class M<T> implements LTieLongFunction<T> {
		...
		private M(int lastValue, LTieLongFunction<T> baseFunction, LIntBinaryOperator mementoFunction) {
