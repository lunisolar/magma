...
public interface LObjByteConsumerX<T, X extends Throwable> extends MetaConsumer, MetaInterface.Throwing<X> {
	...
	static <T, X extends Throwable> void call(T a1, byte a2, final @Nonnull LObjByteConsumerX<T, X> lambda) throws X {
		Null.nonNullArg(lambda, "lambda");
		lambda.doAccept(a1, a2);
	}

	static <T, X extends Throwable> void shoving(T a1, byte a2, final @Nonnull LObjByteConsumerX<T, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		lambda.shovingDoAccept(a1, a2);
	}

	static <T, X extends Throwable> void nesting(T a1, byte a2, final @Nonnull LObjByteConsumerX<T, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		lambda.nestingDoAccept(a1, a2);
	}

	static <T, X extends Throwable, Y extends Throwable> void handling(T a1, byte a2, final HandlingInstructions<Throwable, Y> handling, final @Nonnull LObjByteConsumerX<T, X> lambda) throws Y {
		Null.nonNullArg(lambda, "lambda");
	...
	/** Safe instance. */
	@Nonnull
	static <T, X extends Throwable> LObjByteConsumerX<T, X> safe() {
		return Function4U::doNothing;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <T, X extends Throwable, Y extends Throwable> LSupplierX<LObjByteConsumerX<T, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <T, X extends Throwable> LObjByteConsumerX<T, X> safe(final @Nullable LObjByteConsumerX<T, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <T, X extends Throwable, Y extends Throwable> LSupplierX<LObjByteConsumerX<T, X>, Y> safeSupplier(final @Nullable LSupplierX<LObjByteConsumerX<T, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	/** Combines two LObjByteConsumerX<T,X> together in a order. */
	@Nonnull
	...
	/** Permutation of LObjByteConsumerX for method references. */
	@FunctionalInterface
	interface V1<T, X extends Throwable> extends LObjByteConsumerX<T, X> {

		void doAcceptV1(byte a2, T a1) throws X;

		@Override
		default void doAccept(T a1, byte a2) throws X {
