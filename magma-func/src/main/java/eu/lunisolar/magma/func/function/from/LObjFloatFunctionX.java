...
public interface LObjFloatFunctionX<T, R, X extends Throwable> extends MetaFunction, MetaInterface.Throwing<X> { // NOSONAR
	...
	String DESCRIPTION = "LObjFloatFunctionX: R doApply(T a1,float a2) throws X";

	@Nullable
	R doApply(T a1, float a2) throws X;

	default R tupleApply(LObjFloatPair<T> args) throws X {
	...
	default R shovingDoApply(T a1, float a2) {
		return ((LObjFloatFunctionX<T, R, RuntimeException>) this).doApply(a1, a2);
	}

	/** Function call that handles exceptions according to the instructions. */
	default <Y extends Throwable> R handlingDoApply(T a1, float a2, HandlingInstructions<Throwable, Y> handling) throws Y {
	...
	@Nonnull
	default R nonNullDoApply(T a1, float a2) throws X {
	...
	default LSupplierX<R, X> captureObjFloatFunc(T a1, float a2) {
		return () -> this.doApply(a1, a2);
	}
	...
	static <T, R, X extends Throwable> R call(T a1, float a2, final @Nonnull LObjFloatFunctionX<T, R, X> lambda) throws X {
		Null.nonNullArg(lambda, "lambda");
		return lambda.doApply(a1, a2);
	}

	static <T, R, X extends Throwable> R shoving(T a1, float a2, final @Nonnull LObjFloatFunctionX<T, R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.shovingDoApply(a1, a2);
	}

	static <T, R, X extends Throwable> R nesting(T a1, float a2, final @Nonnull LObjFloatFunctionX<T, R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.nestingDoApply(a1, a2);
	}

	static <T, R, X extends Throwable, Y extends Throwable> R handling(T a1, float a2, final HandlingInstructions<Throwable, Y> handling, final @Nonnull LObjFloatFunctionX<T, R, X> lambda) throws Y {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	static <T, R, X extends Throwable> LObjFloatFunctionX<T, R, X> safe() {
		return Function4U::produce;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <T, R, X extends Throwable, Y extends Throwable> LSupplierX<LObjFloatFunctionX<T, R, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <T, R, X extends Throwable> LObjFloatFunctionX<T, R, X> safe(final @Nullable LObjFloatFunctionX<T, R, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <T, R, X extends Throwable, Y extends Throwable> LSupplierX<LObjFloatFunctionX<T, R, X>, Y> safeSupplier(final @Nullable LSupplierX<LObjFloatFunctionX<T, R, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	default <V1> LObjFloatFunctionX<V1, R, X> objFloatFuncComposeFloat(@Nonnull final LFunctionX<? super V1, ? extends T, X> before1, @Nonnull final LFloatUnaryOperatorX<X> before2) {
		...
		return (V1 v1, float v2) -> this.doApply(before1.doApply(v1), before2.doApplyAsFloat(v2));
	}
	...
	default <V> LObjFloatFunctionX<T, V, X> then(@Nonnull LFunctionX<? super R, ? extends V, X> after) {
		...
		return (T a1, float a2) -> after.doApply(this.doApply(a1, a2));
	}
	...
	default LObjFloatConsumerX<T, X> then(@Nonnull LConsumerX<? super R, X> after) {
		Null.nonNullArg(after, "after");
		return (T a1, float a2) -> after.doAccept(this.doApply(a1, a2));
	}
	...
	default LObjFloatFunction<T, R> handleObjFloatFunc(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
		return (T a1, float a2) -> this.handlingDoApply(a1, a2, handling);
	}
	...
	/** Permutation of LObjFloatFunctionX for method references. */
	@FunctionalInterface
	interface V1<T, R, X extends Throwable> extends LObjFloatFunctionX<T, R, X> {
		@Nullable
		R doApplyV1(float a2, T a1) throws X;

		@Override
		default R doApply(T a1, float a2) throws X {
