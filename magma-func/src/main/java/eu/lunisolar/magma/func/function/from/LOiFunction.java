...
public interface LOiFunction<T, R> extends MetaFunction, MetaInterface.NonThrowing, OiFunction<T, a<R>>, Codomain<a<R>>, Domain2<a<T>, aInt> { // NOSONAR
	...
	public static <M, K, V, R> R from(@Nonnull M container, LBiFunction<M, K, V> extractor, K key, int a2, @Nonnull LOiFunction<V, R> function) {
	...
	final class M<T, R> implements LOiFunction<T, R> {
		...
		public R currentApply(T a1, int a2) {
			R x1 = lastBaseValue;
		...
		public R currentBaseValue(T a1, int a2) {
	...
	default LOiFunction<T, R> shoving() {

		return new LOiFunction<T, R>() {
			...
			public R applyX(T a1, int a2) throws Throwable {
				return LOiFunction.this.applyX(a1, a2);
			}
