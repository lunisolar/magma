...
/**
 *
 * @see {@link eu.lunisolar.magma.func.predicate.BiPredicateX}
 */
@FunctionalInterface
...
public interface BiPredicate<T1, T2> extends java.util.function.BiPredicate<T1, T2>, MetaPredicate, PrimitiveCodomain<BiPredicate<T1, T2>> { // NOSONAR
	...
	/** Wraps JRE instance. */
	@Nonnull
	public static <T1, T2> BiPredicate<T1, T2> std(final java.util.function.BiPredicate<T1, T2> other) {
		return other::test;
	}
	...
	@Nonnull
	public static <T1, T2, X extends Exception, Y extends RuntimeException> BiPredicate<T1, T2> wrapException(@Nonnull final BiPredicate<T1, T2> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (T1 t1, T2 t2) -> {
			try {
				return other.test(t1, t2);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> BiPredicate<T1, T2> handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BiPredicate.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> BiPredicate<T1, T2> handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BiPredicate.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
