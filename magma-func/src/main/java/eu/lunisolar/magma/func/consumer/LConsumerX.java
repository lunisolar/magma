...
public interface LConsumerX<T, X extends Throwable> extends Consumer<T>, MetaConsumer, MetaInterface.Throwing<X> {
	...
	/** Safe instance. */
	@Nonnull
	static <T, X extends Throwable> LConsumerX<T, X> safe() {
		return Function4U::doNothing;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <T, X extends Throwable, Y extends Throwable> LSupplierX<LConsumerX<T, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <T, X extends Throwable> LConsumerX<T, X> safe(final @Nullable LConsumerX<T, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <T, X extends Throwable, Y extends Throwable> LSupplierX<LConsumerX<T, X>, Y> safeSupplier(final @Nullable LSupplierX<LConsumerX<T, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	/** Combines two LConsumerX<T,X> together in a order. */
	@Nonnull
