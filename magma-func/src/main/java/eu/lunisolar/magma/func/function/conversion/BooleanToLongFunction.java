...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.BooleanToLongFunctionX}
 */
@FunctionalInterface
@SuppressWarnings("UnusedDeclaration")
public interface BooleanToLongFunction extends MetaFunction, PrimitiveCodomain<BooleanToLongFunction> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> BooleanToLongFunction wrapException(@Nonnull final BooleanToLongFunction other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (boolean b) -> {
			try {
				return other.applyAsLong(b);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> BooleanToLongFunction handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BooleanToLongFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> BooleanToLongFunction handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BooleanToLongFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
