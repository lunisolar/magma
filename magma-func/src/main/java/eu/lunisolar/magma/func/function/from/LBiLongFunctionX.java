...
public interface LBiLongFunctionX<R, X extends Throwable> extends MetaFunction, MetaInterface.Throwing<X> { // NOSONAR
	...
	@Nonnull
	static <R, X extends Throwable> LBiLongFunctionX<R, X> safe() {
		return Function4U::produce;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <R, X extends Throwable, Y extends Throwable> LSupplierX<LBiLongFunctionX<R, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <R, X extends Throwable> LBiLongFunctionX<R, X> safe(final @Nullable LBiLongFunctionX<R, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <R, X extends Throwable, Y extends Throwable> LSupplierX<LBiLongFunctionX<R, X>, Y> safeSupplier(final @Nullable LSupplierX<LBiLongFunctionX<R, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	/** Permutation of LBiLongFunctionX for method references. */
	@FunctionalInterface
	interface V1<R, X extends Throwable> extends LBiLongFunctionX<R, X> {
		@Nullable
		R doApplyV1(long a2, long a1) throws X;

		@Override
		default R doApply(long a1, long a2) throws X {
