...
/**
 *
 * @see {@link eu.lunisolar.magma.func.predicate.ObjLongPredicateX}
 */
@FunctionalInterface
...
public interface ObjLongPredicate<T> extends MetaPredicate, PrimitiveCodomain<ObjLongPredicate<T>> { // NOSONAR
	...
	@Nonnull
	public static <T, X extends Exception, Y extends RuntimeException> ObjLongPredicate<T> wrapException(@Nonnull final ObjLongPredicate<T> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (T t, long l) -> {
			try {
				return other.test(t, l);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> ObjLongPredicate<T> handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ObjLongPredicate.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> ObjLongPredicate<T> handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ObjLongPredicate.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
