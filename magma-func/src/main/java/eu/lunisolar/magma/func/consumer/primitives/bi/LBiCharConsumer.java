...
public interface LBiCharConsumer extends MetaConsumer, MetaInterface.NonThrowing, Codomain<aVoid>, Domain2<aChar, aChar> {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1, V2> LBiConsumer<V1, V2> unboxingCompose(@Nonnull final LToCharFunction<? super V1> before1, @Nonnull final LToCharFunction<? super V2> before2) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
		return (v1, v2) -> this.accept(before1.applyAsChar(v1), before2.applyAsChar(v2));
	}
	...
	default LBiCharConsumer shoving() {

		return new LBiCharConsumer() {

			public void accept(char a1, char a2) {
				try {
					this.acceptX(a1, a2);
				} catch (Throwable e) { // NOSONAR
					Handling.handleErrors(e);
			...
			public void acceptX(char a1, char a2) throws Throwable {
				LBiCharConsumer.this.acceptX(a1, a2);
			}
