...
public interface LToFltFunction<T> extends MetaFunction, MetaInterface.NonThrowing, OFunction<T, aFloat>, Codomain<aFloat>, Domain1<a<T>> { // NOSONAR
	 ...
	 */
	public static <T> M<T> memento(float initialValue, LToFltFunction<T> baseFunction, LFltBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LToFltFunction.D)
	 */
	final class M<T> implements LToFltFunction<T> {
		...
		private M(float lastValue, LToFltFunction<T> baseFunction, LFltBinaryOperator mementoFunction) {
