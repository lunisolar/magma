...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.from.ObjCharFunction}
 */
@FunctionalInterface
@SuppressWarnings("UnusedDeclaration")
public interface ObjCharFunctionX<T, R, X extends Exception> extends MetaFunction, MetaThrowingInterface<X> { // NOSONAR
	...
	@Nonnull
	public static <T, R, X extends Exception, Y extends Exception> ObjCharFunctionX<T, R, Y> wrapException(@Nonnull final ObjCharFunctionX<T, R, X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (T t, char c) -> {
			try {
	...
	default ObjCharFunctionX<T, R, X> nonNullableX() {
		return new NonNullObjCharFunctionX(this);
	}
	...
	@Nonnull
	default <Y extends Exception> ObjCharFunctionX<T, R, Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ObjCharFunctionX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> ObjCharFunctionX<T, R, Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ObjCharFunctionX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
