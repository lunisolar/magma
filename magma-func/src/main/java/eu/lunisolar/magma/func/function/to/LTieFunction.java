...
public interface LTieFunction<T1, T2> extends MetaFunction, MetaInterface.NonThrowing, TieFunction<T1, a<T2>>, Codomain<aInt>, Domain3<a<T1>, aInt, a<T2>> { // NOSONAR
	 ...
	 */
	public static <T1, T2> M<T1, T2> memento(int initialValue, LTieFunction<T1, T2> baseFunction, LIntBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LTieFunction.D)
	 */
	final class M<T1, T2> implements LTieFunction<T1, T2> {
		...
		private M(int lastValue, LTieFunction<T1, T2> baseFunction, LIntBinaryOperator mementoFunction) {
