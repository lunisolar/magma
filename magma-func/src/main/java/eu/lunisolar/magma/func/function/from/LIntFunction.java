...
public interface LIntFunction<R> extends IntFunction<R>, MetaFunction, MetaInterface.NonThrowing, Codomain<a<R>>, Domain1<aInt> { // NOSONAR
	 ...
	 */
	public static <R> M<R> memento(R initialValue, LIntFunction<R> baseFunction, LBinaryOperator<R> mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LIntFunction.D)
	 */
	final class M<R> implements LIntFunction<R> {
		...
		private M(R lastValue, LIntFunction<R> baseFunction, LBinaryOperator<R> mementoFunction) {
