...
public interface LObjDoubleConsumerX<T, X extends Throwable> extends ObjDoubleConsumer<T>, MetaConsumer, MetaInterface.Throwing<X> {

	String DESCRIPTION = "LObjDoubleConsumerX: void doAccept(T a1,double a2) throws X";

	/**
	 * Default implementation for JRE method that calls exception nesting method.
	 * @deprecated Calling this method via LObjDoubleConsumerX interface should be discouraged.
	 */
	@Override
	@Deprecated
	...
	void doAccept(T a1, double a2) throws X;

	default LTuple.Void tupleAccept(LObjDoublePair<T> args) throws X {
	...
	default void shovingDoAccept(T a1, double a2) {
		((LObjDoubleConsumerX<T, RuntimeException>) this).doAccept(a1, a2);
	}

	/** Function call that handles exceptions according to the instructions. */
	default <Y extends Throwable> void handlingDoAccept(T a1, double a2, HandlingInstructions<Throwable, Y> handling) throws Y {
	...
	default LActionX<X> captureObjDoubleCons(T a1, double a2) {
	...
	@Nonnull
	static <T, X extends Throwable> LObjDoubleConsumerX<T, X> lX(final @Nonnull LObjDoubleConsumerX<T, X> lambda) {
	...
	/** Convenient method in case lambda expression is ambiguous for the compiler (that might happen for overloaded methods accepting different interfaces). */
	@Nonnull
	static <T, X extends Throwable> LObjDoubleConsumerX<T, X> lX(@Nonnull Class<X> xClass, final @Nonnull LObjDoubleConsumerX<T, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda;
	}
	...
	/** Wraps JRE instance. */
	@Nonnull
	static <T, X extends Throwable> LObjDoubleConsumerX<T, X> wrap(final ObjDoubleConsumer<T> other) {
		return other::accept;
	}
	...
	/** Safe instance. */
	@Nonnull
	static <T, X extends Throwable> LObjDoubleConsumerX<T, X> safe() {
		return Function4U::doNothing;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <T, X extends Throwable, Y extends Throwable> LSupplierX<LObjDoubleConsumerX<T, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <T, X extends Throwable> LObjDoubleConsumerX<T, X> safe(final @Nullable LObjDoubleConsumerX<T, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <T, X extends Throwable, Y extends Throwable> LSupplierX<LObjDoubleConsumerX<T, X>, Y> safeSupplier(final @Nullable LSupplierX<LObjDoubleConsumerX<T, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	/** Combines two LObjDoubleConsumerX<T,X> together in a order. */
	@Nonnull
	...
	/** Permutation of LObjDoubleConsumerX for method references. */
	@FunctionalInterface
	interface V1<T, X extends Throwable> extends LObjDoubleConsumerX<T, X> {

		void doAcceptV1(double a2, T a1) throws X;

		@Override
		default void doAccept(T a1, double a2) throws X {
