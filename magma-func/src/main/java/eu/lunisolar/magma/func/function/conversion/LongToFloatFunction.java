...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.LongToFloatFunctionX}
 */
@FunctionalInterface
...
public interface LongToFloatFunction extends MetaFunction, PrimitiveCodomain<LongToFloatFunction> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> LongToFloatFunction wrapException(@Nonnull final LongToFloatFunction other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (long l) -> {
			try {
				return other.applyAsFloat(l);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> LongToFloatFunction handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LongToFloatFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> LongToFloatFunction handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LongToFloatFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
