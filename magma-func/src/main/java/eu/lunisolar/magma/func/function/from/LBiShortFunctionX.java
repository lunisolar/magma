...
public interface LBiShortFunctionX<R, X extends Throwable> extends MetaFunction, MetaInterface.Throwing<X> { // NOSONAR
	...
	static <R, X extends Throwable> R call(short a1, short a2, final @Nonnull LBiShortFunctionX<R, X> lambda) throws X {
		Null.nonNullArg(lambda, "lambda");
		return lambda.doApply(a1, a2);
	}

	static <R, X extends Throwable> R shoving(short a1, short a2, final @Nonnull LBiShortFunctionX<R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.shovingDoApply(a1, a2);
	}

	static <R, X extends Throwable> R nesting(short a1, short a2, final @Nonnull LBiShortFunctionX<R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.nestingDoApply(a1, a2);
	}

	static <R, X extends Throwable, Y extends Throwable> R handling(short a1, short a2, final HandlingInstructions<Throwable, Y> handling, final @Nonnull LBiShortFunctionX<R, X> lambda) throws Y {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	static <R, X extends Throwable> LBiShortFunctionX<R, X> safe() {
		return Function4U::produce;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <R, X extends Throwable, Y extends Throwable> LSupplierX<LBiShortFunctionX<R, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <R, X extends Throwable> LBiShortFunctionX<R, X> safe(final @Nullable LBiShortFunctionX<R, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <R, X extends Throwable, Y extends Throwable> LSupplierX<LBiShortFunctionX<R, X>, Y> safeSupplier(final @Nullable LSupplierX<LBiShortFunctionX<R, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	/** Permutation of LBiShortFunctionX for method references. */
	@FunctionalInterface
	interface V1<R, X extends Throwable> extends LBiShortFunctionX<R, X> {
		@Nullable
		R doApplyV1(short a2, short a1) throws X;

		@Override
		default R doApply(short a1, short a2) throws X {
