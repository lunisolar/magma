...
/**
 *
 * @see {@link eu.lunisolar.magma.func.predicate.BiPredicate}
 */
@FunctionalInterface
...
public interface BiPredicateX<T1, T2, X extends Exception> extends MetaPredicate, PrimitiveCodomain<BiPredicateX<T1, T2, X>>, MetaThrowingInterface<X> { // NOSONAR
	...
	@Nonnull
	public static <T1, T2, X extends Exception, Y extends Exception> BiPredicateX<T1, T2, Y> wrapException(@Nonnull final BiPredicateX<T1, T2, X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (T1 t1, T2 t2) -> {
			try {
	...
	@Nonnull
	default <Y extends Exception> BiPredicateX<T1, T2, Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BiPredicateX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> BiPredicateX<T1, T2, Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BiPredicateX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
