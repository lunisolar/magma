...
/**
 *
 * @see {@link eu.lunisolar.magma.func.supplier.LongSupplierX}
 */
@FunctionalInterface
...
public interface LongSupplier extends java.util.function.LongSupplier, MetaSupplier, PrimitiveCodomain<LongSupplier> {
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> LongSupplier wrapException(@Nonnull final LongSupplier other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return () -> {
			try {
				return other.getAsLong();
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> LongSupplier handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LongSupplier.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> LongSupplier handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LongSupplier.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
