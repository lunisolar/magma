...
public interface LOiToByteFunction<T> extends MetaFunction, MetaInterface.NonThrowing, OiFunction<T, aByte>, Codomain<aByte>, Domain2<a<T>, aInt> { // NOSONAR
	 ...
	 */
	public static <T> M<T> memento(byte initialValue, LOiToByteFunction<T> baseFunction, LByteBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LOiToByteFunction.D)
	 */
	final class M<T> implements LOiToByteFunction<T> {
		...
		private M(byte lastValue, LOiToByteFunction<T> baseFunction, LByteBinaryOperator mementoFunction) {
