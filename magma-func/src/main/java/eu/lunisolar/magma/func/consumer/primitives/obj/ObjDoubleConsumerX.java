...
/**
 * Throwing functional interface (lambda) ObjDoubleConsumerX for Java 8.
 *
 * Type: consumer
 *
...
public interface ObjDoubleConsumerX<T, X extends Exception> extends MetaConsumer, MetaInterface.Throwing<X> {
	...
	default ActionX<X> capture(T t, double d) {
	...
	default ObjDoubleConsumerX<T, RuntimeException> uncheck() {
		return (ObjDoubleConsumerX) this;
	}

	/** Dirty way, checked exception will propagate as it would be unchecked - there is no exception wrapping involved (at least not here). */
	default ObjDoubleConsumer<T> shove() {
		ObjDoubleConsumerX<T, RuntimeException> exceptionCast = (ObjDoubleConsumerX<T, RuntimeException>) this;
	...
	@Nonnull
	public static <T, X extends Exception, E extends Exception, Y extends Exception> ObjDoubleConsumerX<T, Y> wrapException(@Nonnull final ObjDoubleConsumerX<T, X> other, Class<E> exception, ExceptionHandler<E, Y> handler) {
		return (T t, double d) -> {
			try {
				other.accept(t, d);
			} catch (Exception e) {
	...
	@Nonnull
	default <E extends Exception, Y extends Exception> ObjDoubleConsumerX<T, Y> handleX(Class<E> exception, ExceptionHandler<E, Y> handler) {
	...
	@Nonnull
	default <Y extends Exception> ObjDoubleConsumerX<T, Y> handleX(ExceptionHandler<Exception, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);
