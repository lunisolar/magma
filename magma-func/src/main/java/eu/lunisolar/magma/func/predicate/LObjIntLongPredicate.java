...
public interface LObjIntLongPredicate<T> extends MetaPredicate, MetaInterface.NonThrowing, Codomain<aBool>, Domain3<a<T>, aInt, aLong> { // NOSONAR
	 ...
	 */
	public static <T> M<T> memento(boolean initialValue, LObjIntLongPredicate<T> baseFunction, LLogicalBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LObjIntLongPredicate.D)
	 */
	final class M<T> implements LObjIntLongPredicate<T> {
		...
		private M(boolean lastValue, LObjIntLongPredicate<T> baseFunction, LLogicalBinaryOperator mementoFunction) {
