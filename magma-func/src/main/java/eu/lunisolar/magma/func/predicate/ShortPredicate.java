...
/**
 *
 * @see {@link eu.lunisolar.magma.func.predicate.ShortPredicateX}
 */
@FunctionalInterface
...
public interface ShortPredicate extends MetaPredicate, PrimitiveCodomain<ShortPredicate> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> ShortPredicate wrapException(@Nonnull final ShortPredicate other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (short s) -> {
			try {
				return other.test(s);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> ShortPredicate handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ShortPredicate.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> ShortPredicate handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ShortPredicate.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
