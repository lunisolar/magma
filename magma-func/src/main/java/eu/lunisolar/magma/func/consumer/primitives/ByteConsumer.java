...
/**
 * A consumer.
 * @see {@link eu.lunisolar.magma.func.consumer.primitives.ByteConsumerX}
 */
@FunctionalInterface
...
public interface ByteConsumer extends MetaConsumer {
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> ByteConsumer wrapException(@Nonnull final ByteConsumer other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (byte b) -> {
			try {
				other.accept(b);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> ByteConsumer handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
	...
	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> ByteConsumer handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);
