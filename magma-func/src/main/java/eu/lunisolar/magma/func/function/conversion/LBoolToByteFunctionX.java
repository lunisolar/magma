...
public interface LBoolToByteFunctionX<X extends Throwable> extends MetaFunction, MetaInterface.Throwing<X> { // NOSONAR
	...
	String DESCRIPTION = "LBoolToByteFunctionX: byte doApplyAsByte(boolean a1) throws X";

	byte doApplyAsByte(boolean a1) throws X;
	...
	default byte shovingDoApplyAsByte(boolean a1) {
		return ((LBoolToByteFunctionX<RuntimeException>) this).doApplyAsByte(a1);
	}

	/** Function call that handles exceptions according to the instructions. */
	default <Y extends Throwable> byte handlingDoApplyAsByte(boolean a1, HandlingInstructions<Throwable, Y> handling) throws Y {
	...
	default byte nonNullDoApplyAsByte(boolean a1) throws X {
	...
	default LByteSupplierX<X> captureBoolToByteFunc(boolean a1) {
		return () -> this.doApplyAsByte(a1);
	}
	...
	static <X extends Throwable> byte call(boolean a1, final @Nonnull LBoolToByteFunctionX<X> lambda) throws X {
		Null.nonNullArg(lambda, "lambda");
		return lambda.doApplyAsByte(a1);
	}

	static <X extends Throwable> byte shoving(boolean a1, final @Nonnull LBoolToByteFunctionX<X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.shovingDoApplyAsByte(a1);
	}

	static <X extends Throwable> byte nesting(boolean a1, final @Nonnull LBoolToByteFunctionX<X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.nestingDoApplyAsByte(a1);
	}

	static <X extends Throwable, Y extends Throwable> byte handling(boolean a1, final HandlingInstructions<Throwable, Y> handling, final @Nonnull LBoolToByteFunctionX<X> lambda) throws Y {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	default LBoolToByteFunctionX<X> boolToByteFuncComposeBool(@Nonnull final LLogicalOperatorX<X> before1) {
