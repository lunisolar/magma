...
public interface LObjBoolConsumerX<T, X extends Throwable> extends MetaConsumer, MetaInterface.Throwing<X> {
	...
	/** Safe instance. */
	@Nonnull
	static <T, X extends Throwable> LObjBoolConsumerX<T, X> safe() {
		return Function4U::doNothing;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <T, X extends Throwable, Y extends Throwable> LSupplierX<LObjBoolConsumerX<T, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <T, X extends Throwable> LObjBoolConsumerX<T, X> safe(final @Nullable LObjBoolConsumerX<T, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <T, X extends Throwable, Y extends Throwable> LSupplierX<LObjBoolConsumerX<T, X>, Y> safeSupplier(final @Nullable LSupplierX<LObjBoolConsumerX<T, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	@Nonnull
	default <V1> LObjBoolConsumerX<V1, X> objBoolConsComposeBool(@Nonnull final LFunctionX<? super V1, ? extends T, X> before1, @Nonnull final LLogicalOperatorX<X> before2) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
		return (V1 v1, boolean v2) -> this.doAccept(before1.doApply(v1), before2.doApply(v2));
	}
	...
	/** Combines two LObjBoolConsumerX<T,X> together in a order. */
	@Nonnull
	...
	/** Permutation of LObjBoolConsumerX for method references. */
	@FunctionalInterface
	interface V1<T, X extends Throwable> extends LObjBoolConsumerX<T, X> {

		void doAcceptV1(boolean a2, T a1) throws X;

		@Override
		default void doAccept(T a1, boolean a2) throws X {
