...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.FloatToShortFunction}
 */
@FunctionalInterface
...
public interface FloatToShortFunctionX<X extends Exception> extends MetaFunction, PrimitiveCodomain<FloatToShortFunctionX<X>>, MetaThrowingInterface<X> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends Exception> FloatToShortFunctionX<Y> wrapException(@Nonnull final FloatToShortFunctionX<X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (float f) -> {
			try {
	...
	@Nonnull
	default <Y extends Exception> FloatToShortFunctionX<Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return FloatToShortFunctionX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> FloatToShortFunctionX<Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return FloatToShortFunctionX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
