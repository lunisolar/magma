...
/**
 * A consumer.
 * @see {@link eu.lunisolar.magma.func.consumer.primitives.bi.ShortBiConsumerX}
 */
@FunctionalInterface
@SuppressWarnings("UnusedDeclaration")
public interface ShortBiConsumer extends MetaConsumer {
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> ShortBiConsumer wrapException(@Nonnull final ShortBiConsumer other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (short s1, short s2) -> {
			try {
				other.accept(s1, s2);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> ShortBiConsumer handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
	...
	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> ShortBiConsumer handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);
