...
public interface LObjBytePredicateX<T, X extends Throwable> extends MetaPredicate, MetaInterface.Throwing<X> { // NOSONAR

	String DESCRIPTION = "LObjBytePredicateX: boolean doTest(T a1,byte a2) throws X";

	boolean doTest(T a1, byte a2) throws X;

	default boolean tupleTest(LObjBytePair<T> args) throws X {
	...
	default boolean shovingDoTest(T a1, byte a2) {
		return ((LObjBytePredicateX<T, RuntimeException>) this).doTest(a1, a2);
	}

	/** Function call that handles exceptions according to the instructions. */
	default <Y extends Throwable> boolean handlingDoTest(T a1, byte a2, HandlingInstructions<Throwable, Y> handling) throws Y {
	...
	default boolean nonNullDoTest(T a1, byte a2) throws X {
		return doTest(a1, a2);
	}

	/** For convenience, where "test()" makes things more confusing than "applyAsBoolean()". */

	default boolean doApplyAsBoolean(T a1, byte a2) throws X {
		return doTest(a1, a2);
	}
	...
	default LBoolSupplierX<X> captureObjBytePred(T a1, byte a2) {
		return () -> this.doTest(a1, a2);
	}
	...
	@Nonnull
	static <T, X extends Throwable> LObjBytePredicateX<T, X> lX(final @Nonnull LObjBytePredicateX<T, X> lambda) {
	...
	/** Convenient method in case lambda expression is ambiguous for the compiler (that might happen for overloaded methods accepting different interfaces). */
	@Nonnull
	static <T, X extends Throwable> LObjBytePredicateX<T, X> lX(@Nonnull Class<X> xClass, final @Nonnull LObjBytePredicateX<T, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda;
	}
	...
	static <T, X extends Throwable> boolean call(T a1, byte a2, final @Nonnull LObjBytePredicateX<T, X> lambda) throws X {
		Null.nonNullArg(lambda, "lambda");
		return lambda.doTest(a1, a2);
	}

	static <T, X extends Throwable> boolean shoving(T a1, byte a2, final @Nonnull LObjBytePredicateX<T, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.shovingDoTest(a1, a2);
	}

	static <T, X extends Throwable> boolean nesting(T a1, byte a2, final @Nonnull LObjBytePredicateX<T, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.nestingDoTest(a1, a2);
	}

	static <T, X extends Throwable, Y extends Throwable> boolean handling(T a1, byte a2, final HandlingInstructions<Throwable, Y> handling, final @Nonnull LObjBytePredicateX<T, X> lambda) throws Y {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	static <T, X extends Throwable> LObjBytePredicateX<T, X> safe() {
		return Function4U::alwaysFalse;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <T, X extends Throwable, Y extends Throwable> LSupplierX<LObjBytePredicateX<T, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <T, X extends Throwable> LObjBytePredicateX<T, X> safe(final @Nullable LObjBytePredicateX<T, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <T, X extends Throwable, Y extends Throwable> LSupplierX<LObjBytePredicateX<T, X>, Y> safeSupplier(final @Nullable LSupplierX<LObjBytePredicateX<T, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	default LObjBytePredicateX<T, X> negate() {
		return (T a1, byte a2) -> !doTest(a1, a2);
	}
	 ...
	 */
	@Nonnull
	static <T, X extends Throwable> LObjBytePredicateX<T, X> isEqual(T v1, byte v2) {
	...
	default <V1> LObjBytePredicateX<V1, X> objBytePredComposeByte(@Nonnull final LFunctionX<? super V1, ? extends T, X> before1, @Nonnull final LByteUnaryOperatorX<X> before2) {
		...
		return (V1 v1, byte v2) -> this.doTest(before1.doApply(v1), before2.doApplyAsByte(v2));
	}
	...
	default <V> LObjByteFunctionX<T, V, X> boolToObjByteFunction(@Nonnull LBoolFunctionX<? extends V, X> after) {
		...
		return (T a1, byte a2) -> after.doApply(this.doTest(a1, a2));
	}
	...
	default LObjBytePredicate<T> handleObjBytePred(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
		return (T a1, byte a2) -> this.handlingDoTest(a1, a2, handling);
	}
	...
	/** Permutation of LObjBytePredicateX for method references. */
	@FunctionalInterface
	interface V1<T, X extends Throwable> extends LObjBytePredicateX<T, X> {

		boolean doTestV1(byte a2, T a1) throws X;

		@Override
		default boolean doTest(T a1, byte a2) throws X {
