...
public interface LFloatFunctionX<R, X extends Throwable> extends MetaFunction, MetaInterface.Throwing<X> { // NOSONAR
	...
	String DESCRIPTION = "LFloatFunctionX: R doApply(float a) throws X";

	@Nullable
	R doApply(float a) throws X;
	...
	default R shovingDoApply(float a) {
		return ((LFloatFunctionX<R, RuntimeException>) this).doApply(a);
	}

	/** Function call that handles exceptions according to the instructions. */
	default <Y extends Throwable> R handlingDoApply(float a, HandlingInstructions<Throwable, Y> handling) throws Y {
	...
	@Nonnull
	default R nonNullDoApply(float a) throws X {
	...
	default LSupplierX<R, X> captureFloatFunc(float a) {
		return () -> this.doApply(a);
	}
	...
	static <R, X extends Throwable> R call(float a, final @Nonnull LFloatFunctionX<R, X> lambda) throws X {
		Null.nonNullArg(lambda, "lambda");
		return lambda.doApply(a);
	}

	static <R, X extends Throwable> R shoving(float a, final @Nonnull LFloatFunctionX<R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.shovingDoApply(a);
	}

	static <R, X extends Throwable> R nesting(float a, final @Nonnull LFloatFunctionX<R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.nestingDoApply(a);
	}

	static <R, X extends Throwable, Y extends Throwable> R handling(float a, final HandlingInstructions<Throwable, Y> handling, final @Nonnull LFloatFunctionX<R, X> lambda) throws Y {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	static <R, X extends Throwable> LFloatFunctionX<R, X> safe() {
		return Function4U::produce;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <R, X extends Throwable, Y extends Throwable> LSupplierX<LFloatFunctionX<R, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <R, X extends Throwable> LFloatFunctionX<R, X> safe(final @Nullable LFloatFunctionX<R, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <R, X extends Throwable, Y extends Throwable> LSupplierX<LFloatFunctionX<R, X>, Y> safeSupplier(final @Nullable LSupplierX<LFloatFunctionX<R, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	@Nonnull
	default LFloatFunctionX<R, X> floatFuncComposeFloat(@Nonnull final LFloatUnaryOperatorX<X> before) {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V> LFunctionX<V, R, X> floatFuncCompose(@Nonnull final LToFloatFunctionX<? super V, X> before) {
		Null.nonNullArg(before, "before");
		return v -> this.doApply(before.doApplyAsFloat(v));
	}
