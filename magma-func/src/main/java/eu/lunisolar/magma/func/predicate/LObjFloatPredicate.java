...
public interface LObjFloatPredicate<T> extends LObjFloatPredicateX<T, RuntimeException>, MetaPredicate, MetaInterface.NonThrowing { // NOSONAR

	String DESCRIPTION = "LObjFloatPredicate: boolean doTest(T a1,float a2)";

	boolean doTest(T a1, float a2);

	default boolean tupleTest(LObjFloatPair<T> args) {
	...
	default LBoolSupplier captureObjFloatPred(T a1, float a2) {
		return () -> this.doTest(a1, a2);
	}
	...
	@Nonnull
	static <T> LObjFloatPredicate<T> test1st(@Nonnull LPredicate<T> func) {
		return (a1, a2) -> func.doTest(a1);
	}

	/** Captures single parameter function into this interface where only 2nd parameter will be used. */
	@Nonnull
	static <T> LObjFloatPredicate<T> test2nd(@Nonnull LFloatPredicate func) {
	...
	@Nonnull
	static <T> LObjFloatPredicate<T> safe() {
		return Function4U::alwaysFalse;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <T> LSupplier<LObjFloatPredicate<T>> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <T> LObjFloatPredicate<T> safe(final @Nullable LObjFloatPredicate<T> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <T> LSupplier<LObjFloatPredicate<T>> safeSupplier(final @Nullable LSupplier<LObjFloatPredicate<T>> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	 ...
	 */
	@Nonnull
	static <T> LObjFloatPredicate<T> isEqual(T v1, float v2) {
	...
	/** Permutation of LObjFloatPredicate for method references. */
	@FunctionalInterface
	interface V1<T> extends LObjFloatPredicate<T> {

		boolean doTestV1(float a2, T a1);
