...
public interface LBiObjCharConsumer<T1, T2> extends LBiObjCharConsumerX<T1, T2, RuntimeException>, MetaConsumer, MetaInterface.NonThrowing {
	...
	public static final String DESCRIPTION = "LBiObjCharConsumer: void doAccept(T1 t1,T2 t2, char c)";

	public void doAccept(T1 t1, T2 t2, char c);
	...
	@Nonnull
	public static <T1, T2> LBiObjCharConsumer<T1, T2> l(final @Nonnull LBiObjCharConsumer<T1, T2> lambda) {
	...
	@Nonnull
	public static <T1, T2, X extends Exception> LBiObjCharConsumer<T1, T2> wrap(final @Nonnull LBiObjCharConsumerX<T1, T2, X> other) {
	...
	@Nonnull
	public static <T1, T2, X extends Exception, E extends Exception, Y extends RuntimeException> LBiObjCharConsumer<T1, T2> wrapException(@Nonnull final LBiObjCharConsumer<T1, T2> other, Class<E> exception, ExceptionHandler<E, Y> handler) {
		return (T1 t1, T2 t2, char c) -> {
			try {
	...
	@Nonnull
	default <E extends Exception, Y extends RuntimeException> LBiObjCharConsumer<T1, T2> handle(Class<E> exception, ExceptionHandler<E, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LBiObjCharConsumer.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for any exception (including unchecked exception that might be different from X) will call handler function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> LBiObjCharConsumer<T1, T2> handle(ExceptionHandler<Exception, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LBiObjCharConsumer.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
