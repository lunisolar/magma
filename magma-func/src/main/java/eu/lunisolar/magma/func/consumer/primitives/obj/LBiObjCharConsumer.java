...
public interface LBiObjCharConsumer<T1, T2> extends MetaConsumer, MetaInterface.NonThrowing, Codomain<aVoid>, Domain3<a<T1>, a<T2>, aChar> {
	...
	default LObjCharConsumer<T2> _with(T1 a1) {
		return (a2, a3) -> accept(a1, a2, a3);
	}

	default LBiConsumer<T1, T2> with(char a3) {
		return (a1, a2) -> accept(a1, a2, a3);
	}

	default LCharConsumer _with(T1 a1, T2 a2) {
		return a3 -> accept(a1, a2, a3);
	}

	default LConsumer<T1> with(T2 a2, char a3) {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1, V2, V3> LTriConsumer<V1, V2, V3> unboxingCompose(@Nonnull final LFunction<? super V1, ? extends T1> before1, @Nonnull final LFunction<? super V2, ? extends T2> before2, @Nonnull final LToCharFunction<? super V3> before3) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
		Null.nonNullArg(before3, "before3");
		return (v1, v2, v3) -> this.accept(before1.apply(v1), before2.apply(v2), before3.applyAsChar(v3));
	...
	default LBiObjCharConsumer<T1, T2> shoving() {

		return new LBiObjCharConsumer<T1, T2>() {

			public void accept(T1 a1, T2 a2, char a3) {
				try {
					this.acceptX(a1, a2, a3);
				} catch (Throwable e) { // NOSONAR
					Handling.handleErrors(e);
			...
			public void acceptX(T1 a1, T2 a2, char a3) throws Throwable {
				LBiObjCharConsumer.this.acceptX(a1, a2, a3);
			}
