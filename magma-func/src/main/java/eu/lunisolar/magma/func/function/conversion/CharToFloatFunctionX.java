...
/**
 * Throwing functional interface (lambda) CharToFloatFunctionX for Java 8.
 *
 * Type: function
 *
 * Domain (lvl: 1): char c
...
public interface CharToFloatFunctionX<X extends Exception> extends MetaFunction, PrimitiveCodomain<Object>, MetaInterface.Throwing<X> { // NOSONAR
	...
	default CharToFloatFunctionX<RuntimeException> uncheck() {
		return (CharToFloatFunctionX) this;
	}
	...
	@Nonnull
	default <E extends Exception, Y extends Exception> CharToFloatFunctionX<Y> handleX(Class<E> exception, ExceptionHandler<E, Y> handler) {
	...
	@Nonnull
	default <Y extends Exception> CharToFloatFunctionX<Y> handleX(ExceptionHandler<Exception, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);
	...
	@Nonnull
	default <E extends Exception, Y extends Exception> CharToFloatFunctionX<Y> handleX(Class<E> exception, FloatSupplierX<X> supplier) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
	...
	@Nonnull
	default <Y extends Exception> CharToFloatFunctionX<Y> handleX(FloatSupplierX<X> supplier) {
		Objects.requireNonNull(supplier, Function4U.VALIDATION_MESSAGE_HANDLER);
