...
public interface LObjFltFunction<T, R> extends MetaFunction, MetaInterface.NonThrowing, Codomain<a<R>>, Domain2<a<T>, aFloat> { // NOSONAR
	 ...
	 */
	public static <T, R> M<T, R> memento(R initialValue, LObjFltFunction<T, R> baseFunction, LBinaryOperator<R> mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LObjFltFunction.D)
	 */
	final class M<T, R> implements LObjFltFunction<T, R> {
		...
		private M(R lastValue, LObjFltFunction<T, R> baseFunction, LBinaryOperator<R> mementoFunction) {
