...
public interface LObjFltFunction<T, R> extends MetaFunction, MetaInterface.NonThrowing, Codomain<a<R>>, Domain2<a<T>, aFloat> { // NOSONAR
	...
	public static <M, K, V, R> R from(@Nonnull M container, LBiFunction<M, K, V> extractor, K key, float a2, @Nonnull LObjFltFunction<V, R> function) {
	...
	final class M<T, R> implements LObjFltFunction<T, R> {
		...
		public R currentApply(T a1, float a2) {
			R x1 = lastBaseValue;
		...
		public R currentBaseValue(T a1, float a2) {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1, V2> LBiFunction<V1, V2, R> unboxingCompose(@Nonnull final LFunction<? super V1, ? extends T> before1, @Nonnull final LToFltFunction<? super V2> before2) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
		return (v1, v2) -> this.apply(before1.apply(v1), before2.applyAsFlt(v2));
	}
