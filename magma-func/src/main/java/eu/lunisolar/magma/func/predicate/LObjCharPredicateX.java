...
public interface LObjCharPredicateX<T, X extends Throwable> extends MetaPredicate, MetaInterface.Throwing<X> { // NOSONAR

	String DESCRIPTION = "LObjCharPredicateX: boolean doTest(T a1,char a2) throws X";

	boolean doTest(T a1, char a2) throws X;

	default boolean tupleTest(LObjCharPair<T> args) throws X {
	...
	default boolean shovingDoTest(T a1, char a2) {
		return ((LObjCharPredicateX<T, RuntimeException>) this).doTest(a1, a2);
	}

	/** Function call that handles exceptions according to the instructions. */
	default <Y extends Throwable> boolean handlingDoTest(T a1, char a2, HandlingInstructions<Throwable, Y> handling) throws Y {
	...
	default boolean nonNullDoTest(T a1, char a2) throws X {
		return doTest(a1, a2);
	}

	/** For convenience, where "test()" makes things more confusing than "applyAsBoolean()". */

	default boolean doApplyAsBoolean(T a1, char a2) throws X {
		return doTest(a1, a2);
	}
	...
	default LBoolSupplierX<X> captureObjCharPred(T a1, char a2) {
		return () -> this.doTest(a1, a2);
	}
	...
	static <T, X extends Throwable> boolean call(T a1, char a2, final @Nonnull LObjCharPredicateX<T, X> lambda) throws X {
		Null.nonNullArg(lambda, "lambda");
		return lambda.doTest(a1, a2);
	}

	static <T, X extends Throwable> boolean shoving(T a1, char a2, final @Nonnull LObjCharPredicateX<T, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.shovingDoTest(a1, a2);
	}

	static <T, X extends Throwable> boolean nesting(T a1, char a2, final @Nonnull LObjCharPredicateX<T, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.nestingDoTest(a1, a2);
	}

	static <T, X extends Throwable, Y extends Throwable> boolean handling(T a1, char a2, final HandlingInstructions<Throwable, Y> handling, final @Nonnull LObjCharPredicateX<T, X> lambda) throws Y {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	static <T, X extends Throwable> LObjCharPredicateX<T, X> safe() {
		return Function4U::alwaysFalse;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <T, X extends Throwable, Y extends Throwable> LSupplierX<LObjCharPredicateX<T, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <T, X extends Throwable> LObjCharPredicateX<T, X> safe(final @Nullable LObjCharPredicateX<T, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <T, X extends Throwable, Y extends Throwable> LSupplierX<LObjCharPredicateX<T, X>, Y> safeSupplier(final @Nullable LSupplierX<LObjCharPredicateX<T, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	 ...
	 */
	@Nonnull
	static <T, X extends Throwable> LObjCharPredicateX<T, X> isEqual(T v1, char v2) {
	...
	@Nonnull
	default <V> LObjCharFunctionX<T, V, X> boolToObjCharFunc(@Nonnull LBoolFunctionX<? extends V, X> after) {
	...
	/** Permutation of LObjCharPredicateX for method references. */
	@FunctionalInterface
	interface V1<T, X extends Throwable> extends LObjCharPredicateX<T, X> {

		boolean doTestV1(char a2, T a1) throws X;

		@Override
		default boolean doTest(T a1, char a2) throws X {
