 ...
 * Non-throwing interface/lambda variant: BiObjFloatConsumer
 *
...
public interface BiObjFloatConsumerX<T1, T2, X extends Exception> extends MetaConsumer, MetaThrowingInterface<X> {
	...
	@Nonnull
	default <E extends Exception, Y extends Exception> BiObjFloatConsumerX<T1, T2, Y> handle(Class<E> exception, ExceptionHandler<E, Y> handler) {
	...
	@Nonnull
	default <Y extends Exception> BiObjFloatConsumerX<T1, T2, Y> handle(ExceptionHandler<Exception, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);
