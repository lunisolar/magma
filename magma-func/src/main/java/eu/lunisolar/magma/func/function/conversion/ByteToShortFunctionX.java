...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.ByteToShortFunction}
 */
@FunctionalInterface
...
public interface ByteToShortFunctionX<X extends Exception> extends MetaFunction, PrimitiveCodomain<ByteToShortFunctionX<X>>, MetaThrowingInterface<X> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends Exception> ByteToShortFunctionX<Y> wrapException(@Nonnull final ByteToShortFunctionX<X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (byte b) -> {
			try {
	...
	@Nonnull
	default <Y extends Exception> ByteToShortFunctionX<Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ByteToShortFunctionX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> ByteToShortFunctionX<Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ByteToShortFunctionX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
