...
public interface LBiFunction<T1, T2, R> extends BiFunction<T1, T2, R>, MetaFunction, MetaInterface.NonThrowing, Codomain<a<R>>, Domain2<a<T1>, a<T2>> { // NOSONAR
	...
	default @Nonnull LBiFunction<T1, T2, R> wrapWith(@Nonnull CallContext ctx) {
		Null.nonNullArg(ctx, "ctx");
		return (a1, a2) -> applyX(ctx, a1, a2, this);
	}

	static <T1, T2, R> R applyX(@Nonnull CallContext ctx, T1 a1, T2 a2, @Nonnull LBiFunction<T1, T2, R> function) throws Throwable {
		Null.nonNullArg(ctx, "ctx");
		Null.nonNullArg(function, "function");
		return (R) ctx.call(() -> function.applyX(a1, a2));
	}

	static <T1, T2, R> R nestingApply(@Nonnull CallContext ctx, T1 a1, T2 a2, @Nonnull LBiFunction<T1, T2, R> function) {
		Null.nonNullArg(ctx, "ctx");
		Null.nonNullArg(function, "function");
		try {
			return applyX(ctx, a1, a2, function);
		} catch (Throwable e) {
			throw Handling.nestCheckedAndThrow(e);
		}
	}

	static <T1, T2, R> R shovingApply(@Nonnull CallContext ctx, T1 a1, T2 a2, @Nonnull LBiFunction<T1, T2, R> function) {
		Null.nonNullArg(ctx, "ctx");
		Null.nonNullArg(function, "function");
		try {
			return applyX(ctx, a1, a2, function);
	...
	static <T1, T2, R> CompletableFuture<R> asyncApply(@Nonnull AsyncCallContext async, @Nonnull CallContext ctx, T1 a1, T2 a2, @Nonnull LBiFunction<T1, T2, R> function) {
		Null.nonNullArg(async, "async");
		Null.nonNullArg(ctx, "ctx");
		Null.nonNullArg(function, "function");
		CompletableFuture<R> future = new CompletableFuture<>();
		try {
			async.call(() -> {
				try {
					var v = LBiFunction.applyX(ctx, a1, a2, function);
					future.complete(v);
				} catch (Throwable e) {
					Handling.handleErrors(e);
					future.completeExceptionally(e);
