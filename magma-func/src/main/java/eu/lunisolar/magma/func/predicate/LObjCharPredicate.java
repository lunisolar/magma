...
public interface LObjCharPredicate<T> extends MetaPredicate, MetaInterface.NonThrowing, Codomain<aBool>, Domain2<a<T>, aChar> { // NOSONAR
	 ...
	 */
	public static <T> M<T> memento(boolean initialValue, LObjCharPredicate<T> baseFunction, LLogicalBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LObjCharPredicate.D)
	 */
	final class M<T> implements LObjCharPredicate<T> {
		...
		private M(boolean lastValue, LObjCharPredicate<T> baseFunction, LLogicalBinaryOperator mementoFunction) {
