...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.ShortToIntFunction}
 */
@FunctionalInterface
...
public interface ShortToIntFunctionX<X extends Exception> extends MetaFunction, PrimitiveCodomain<ShortToIntFunctionX<X>>, MetaThrowingInterface<X> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends Exception> ShortToIntFunctionX<Y> wrapException(@Nonnull final ShortToIntFunctionX<X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (short s) -> {
			try {
	...
	@Nonnull
	default <Y extends Exception> ShortToIntFunctionX<Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ShortToIntFunctionX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> ShortToIntFunctionX<Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ShortToIntFunctionX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
