...
public interface LObjBoolFunctionX<T, R, X extends Throwable> extends MetaFunction, MetaInterface.Throwing<X> { // NOSONAR
	...
	String DESCRIPTION = "LObjBoolFunctionX: R doApply(T a1,boolean a2) throws X";

	@Nullable
	R doApply(T a1, boolean a2) throws X;

	default R tupleApply(LObjBoolPair<T> args) throws X {
	...
	default R shovingDoApply(T a1, boolean a2) {
		return ((LObjBoolFunctionX<T, R, RuntimeException>) this).doApply(a1, a2);
	}

	/** Function call that handles exceptions according to the instructions. */
	default <Y extends Throwable> R handlingDoApply(T a1, boolean a2, HandlingInstructions<Throwable, Y> handling) throws Y {
	...
	@Nonnull
	default R nonNullDoApply(T a1, boolean a2) throws X {
	...
	default LSupplierX<R, X> captureObjBoolFunc(T a1, boolean a2) {
		return () -> this.doApply(a1, a2);
	}
	...
	static <T, R, X extends Throwable> R call(T a1, boolean a2, final @Nonnull LObjBoolFunctionX<T, R, X> lambda) throws X {
		Null.nonNullArg(lambda, "lambda");
		return lambda.doApply(a1, a2);
	}

	static <T, R, X extends Throwable> R shoving(T a1, boolean a2, final @Nonnull LObjBoolFunctionX<T, R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.shovingDoApply(a1, a2);
	}

	static <T, R, X extends Throwable> R nesting(T a1, boolean a2, final @Nonnull LObjBoolFunctionX<T, R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.nestingDoApply(a1, a2);
	}

	static <T, R, X extends Throwable, Y extends Throwable> R handling(T a1, boolean a2, final HandlingInstructions<Throwable, Y> handling, final @Nonnull LObjBoolFunctionX<T, R, X> lambda) throws Y {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	static <T, R, X extends Throwable, Y extends Throwable> LSupplierX<LObjBoolFunctionX<T, R, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <T, R, X extends Throwable> LObjBoolFunctionX<T, R, X> safe(final @Nullable LObjBoolFunctionX<T, R, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <T, R, X extends Throwable, Y extends Throwable> LSupplierX<LObjBoolFunctionX<T, R, X>, Y> safeSupplier(final @Nullable LSupplierX<LObjBoolFunctionX<T, R, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	/** Permutation of LObjBoolFunctionX for method references. */
	@FunctionalInterface
