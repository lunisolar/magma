...
public interface LObjDblPredicate<T> extends MetaPredicate, MetaInterface.NonThrowing, Codomain<aBool>, Domain2<a<T>, aDouble> { //NOSONAR
	...
	default LDblPredicate _with(T a1) {
		return a2 -> test(a1, a2);
	}

	default LPredicate<T> with(double a2) {
	...
	final class M<T> implements LObjDblPredicate<T> {
		...
		public boolean currentTest(T a1, double a2) {
			boolean x1 = lastBaseValue;
		...
		public boolean currentBaseValue(T a1, double a2) {
	...
	default LObjDblPredicate<T> shoving() {

		return new LObjDblPredicate<T>() {

			public boolean test(T a1, double a2) {
				try {
					return this.testX(a1, a2);
				} catch (Throwable e) { // NOSONAR
					Handling.handleErrors(e);
			...
			public boolean testX(T a1, double a2) throws Throwable {
				return LObjDblPredicate.this.testX(a1, a2);
			}
