...
public interface LObjBoolPredicate<T> extends MetaPredicate, MetaInterface.NonThrowing, Codomain<aBool>, Domain2<a<T>, aBool> { // NOSONAR
	 ...
	 */
	public static <T> M<T> memento(boolean initialValue, LObjBoolPredicate<T> baseFunction, LLogicalBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LObjBoolPredicate.D)
	 */
	final class M<T> implements LObjBoolPredicate<T> {
		...
		private M(boolean lastValue, LObjBoolPredicate<T> baseFunction, LLogicalBinaryOperator mementoFunction) {
