...
public interface LBiObjFltFunction<T1, T2, R> extends MetaFunction, MetaInterface.NonThrowing, Codomain<a<R>>, Domain3<a<T1>, a<T2>, aFloat> { // NOSONAR
	...
	public static <M, K, V, T2, R> R from(@Nonnull M container, LBiFunction<M, K, V> extractor, K key, T2 a2, float a3, @Nonnull LBiObjFltFunction<V, T2, R> function) {
	...
	default LObjFltFunction<T2, R> _with(T1 a1) {
		return (a2, a3) -> apply(a1, a2, a3);
	}

	default LBiFunction<T1, T2, R> with(float a3) {
		return (a1, a2) -> apply(a1, a2, a3);
	}

	default LFltFunction<R> _with(T1 a1, T2 a2) {
		return a3 -> apply(a1, a2, a3);
	}

	default LFunction<T1, R> with(T2 a2, float a3) {
	...
	final class M<T1, T2, R> implements LBiObjFltFunction<T1, T2, R> {
		...
		public R currentApply(T1 a1, T2 a2, float a3) {
			R x1 = lastBaseValue;
