...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.CharToFloatFunctionX}
 */
@FunctionalInterface
...
public interface CharToFloatFunction extends MetaFunction, PrimitiveCodomain<CharToFloatFunction> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> CharToFloatFunction wrapException(@Nonnull final CharToFloatFunction other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (char c) -> {
			try {
				return other.applyAsFloat(c);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> CharToFloatFunction handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return CharToFloatFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> CharToFloatFunction handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return CharToFloatFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
