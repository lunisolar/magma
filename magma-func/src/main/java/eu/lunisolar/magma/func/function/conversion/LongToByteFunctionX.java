...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.LongToByteFunction}
 */
@FunctionalInterface
...
public interface LongToByteFunctionX<X extends Exception> extends MetaFunction, PrimitiveCodomain<LongToByteFunctionX<X>>, MetaThrowingInterface<X> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends Exception> LongToByteFunctionX<Y> wrapException(@Nonnull final LongToByteFunctionX<X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (long l) -> {
			try {
	...
	@Nonnull
	default <Y extends Exception> LongToByteFunctionX<Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LongToByteFunctionX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> LongToByteFunctionX<Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LongToByteFunctionX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
