...
public interface LTieCharFunction<T> extends MetaFunction, MetaInterface.NonThrowing, TieFunction<T, aChar>, Codomain<aInt>, Domain3<a<T>, aInt, aChar> { // NOSONAR
	 ...
	 */
	public static <T> M<T> memento(int initialValue, LTieCharFunction<T> baseFunction, LIntBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LTieCharFunction.D)
	 */
	final class M<T> implements LTieCharFunction<T> {
		...
		private M(int lastValue, LTieCharFunction<T> baseFunction, LIntBinaryOperator mementoFunction) {
