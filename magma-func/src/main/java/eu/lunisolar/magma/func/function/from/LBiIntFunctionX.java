...
public interface LBiIntFunctionX<R, X extends Throwable> extends MetaFunction, MetaInterface.Throwing<X> { // NOSONAR
	...
	String DESCRIPTION = "LBiIntFunctionX: R doApply(int a1,int a2) throws X";

	@Nullable
	R doApply(int a1, int a2) throws X;

	default R tupleApply(LIntPair args) throws X {
	...
	default R shovingDoApply(int a1, int a2) {
		return ((LBiIntFunctionX<R, RuntimeException>) this).doApply(a1, a2);
	}

	/** Function call that handles exceptions according to the instructions. */
	default <Y extends Throwable> R handlingDoApply(int a1, int a2, HandlingInstructions<Throwable, Y> handling) throws Y {
	...
	@Nonnull
	default R nonNullDoApply(int a1, int a2) throws X {
	...
	default LSupplierX<R, X> captureBiIntFunc(int a1, int a2) {
		return () -> this.doApply(a1, a2);
	}
	...
	static <R, X extends Throwable> R call(int a1, int a2, final @Nonnull LBiIntFunctionX<R, X> lambda) throws X {
		Null.nonNullArg(lambda, "lambda");
		return lambda.doApply(a1, a2);
	}

	static <R, X extends Throwable> R shoving(int a1, int a2, final @Nonnull LBiIntFunctionX<R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.shovingDoApply(a1, a2);
	}

	static <R, X extends Throwable> R nesting(int a1, int a2, final @Nonnull LBiIntFunctionX<R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.nestingDoApply(a1, a2);
	}

	static <R, X extends Throwable, Y extends Throwable> R handling(int a1, int a2, final HandlingInstructions<Throwable, Y> handling, final @Nonnull LBiIntFunctionX<R, X> lambda) throws Y {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	static <R, X extends Throwable> LBiIntFunctionX<R, X> safe() {
		return Function4U::produce;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <R, X extends Throwable, Y extends Throwable> LSupplierX<LBiIntFunctionX<R, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <R, X extends Throwable> LBiIntFunctionX<R, X> safe(final @Nullable LBiIntFunctionX<R, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <R, X extends Throwable, Y extends Throwable> LSupplierX<LBiIntFunctionX<R, X>, Y> safeSupplier(final @Nullable LSupplierX<LBiIntFunctionX<R, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	/** Permutation of LBiIntFunctionX for method references. */
	@FunctionalInterface
	interface V1<R, X extends Throwable> extends LBiIntFunctionX<R, X> {
		@Nullable
		R doApplyV1(int a2, int a1) throws X;

		@Override
		default R doApply(int a1, int a2) throws X {
