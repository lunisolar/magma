...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.from.CharFunctionX}
 */
@FunctionalInterface
@SuppressWarnings("UnusedDeclaration")
public interface CharFunction<R> extends MetaFunction { // NOSONAR
	...
	@Nonnull
	public static <R, X extends Exception, Y extends RuntimeException> CharFunction<R> wrapException(@Nonnull final CharFunction<R> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (char c) -> {
			try {
				return other.apply(c);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> CharFunction<R> handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return CharFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> CharFunction<R> handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return CharFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
