...
public interface LFltIntPredicate extends MetaPredicate, MetaInterface.NonThrowing, Codomain<aBool>, Domain2<aFloat, aInt> { // NOSONAR
	 ...
	 */
	public static M memento(boolean initialValue, LFltIntPredicate baseFunction, LLogicalBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LFltIntPredicate.D)
	 */
	final class M implements LFltIntPredicate {
		...
		private M(boolean lastValue, LFltIntPredicate baseFunction, LLogicalBinaryOperator mementoFunction) {
