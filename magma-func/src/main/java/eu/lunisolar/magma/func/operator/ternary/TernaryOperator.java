...
/**
 *
 * @see {@link eu.lunisolar.magma.func.operator.ternary.TernaryOperatorX}
 */
@FunctionalInterface
...
public interface TernaryOperator<T> extends MetaOperator { // NOSONAR
	...
	@Nonnull
	public static <T, X extends Exception, Y extends RuntimeException> TernaryOperator<T> wrapException(@Nonnull final TernaryOperator<T> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (T t1, T t2, T t3) -> {
			try {
				return other.apply(t1, t2, t3);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> TernaryOperator<T> handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return TernaryOperator.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> TernaryOperator<T> handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return TernaryOperator.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
