...
/**
 *
 * @see {@link eu.lunisolar.magma.func.predicate.ObjCharPredicate}
 */
@FunctionalInterface
@SuppressWarnings("UnusedDeclaration")
public interface ObjCharPredicateX<T, X extends Exception> extends MetaPredicate, PrimitiveCodomain<ObjCharPredicateX<T, X>>, MetaThrowingInterface<X> { // NOSONAR
	...
	@Nonnull
	public static <T, X extends Exception, Y extends Exception> ObjCharPredicateX<T, Y> wrapException(@Nonnull final ObjCharPredicateX<T, X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (T t, char c) -> {
			try {
	...
	@Nonnull
	default <Y extends Exception> ObjCharPredicateX<T, Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ObjCharPredicateX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> ObjCharPredicateX<T, Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ObjCharPredicateX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
