...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.CharToLongFunctionX}
 */
@FunctionalInterface
...
public interface CharToLongFunction extends MetaFunction, PrimitiveCodomain<CharToLongFunction> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> CharToLongFunction wrapException(@Nonnull final CharToLongFunction other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (char c) -> {
			try {
				return other.applyAsLong(c);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> CharToLongFunction handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return CharToLongFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> CharToLongFunction handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return CharToLongFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
