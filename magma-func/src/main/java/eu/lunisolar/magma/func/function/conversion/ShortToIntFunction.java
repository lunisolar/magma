...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.ShortToIntFunctionX}
 */
@FunctionalInterface
...
public interface ShortToIntFunction extends MetaFunction, PrimitiveCodomain<ShortToIntFunction> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> ShortToIntFunction wrapException(@Nonnull final ShortToIntFunction other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (short s) -> {
			try {
				return other.applyAsInt(s);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> ShortToIntFunction handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ShortToIntFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> ShortToIntFunction handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ShortToIntFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
