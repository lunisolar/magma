...
public interface LBinaryOperatorX<T, X extends Throwable> extends BinaryOperator<T>, MetaOperator, MetaInterface.Throwing<X> { // NOSONAR

	String DESCRIPTION = "LBinaryOperatorX: T doApply(T a1,T a2) throws X";

	/**
	 * Default implementation for JRE method that calls exception nesting method.
	 * @deprecated Calling this method via LBinaryOperatorX interface should be discouraged.
	 */
	@Override
	@Deprecated
	...
	@Nonnull
	static <T, X extends Throwable> LBinaryOperatorX<T, X> safe() {
		return Function4U::produce;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <T, X extends Throwable, Y extends Throwable> LSupplierX<LBinaryOperatorX<T, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <T, X extends Throwable> LBinaryOperatorX<T, X> safe(final @Nullable LBinaryOperatorX<T, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <T, X extends Throwable, Y extends Throwable> LSupplierX<LBinaryOperatorX<T, X>, Y> safeSupplier(final @Nullable LSupplierX<LBinaryOperatorX<T, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	 ...
	 */
	@Nonnull
	static <T, X extends Throwable> LBinaryOperatorX<T, X> minBy(@Nonnull Comparator<T> comparator) {
	 ...
	 */
	@Nonnull
	static <T, X extends Throwable> LBinaryOperatorX<T, X> maxBy(@Nonnull Comparator<T> comparator) {
		Null.nonNullArg(comparator, "comparator");
		return (a, b) -> comparator.compare(a, b) >= 0 ? a : b;
	}
