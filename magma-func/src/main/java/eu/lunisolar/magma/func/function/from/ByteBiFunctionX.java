...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.from.ByteBiFunction}
 */
@FunctionalInterface
...
public interface ByteBiFunctionX<R, X extends Exception> extends MetaFunction, MetaThrowingInterface<X> { // NOSONAR
	...
	@Nonnull
	public static <R, X extends Exception, Y extends Exception> ByteBiFunctionX<R, Y> wrapException(@Nonnull final ByteBiFunctionX<R, X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
	...
	default ByteBiFunctionX<R, X> nonNullableX() {
		return new NonNullByteBiFunctionX(this);
	}
	...
	@Nonnull
	default <Y extends Exception> ByteBiFunctionX<R, Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ByteBiFunctionX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> ByteBiFunctionX<R, Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ByteBiFunctionX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
