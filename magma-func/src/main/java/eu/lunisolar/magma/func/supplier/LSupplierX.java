...
public interface LSupplierX<R, X extends Throwable> extends Supplier<R>, MetaSupplier, MetaInterface.Throwing<X> {
	...
	@Nonnull
	static <R, X extends Throwable> LSupplierX<R, X> safe() {
		return Function4U::produce;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <R, X extends Throwable, Y extends Throwable> LSupplierX<LSupplierX<R, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <R, X extends Throwable> LSupplierX<R, X> safe(final @Nullable LSupplierX<R, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <R, X extends Throwable, Y extends Throwable> LSupplierX<LSupplierX<R, X>, Y> safeSupplier(final @Nullable LSupplierX<LSupplierX<R, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
