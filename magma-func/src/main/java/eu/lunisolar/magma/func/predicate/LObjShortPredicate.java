...
public interface LObjShortPredicate<T> extends LObjShortPredicateX<T, RuntimeException>, MetaPredicate, MetaInterface.NonThrowing { // NOSONAR
	...
	@Nonnull
	static <T> LObjShortPredicate<T> safe() {
		return Function4U::alwaysFalse;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <T> LSupplier<LObjShortPredicate<T>> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <T> LObjShortPredicate<T> safe(final @Nullable LObjShortPredicate<T> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <T> LSupplier<LObjShortPredicate<T>> safeSupplier(final @Nullable LSupplier<LObjShortPredicate<T>> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	@Nonnull
	default <V> LObjShortFunction<T, V> boolToObjShortFunc(@Nonnull LBoolFunction<? extends V> after) {
	...
	/** Permutation of LObjShortPredicate for method references. */
	@FunctionalInterface
	interface V1<T> extends LObjShortPredicate<T> {

		boolean doTestV1(short a2, T a1);
