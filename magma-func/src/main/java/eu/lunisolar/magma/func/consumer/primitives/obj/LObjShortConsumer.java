...
public interface LObjShortConsumer<T> extends LObjShortConsumerX<T, RuntimeException>, MetaConsumer, MetaInterface.NonThrowing {
	...
	String DESCRIPTION = "LObjShortConsumer: void doAccept(T a1,short a2)";

	void doAccept(T a1, short a2);

	default LTuple.Void tupleAccept(LObjShortPair<T> args) {
	...
	default LAction captureObjShortCons(T a1, short a2) {
	...
	/** Safe instance. */
	@Nonnull
	static <T> LObjShortConsumer<T> safe() {
		return Function4U::doNothing;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <T> LSupplier<LObjShortConsumer<T>> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <T> LObjShortConsumer<T> safe(final @Nullable LObjShortConsumer<T> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <T> LSupplier<LObjShortConsumer<T>> safeSupplier(final @Nullable LSupplier<LObjShortConsumer<T>> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	/** Combines two LObjShortConsumer<T> together in a order. */
	@Nonnull
	...
	/** Permutation of LObjShortConsumer for method references. */
	@FunctionalInterface
	interface V1<T> extends LObjShortConsumer<T> {

		void doAcceptV1(short a2, T a1);
