...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.from.BiObjIntFunctionX}
 */
@FunctionalInterface
...
public interface BiObjIntFunction<T1, T2, R> extends MetaFunction { // NOSONAR
	...
	@Nonnull
	public static <T1, T2, R, X extends Exception, Y extends RuntimeException> BiObjIntFunction<T1, T2, R> wrapException(@Nonnull final BiObjIntFunction<T1, T2, R> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (T1 t1, T2 t2, int i) -> {
			try {
				return other.apply(t1, t2, i);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> BiObjIntFunction<T1, T2, R> handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BiObjIntFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> BiObjIntFunction<T1, T2, R> handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BiObjIntFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
