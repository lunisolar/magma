...
public interface LTieIntConsumer<T> extends MetaConsumer, MetaInterface.NonThrowing, TieConsumer<T, aInt>, Codomain<aVoid>, Domain3<a<T>, aInt, aInt> {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1, V2, V3> LTriConsumer<V1, V2, V3> unboxingCompose(@Nonnull final LFunction<? super V1, ? extends T> before1, @Nonnull final LToIntFunction<? super V2> before2, @Nonnull final LToIntFunction<? super V3> before3) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
		Null.nonNullArg(before3, "before3");
		return (v1, v2, v3) -> this.accept(before1.apply(v1), before2.applyAsInt(v2), before3.applyAsInt(v3));
	...
	default LTieIntConsumer<T> shoving() {

		return new LTieIntConsumer<T>() {

			public void accept(T a1, int a2, int a3) {
				try {
					this.acceptX(a1, a2, a3);
				} catch (Throwable e) { // NOSONAR
					Handling.handleErrors(e);
			...
			public void acceptX(T a1, int a2, int a3) throws Throwable {
				LTieIntConsumer.this.acceptX(a1, a2, a3);
			}
