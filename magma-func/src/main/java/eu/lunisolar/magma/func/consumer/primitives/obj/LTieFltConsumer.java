...
public interface LTieFltConsumer<T> extends MetaConsumer, MetaInterface.NonThrowing, TieConsumer<T, aFloat>, Codomain<aVoid>, Domain3<a<T>, aInt, aFloat> {
	...
	default LObjIntConsumer<T> with(float a3) {
		return (a1, a2) -> accept(a1, a2, a3);
	}

	default LFltConsumer _with(T a1, int a2) {
		return a3 -> accept(a1, a2, a3);
	}

	default LConsumer<T> with(int a2, float a3) {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1, V2, V3> LTriConsumer<V1, V2, V3> unboxingCompose(@Nonnull final LFunction<? super V1, ? extends T> before1, @Nonnull final LToIntFunction<? super V2> before2, @Nonnull final LToFltFunction<? super V3> before3) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
		Null.nonNullArg(before3, "before3");
		return (v1, v2, v3) -> this.accept(before1.apply(v1), before2.applyAsInt(v2), before3.applyAsFlt(v3));
