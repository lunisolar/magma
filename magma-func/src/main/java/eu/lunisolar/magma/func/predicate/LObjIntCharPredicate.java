...
public interface LObjIntCharPredicate<T> extends MetaPredicate, MetaInterface.NonThrowing, Codomain<aBool>, Domain3<a<T>, aInt, aChar> { // NOSONAR
	 ...
	 */
	public static <T> M<T> memento(boolean initialValue, LObjIntCharPredicate<T> baseFunction, LLogicalBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LObjIntCharPredicate.D)
	 */
	final class M<T> implements LObjIntCharPredicate<T> {
		...
		private M(boolean lastValue, LObjIntCharPredicate<T> baseFunction, LLogicalBinaryOperator mementoFunction) {
