...
/**
 *
 * @see {@link eu.lunisolar.magma.func.predicate.BytePredicateX}
 */
@FunctionalInterface
...
public interface BytePredicate extends MetaPredicate, PrimitiveCodomain<BytePredicate> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> BytePredicate wrapException(@Nonnull final BytePredicate other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (byte b) -> {
			try {
				return other.test(b);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> BytePredicate handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BytePredicate.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> BytePredicate handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BytePredicate.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
