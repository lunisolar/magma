...
public interface LTieFltFunction<T> extends MetaFunction, MetaInterface.NonThrowing, TieFunction<T, aFloat>, Codomain<aInt>, Domain3<a<T>, aInt, aFloat> { // NOSONAR
	...
	final class M<T> implements LTieFltFunction<T> {
		...
		public int currentApplyAsInt(T a1, int a2, float a3) {
			int x1 = lastBaseValue;
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1, V2, V3> LToIntTriFunction<V1, V2, V3> unboxingCompose(@Nonnull final LFunction<? super V1, ? extends T> before1, @Nonnull final LToIntFunction<? super V2> before2, @Nonnull final LToFltFunction<? super V3> before3) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
		Null.nonNullArg(before3, "before3");
		return (v1, v2, v3) -> this.applyAsInt(before1.apply(v1), before2.applyAsInt(v2), before3.applyAsFlt(v3));
