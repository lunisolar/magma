...
/**
 *
 * @see {@link eu.lunisolar.magma.func.predicate.LongPredicate}
 */
@FunctionalInterface
...
public interface LongPredicateX<X extends Exception> extends MetaPredicate, PrimitiveCodomain<LongPredicateX<X>>, MetaThrowingInterface<X> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends Exception> LongPredicateX<Y> wrapException(@Nonnull final LongPredicateX<X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (long l) -> {
			try {
				return other.test(l);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends Exception> LongPredicateX<Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LongPredicateX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> LongPredicateX<Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LongPredicateX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
