...
/**
 * Non-throwing functional interface (lambda) LongConsumer for Java 8.
 *
 * Type: consumer
 *
 * Domain (lvl: 1): long l
...
public interface LongConsumer extends java.util.function.LongConsumer, LongConsumerX<RuntimeException>, MetaConsumer, MetaInterface.NonThrowing {
	...
	@Nonnull
	public static <X extends Exception, E extends Exception, Y extends RuntimeException> LongConsumer wrapException(@Nonnull final LongConsumer other, Class<E> exception, ExceptionHandler<E, Y> handler) {
		return (long l) -> {
			try {
				other.accept(l);
			} catch (Exception e) {
	...
	@Nonnull
	default <E extends Exception, Y extends RuntimeException> LongConsumer handle(Class<E> exception, ExceptionHandler<E, Y> handler) {
	...
	@Nonnull
	default <Y extends RuntimeException> LongConsumer handle(ExceptionHandler<Exception, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);
