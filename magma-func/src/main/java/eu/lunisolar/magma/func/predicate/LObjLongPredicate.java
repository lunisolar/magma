...
public interface LObjLongPredicate<T> extends MetaPredicate, MetaInterface.NonThrowing, Codomain<aBool>, Domain2<a<T>, aLong> { // NOSONAR
	 ...
	 */
	public static <T> M<T> memento(boolean initialValue, LObjLongPredicate<T> baseFunction, LLogicalBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LObjLongPredicate.D)
	 */
	final class M<T> implements LObjLongPredicate<T> {
		...
		private M(boolean lastValue, LObjLongPredicate<T> baseFunction, LLogicalBinaryOperator mementoFunction) {
