...
/**
 * Throwing functional interface (lambda) BiObjShortConsumerX for Java 8.
 *
 * Type: consumer
 *
...
public interface BiObjShortConsumerX<T1, T2, X extends Exception> extends MetaConsumer, MetaInterface.Throwing<X> {
	...
	@Nonnull
	default <E extends Exception, Y extends Exception> BiObjShortConsumerX<T1, T2, Y> handleX(Class<E> exception, ExceptionHandler<E, Y> handler) {
	...
	@Nonnull
	default <Y extends Exception> BiObjShortConsumerX<T1, T2, Y> handleX(ExceptionHandler<Exception, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);
