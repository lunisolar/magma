...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.to.ToLongBiFunctionX}
 */
@FunctionalInterface
...
public interface ToLongBiFunction<T1, T2> extends java.util.function.ToLongBiFunction<T1, T2>, MetaFunction, PrimitiveCodomain<ToLongBiFunction<T1, T2>> { // NOSONAR
	...
	/** Wraps JRE instance. */
	@Nonnull
	public static <T1, T2> ToLongBiFunction<T1, T2> std(final java.util.function.ToLongBiFunction<T1, T2> other) {
	...
	@Nonnull
	public static <T1, T2, X extends Exception, Y extends RuntimeException> ToLongBiFunction<T1, T2> wrapException(@Nonnull final ToLongBiFunction<T1, T2> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (T1 t1, T2 t2) -> {
			try {
				return other.applyAsLong(t1, t2);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> ToLongBiFunction<T1, T2> handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ToLongBiFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> ToLongBiFunction<T1, T2> handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ToLongBiFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
