...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.FloatToIntFunctionX}
 */
@FunctionalInterface
...
public interface FloatToIntFunction extends MetaFunction, PrimitiveCodomain<FloatToIntFunction> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> FloatToIntFunction wrapException(@Nonnull final FloatToIntFunction other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (float f) -> {
			try {
				return other.applyAsInt(f);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> FloatToIntFunction handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return FloatToIntFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> FloatToIntFunction handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return FloatToIntFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
