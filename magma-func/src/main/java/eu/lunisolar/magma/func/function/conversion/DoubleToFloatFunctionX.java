...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.DoubleToFloatFunction}
 */
@FunctionalInterface
...
public interface DoubleToFloatFunctionX<X extends Exception> extends MetaFunction, PrimitiveCodomain<DoubleToFloatFunctionX<X>>, MetaThrowingInterface<X> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends Exception> DoubleToFloatFunctionX<Y> wrapException(@Nonnull final DoubleToFloatFunctionX<X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (double d) -> {
			try {
	...
	@Nonnull
	default <Y extends Exception> DoubleToFloatFunctionX<Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return DoubleToFloatFunctionX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> DoubleToFloatFunctionX<Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return DoubleToFloatFunctionX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
