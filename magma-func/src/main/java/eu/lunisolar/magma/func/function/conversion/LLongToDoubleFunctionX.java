...
public interface LLongToDoubleFunctionX<X extends Throwable> extends LongToDoubleFunction, MetaFunction, MetaInterface.Throwing<X> { // NOSONAR

	String DESCRIPTION = "LLongToDoubleFunctionX: double doApplyAsDouble(long a1) throws X";
	...
	@Override
	@Deprecated
	default double applyAsDouble(long a1) {
		return this.nestingDoApplyAsDouble(a1);
	}

	double doApplyAsDouble(long a1) throws X;

	default double tupleApplyAsDouble(LLongSingle args) throws X {
		return doApplyAsDouble(args.first());
	}

	/** Function call that handles exceptions by always nesting checked exceptions and propagating the others as is. */
	default double nestingDoApplyAsDouble(long a1) {
		try {
	...
	default double shovingDoApplyAsDouble(long a1) {
		return ((LLongToDoubleFunctionX<RuntimeException>) this).doApplyAsDouble(a1);
	}

	/** Function call that handles exceptions according to the instructions. */
	default <Y extends Throwable> double handlingDoApplyAsDouble(long a1, HandlingInstructions<Throwable, Y> handling) throws Y {
	...
	default double nonNullDoApplyAsDouble(long a1) throws X {
	...
	default LDoubleSupplierX<X> captureLongToDoubleFunc(long a1) {
		return () -> this.doApplyAsDouble(a1);
	}
	...
	static <X extends Throwable> double call(long a1, final @Nonnull LLongToDoubleFunctionX<X> lambda) throws X {
		Null.nonNullArg(lambda, "lambda");
		return lambda.doApplyAsDouble(a1);
	}

	static <X extends Throwable> double shoving(long a1, final @Nonnull LLongToDoubleFunctionX<X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.shovingDoApplyAsDouble(a1);
	}

	static <X extends Throwable> double nesting(long a1, final @Nonnull LLongToDoubleFunctionX<X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.nestingDoApplyAsDouble(a1);
	}

	static <X extends Throwable, Y extends Throwable> double handling(long a1, final HandlingInstructions<Throwable, Y> handling, final @Nonnull LLongToDoubleFunctionX<X> lambda) throws Y {
		Null.nonNullArg(lambda, "lambda");
	...
	/** Wraps JRE instance. */
	@Nonnull
	static <X extends Throwable> LLongToDoubleFunctionX<X> wrap(final LongToDoubleFunction other) {
	...
	@Nonnull
	static <X extends Throwable> LLongToDoubleFunctionX<X> safe() {
		return Function4U::produceDouble;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <X extends Throwable, Y extends Throwable> LSupplierX<LLongToDoubleFunctionX<X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <X extends Throwable> LLongToDoubleFunctionX<X> safe(final @Nullable LLongToDoubleFunctionX<X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <X extends Throwable, Y extends Throwable> LSupplierX<LLongToDoubleFunctionX<X>, Y> safeSupplier(final @Nullable LSupplierX<LLongToDoubleFunctionX<X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	@Nonnull
	default LLongToDoubleFunctionX<X> longToDoubleFuncComposeLong(@Nonnull final LLongUnaryOperatorX<X> before1) {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1> LToDoubleFunctionX<V1, X> longToDoubleFuncCompose(@Nonnull final LToLongFunctionX<? super V1, X> before1) {
		Null.nonNullArg(before1, "before1");
		return v1 -> this.doApplyAsDouble(before1.doApplyAsLong(v1));
	}
	...
	/** Combines two functions together in a order. */
	@Nonnull
	default LLongPredicateX<X> thenToBool(@Nonnull LDoublePredicateX<X> after) {
		Null.nonNullArg(after, "after");
	...
	@Nonnull
	default LLongToDoubleFunction handleLongToDoubleFunc(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
		return a1 -> this.handlingDoApplyAsDouble(a1, handling);
	}

	/** Converts to function that handles exceptions according to the instructions. */
	@Nonnull
	default <Y extends Throwable> LLongToDoubleFunctionX<Y> handleLongToDoubleFuncX(@Nonnull HandlingInstructions<Throwable, Y> handling) {
		return a1 -> this.handlingDoApplyAsDouble(a1, handling);
	}
