...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.LongToCharFunctionX}
 */
@FunctionalInterface
...
public interface LongToCharFunction extends MetaFunction, PrimitiveCodomain<LongToCharFunction> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> LongToCharFunction wrapException(@Nonnull final LongToCharFunction other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (long l) -> {
			try {
				return other.applyAsChar(l);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> LongToCharFunction handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LongToCharFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> LongToCharFunction handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LongToCharFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
