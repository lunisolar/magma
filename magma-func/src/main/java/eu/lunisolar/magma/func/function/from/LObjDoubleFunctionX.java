...
public interface LObjDoubleFunctionX<T, R, X extends Throwable> extends MetaFunction, MetaInterface.Throwing<X> { // NOSONAR
	...
	static <T, R, X extends Throwable> R call(T a1, double a2, final @Nonnull LObjDoubleFunctionX<T, R, X> lambda) throws X {
		Null.nonNullArg(lambda, "lambda");
		return lambda.doApply(a1, a2);
	}

	static <T, R, X extends Throwable> R shoving(T a1, double a2, final @Nonnull LObjDoubleFunctionX<T, R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.shovingDoApply(a1, a2);
	}

	static <T, R, X extends Throwable> R nesting(T a1, double a2, final @Nonnull LObjDoubleFunctionX<T, R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.nestingDoApply(a1, a2);
	}

	static <T, R, X extends Throwable, Y extends Throwable> R handling(T a1, double a2, final HandlingInstructions<Throwable, Y> handling, final @Nonnull LObjDoubleFunctionX<T, R, X> lambda) throws Y {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	static <T, R, X extends Throwable> LObjDoubleFunctionX<T, R, X> safe() {
		return Function4U::produce;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <T, R, X extends Throwable, Y extends Throwable> LSupplierX<LObjDoubleFunctionX<T, R, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <T, R, X extends Throwable> LObjDoubleFunctionX<T, R, X> safe(final @Nullable LObjDoubleFunctionX<T, R, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <T, R, X extends Throwable, Y extends Throwable> LSupplierX<LObjDoubleFunctionX<T, R, X>, Y> safeSupplier(final @Nullable LSupplierX<LObjDoubleFunctionX<T, R, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	/** Permutation of LObjDoubleFunctionX for method references. */
	@FunctionalInterface
	interface V1<T, R, X extends Throwable> extends LObjDoubleFunctionX<T, R, X> {
		@Nullable
		R doApplyV1(double a2, T a1) throws X;

		@Override
		default R doApply(T a1, double a2) throws X {
