...
public interface LToDoubleFunction<T> extends LToDoubleFunctionX<T, RuntimeException>, MetaFunction, MetaInterface.NonThrowing { // NOSONAR
	...
	String DESCRIPTION = "LToDoubleFunction: double doApplyAsDouble(T a)";
	...
	default double nonNullDoApplyAsDouble(T a) {
	...
	default LDoubleSupplier captureToDoubleFunc(T a) {
		return () -> this.doApplyAsDouble(a);
	}
	...
	static <T> double call(T a, final @Nonnull LToDoubleFunction<T> lambda) {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	static <T> LToDoubleFunction<T> safe() {
		return Function4U::produceDouble;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <T> LSupplier<LToDoubleFunction<T>> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <T> LToDoubleFunction<T> safe(final @Nullable LToDoubleFunction<T> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <T> LSupplier<LToDoubleFunction<T>> safeSupplier(final @Nullable LSupplier<LToDoubleFunction<T>> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	@Nonnull
	default <V> LToDoubleFunction<V> toDoubleFuncCompose(@Nonnull final LFunction<? super V, ? extends T> before) {
