...
/**
 *
 * @see {@link eu.lunisolar.magma.func.operator.unary.BooleanUnaryOperatorX}
 */
@FunctionalInterface
@SuppressWarnings("UnusedDeclaration")
public interface BooleanUnaryOperator extends MetaLogicalOperator, PrimitiveCodomain<BooleanUnaryOperator> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> BooleanUnaryOperator wrapException(@Nonnull final BooleanUnaryOperator other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (boolean b) -> {
			try {
				return other.applyAsBoolean(b);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> BooleanUnaryOperator handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BooleanUnaryOperator.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> BooleanUnaryOperator handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BooleanUnaryOperator.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
