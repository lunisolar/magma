...
public interface LTieSrtFunction<T> extends MetaFunction, MetaInterface.NonThrowing, TieFunction<T, aShort>, Codomain<aInt>, Domain3<a<T>, aInt, aShort> { // NOSONAR
	...
	default LOiToIntFunction<T> with(short a3) {
		return (a1, a2) -> applyAsInt(a1, a2, a3);
	}

	default LSrtToIntFunction _with(T a1, int a2) {
		return a3 -> applyAsInt(a1, a2, a3);
	}

	default LToIntFunction<T> with(int a2, short a3) {
	...
	final class M<T> implements LTieSrtFunction<T> {
		...
		public int currentApplyAsInt(T a1, int a2, short a3) {
			int x1 = lastBaseValue;
		...
		public int currentBaseValue(T a1, int a2, short a3) {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1, V2, V3> LToIntTriFunction<V1, V2, V3> unboxingCompose(@Nonnull final LFunction<? super V1, ? extends T> before1, @Nonnull final LToIntFunction<? super V2> before2, @Nonnull final LToSrtFunction<? super V3> before3) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
		Null.nonNullArg(before3, "before3");
		return (v1, v2, v3) -> this.applyAsInt(before1.apply(v1), before2.applyAsInt(v2), before3.applyAsSrt(v3));
	...
	default LTieSrtFunction<T> shoving() {

		return new LTieSrtFunction<T>() {
			...
			public int applyAsIntX(T a1, int a2, short a3) throws Throwable {
				return LTieSrtFunction.this.applyAsIntX(a1, a2, a3);
			}
	...
	*/
	default <C1, C2, C3, I3> void iterate(IndexedRead<C1, a<T>> ia1, C1 source1, IndexedRead<C2, aInt> ia2, C2 source2, SequentialRead<C3, I3, aShort> sa3, C3 source3, LIntConsumer consumer) {
		...
		var nextFunc3 = SA.srtSupplier(sa3);
		int i = 0;
