...
public interface LObjBytePredicate<T> extends MetaPredicate, MetaInterface.NonThrowing, Codomain<aBool>, Domain2<a<T>, aByte> { // NOSONAR
	...
	static <T> void throwIf(T a1, byte a2, LObjBytePredicate<T> pred, ExMF<RuntimeException> factory, @Nonnull String newMessage, @Nullable Object... messageParams) {
		if (pred.test(a1, a2)) {
			throw Handling.create(factory, newMessage, messageParams);
		}
	}

	static <T> void throwIfNot(T a1, byte a2, LObjBytePredicate<T> pred, ExMF<RuntimeException> factory, @Nonnull String newMessage, @Nullable Object... messageParams) {
		if (!pred.test(a1, a2)) {
			throw Handling.create(factory, newMessage, messageParams);
		}
	}
	...
	public static <M, K, V> boolean from(M container, LBiFunction<M, K, V> extractor, K key, byte a2, LObjBytePredicate<V> function) {
	...
	/**  */
	public static <T> LObjBytePredicate<T> uncurry(LFunction<T, LBytePredicate> func) {
	...
	@Nonnull
	static <T> LObjBytePredicate<T> objBytePred(Class<T> c1, final @Nonnull LObjBytePredicate<T> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda;
	}
