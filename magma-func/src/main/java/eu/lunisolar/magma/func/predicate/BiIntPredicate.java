...
/**
 *
 * @see {@link eu.lunisolar.magma.func.predicate.BiIntPredicateX}
 */
@FunctionalInterface
...
public interface BiIntPredicate extends MetaPredicate, PrimitiveCodomain<BiIntPredicate> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> BiIntPredicate wrapException(@Nonnull final BiIntPredicate other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (int i1, int i2) -> {
			try {
				return other.test(i1, i2);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> BiIntPredicate handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BiIntPredicate.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> BiIntPredicate handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BiIntPredicate.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
