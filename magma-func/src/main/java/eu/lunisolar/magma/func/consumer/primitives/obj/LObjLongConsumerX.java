...
public interface LObjLongConsumerX<T, X extends Throwable> extends ObjLongConsumer<T>, MetaConsumer, MetaInterface.Throwing<X> {

	String DESCRIPTION = "LObjLongConsumerX: void doAccept(T a1,long a2) throws X";

	/**
	 * Default implementation for JRE method that calls exception nesting method.
	 * @deprecated Calling this method via LObjLongConsumerX interface should be discouraged.
	 */
	@Override
	@Deprecated
	...
	void doAccept(T a1, long a2) throws X;

	default LTuple.Void tupleAccept(LObjLongPair<T> args) throws X {
	...
	default void shovingDoAccept(T a1, long a2) {
		((LObjLongConsumerX<T, RuntimeException>) this).doAccept(a1, a2);
	}

	/** Function call that handles exceptions according to the instructions. */
	default <Y extends Throwable> void handlingDoAccept(T a1, long a2, HandlingInstructions<Throwable, Y> handling) throws Y {
	...
	default LActionX<X> captureObjLongCons(T a1, long a2) {
	...
	static <T, X extends Throwable> void call(T a1, long a2, final @Nonnull LObjLongConsumerX<T, X> lambda) throws X {
		Null.nonNullArg(lambda, "lambda");
		lambda.doAccept(a1, a2);
	}

	static <T, X extends Throwable> void shoving(T a1, long a2, final @Nonnull LObjLongConsumerX<T, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		lambda.shovingDoAccept(a1, a2);
	}

	static <T, X extends Throwable> void nesting(T a1, long a2, final @Nonnull LObjLongConsumerX<T, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		lambda.nestingDoAccept(a1, a2);
	}

	static <T, X extends Throwable, Y extends Throwable> void handling(T a1, long a2, final HandlingInstructions<Throwable, Y> handling, final @Nonnull LObjLongConsumerX<T, X> lambda) throws Y {
		Null.nonNullArg(lambda, "lambda");
	...
	/** Wraps JRE instance. */
	@Nonnull
	static <T, X extends Throwable> LObjLongConsumerX<T, X> wrap(final ObjLongConsumer<T> other) {
		return other::accept;
	}
	...
	/** Safe instance. */
	@Nonnull
	static <T, X extends Throwable> LObjLongConsumerX<T, X> safe() {
		return Function4U::doNothing;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <T, X extends Throwable, Y extends Throwable> LSupplierX<LObjLongConsumerX<T, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <T, X extends Throwable> LObjLongConsumerX<T, X> safe(final @Nullable LObjLongConsumerX<T, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <T, X extends Throwable, Y extends Throwable> LSupplierX<LObjLongConsumerX<T, X>, Y> safeSupplier(final @Nullable LSupplierX<LObjLongConsumerX<T, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	/** Combines two LObjLongConsumerX<T,X> together in a order. */
	@Nonnull
	...
	default LObjLongConsumer<T> handleObjLongCons(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
		return (T a1, long a2) -> this.handlingDoAccept(a1, a2, handling);
	}
	...
	/** Permutation of LObjLongConsumerX for method references. */
	@FunctionalInterface
	interface V1<T, X extends Throwable> extends LObjLongConsumerX<T, X> {

		void doAcceptV1(long a2, T a1) throws X;

		@Override
		default void doAccept(T a1, long a2) throws X {
