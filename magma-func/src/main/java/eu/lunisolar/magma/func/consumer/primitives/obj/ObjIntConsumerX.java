...
/**
 * Throwing functional interface (lambda) ObjIntConsumerX for Java 8.
 *
 * Type: consumer
 *
...
public interface ObjIntConsumerX<T, X extends Exception> extends MetaConsumer, MetaInterface.Throwing<X> {
	...
	@Nonnull
	default <E extends Exception, Y extends Exception> ObjIntConsumerX<T, Y> handleX(Class<E> exception, ExceptionHandler<E, Y> handler) {
	...
	@Nonnull
	default <Y extends Exception> ObjIntConsumerX<T, Y> handleX(ExceptionHandler<Exception, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);
