...
public interface LObjLongConsumer<T> extends ObjLongConsumer<T>, MetaConsumer, MetaInterface.NonThrowing, TeConsumer<T, aLong>, Codomain<aVoid>, Domain2<a<T>, aLong> {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1, V2> LBiConsumer<V1, V2> unboxingCompose(@Nonnull final LFunction<? super V1, ? extends T> before1, @Nonnull final LToLongFunction<? super V2> before2) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
		return (v1, v2) -> this.accept(before1.apply(v1), before2.applyAsLong(v2));
	}
	...
	*/
	public static <C1, C2, I2, T> int iterate(IndexedRead<C1, a<T>> ia1, C1 source1, SequentialRead<C2, I2, aLong> sa2, C2 source2, LObjLongConsumer<? super T> consumer) {
		...
		var nextFunc2 = SA.longSupplier(sa2);
		int i = 0;
	...
	*/
	...
	public static <T, R> R inlineAcceptR(R retval, T a1, long a2, LObjLongConsumer<T> consumer) {
		consumer.accept(a1, a2);
		return retval;
	}

	public static <T> T inlineAccept(T a1, long a2, LObjLongConsumer<T> consumer) {
		consumer.accept(a1, a2);
		return a1;
	}
