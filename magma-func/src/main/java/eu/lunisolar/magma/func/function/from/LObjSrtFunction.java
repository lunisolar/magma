...
public interface LObjSrtFunction<T, R> extends MetaFunction, MetaInterface.NonThrowing, Codomain<a<R>>, Domain2<a<T>, aShort> { // NOSONAR
	...
	public default LSrtFunction<R> lShrink(@Nonnull LSrtFunction<T> left) {
		Null.nonNullArg(left, "left");
		return a2 -> apply(left.apply(a2), a2);
	}

	public default LSrtFunction<R> lShrink_(T a1) {
	...
	public default LFunction<T, R> rShrink(@Nonnull LToSrtFunction<T> right) {
		Null.nonNullArg(right, "right");
		return a1 -> apply(a1, right.applyAsSrt(a1));
	}

	public default LFunction<T, R> rShrink_(short a2) {
	...
	public default LObjSrtFunction<T, R> beforeDo(@Nonnull LObjSrtConsumer<T> before) {
	...
	public default LObjSrtFunction<T, R> afterDo(@Nonnull LConsumer<R> after) {
	...
	final class LObjSrtFunctionSingle<T, R> implements LSingle<LObjSrtFunction<T, R>>, LObjSrtFunction<T, R> {
		...
		@Override
		public LObjSrtFunction<T, R> value() {
			return target;
		}
	}
