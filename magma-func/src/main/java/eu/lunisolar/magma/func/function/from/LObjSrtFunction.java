...
public interface LObjSrtFunction<T, R> extends MetaFunction, MetaInterface.NonThrowing, Codomain<a<R>>, Domain2<a<T>, aShort> { // NOSONAR
	...
	public static <M, K, V, R> R from(@Nonnull M container, LBiFunction<M, K, V> extractor, K key, short a2, @Nonnull LObjSrtFunction<V, R> function) {
	...
	final class M<T, R> implements LObjSrtFunction<T, R> {
		...
		public R currentApply(T a1, short a2) {
			R x1 = lastBaseValue;
		...
		public R currentBaseValue(T a1, short a2) {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1, V2> LBiFunction<V1, V2, R> unboxingCompose(@Nonnull final LFunction<? super V1, ? extends T> before1, @Nonnull final LToSrtFunction<? super V2> before2) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
		return (v1, v2) -> this.apply(before1.apply(v1), before2.applyAsSrt(v2));
	}
	...
	default LObjSrtFunction<T, R> shoving() {

		return new LObjSrtFunction<T, R>() {
			...
			public R applyX(T a1, short a2) throws Throwable {
				return LObjSrtFunction.this.applyX(a1, a2);
			}
