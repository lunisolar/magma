...
public interface LObjSrtFunction<T, R> extends MetaFunction, MetaInterface.NonThrowing, Codomain<a<R>>, Domain2<a<T>, aShort> { // NOSONAR
	...
	public static <R, M, K, V> R from(M container, LBiFunction<M, K, V> extractor, K key, short a2, LObjSrtFunction<V, R> function) {
	...
	public static <R, T> LSrtFunction<R> lShrinkedc(T a1, LObjSrtFunction<T, R> func) {
	...
	public static <T, R> LFunction<T, R> rShrinkedc(short a2, LObjSrtFunction<T, R> func) {
	...
	@Nonnull
	static <T, R> LObjSrtFunction<T, R> objSrtFunc(Class<T> c1, Class<R> c2, final @Nonnull LObjSrtFunction<T, R> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda;
	}
