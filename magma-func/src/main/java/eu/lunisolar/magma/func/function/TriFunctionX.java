...
/**
 * Throwing functional interface (lambda) TriFunctionX for Java 8.
 *
 * Type: function
 *
...
public interface TriFunctionX<T1, T2, T3, R, X extends Exception> extends MetaFunction, MetaInterface.Throwing<X> { // NOSONAR
	...
	default SupplierX<R, X> capture(T1 t1, T2 t2, T3 t3) {
	...
	default TriFunction<T1, T2, T3, R> shove() {
		TriFunctionX<T1, T2, T3, R, RuntimeException> exceptionCast = (TriFunctionX<T1, T2, T3, R, RuntimeException>) this;
	...
	@Nonnull
	public static <T1, T2, T3, R, X extends Exception, E extends Exception, Y extends Exception> TriFunctionX<T1, T2, T3, R, Y> wrapException(@Nonnull final TriFunctionX<T1, T2, T3, R, X> other, Class<E> exception, SupplierX<R, X> supplier,
	...
	@Nonnull
	default <E extends Exception, Y extends Exception> TriFunctionX<T1, T2, T3, R, Y> handleX(Class<E> exception, ExceptionHandler<E, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return TriFunctionX.wrapException(this, exception, null, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for any exception (including unchecked exception that might be different from X) will call handler function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> TriFunctionX<T1, T2, T3, R, Y> handleX(ExceptionHandler<Exception, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return TriFunctionX.wrapException(this, Exception.class, null, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call supplier and return default value instead for propagating exception.  */
	@Nonnull
	default <E extends Exception, Y extends Exception> TriFunctionX<T1, T2, T3, R, Y> handleX(Class<E> exception, SupplierX<R, X> supplier) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(supplier, Function4U.VALIDATION_MESSAGE_HANDLER);

		return TriFunctionX.wrapException(this, exception, supplier, null);
	}

	/** Wraps with exception handling that for any exception will call supplier and return default value instead for propagating exception.  */
	@Nonnull
	default <Y extends Exception> TriFunctionX<T1, T2, T3, R, Y> handleX(SupplierX<R, X> supplier) {
		Objects.requireNonNull(supplier, Function4U.VALIDATION_MESSAGE_HANDLER);

		return TriFunctionX.wrapException(this, Exception.class, supplier, null);
	}
