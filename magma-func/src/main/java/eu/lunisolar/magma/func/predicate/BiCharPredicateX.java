...
/**
 *
 * @see {@link eu.lunisolar.magma.func.predicate.BiCharPredicate}
 */
@FunctionalInterface
...
public interface BiCharPredicateX<X extends Exception> extends MetaPredicate, PrimitiveCodomain<BiCharPredicateX<X>>, MetaThrowingInterface<X> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends Exception> BiCharPredicateX<Y> wrapException(@Nonnull final BiCharPredicateX<X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
	...
	@Nonnull
	default <Y extends Exception> BiCharPredicateX<Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BiCharPredicateX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> BiCharPredicateX<Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BiCharPredicateX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
