...
public interface LTriIntFunction<R> extends MetaFunction, MetaInterface.NonThrowing, Codomain<a<R>>, Domain3<aInt, aInt, aInt> { // NOSONAR
	...
	final class M<R> implements LTriIntFunction<R> {
		...
		public R currentApply(int a1, int a2, int a3) {
			R x1 = lastBaseValue;
		...
		public R currentBaseValue(int a1, int a2, int a3) {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1, V2, V3> LTriFunction<V1, V2, V3, R> unboxingCompose(@Nonnull final LToIntFunction<? super V1> before1, @Nonnull final LToIntFunction<? super V2> before2, @Nonnull final LToIntFunction<? super V3> before3) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
		Null.nonNullArg(before3, "before3");
		return (v1, v2, v3) -> this.apply(before1.applyAsInt(v1), before2.applyAsInt(v2), before3.applyAsInt(v3));
