...
public interface LFltToByteFunction extends MetaFunction, MetaInterface.NonThrowing, Codomain<aByte>, Domain1<aFloat> { // NOSONAR
	 ...
	 */
	public static M memento(byte initialValue, LFltToByteFunction baseFunction, LByteBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LFltToByteFunction.D)
	 */
	final class M implements LFltToByteFunction {
		...
		private M(byte lastValue, LFltToByteFunction baseFunction, LByteBinaryOperator mementoFunction) {
	...
	static LFltToByteFunction safe() {
		return LFltToByteFunction::doNothing;
	}
