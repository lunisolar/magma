...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.LongToShortFunctionX}
 */
@FunctionalInterface
...
public interface LongToShortFunction extends MetaFunction, PrimitiveCodomain<LongToShortFunction> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> LongToShortFunction wrapException(@Nonnull final LongToShortFunction other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (long l) -> {
			try {
				return other.applyAsShort(l);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> LongToShortFunction handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LongToShortFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> LongToShortFunction handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LongToShortFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
