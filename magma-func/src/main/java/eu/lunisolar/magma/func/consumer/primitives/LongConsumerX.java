...
/**
 * Throwing functional interface (lambda) LongConsumerX for Java 8.
 *
 * Type: consumer
 *
 * Domain (lvl: 1): long l
...
public interface LongConsumerX<X extends Exception> extends MetaConsumer, MetaInterface.Throwing<X> {
	...
	default ActionX<X> capture(long l) {
	...
	/** Wraps JRE instance. */
	@Nonnull
	public static <X extends Exception> LongConsumerX<X> wrapStd(final java.util.function.LongConsumer other) {
		return other::accept;
	}
	...
	default LongConsumerX<RuntimeException> uncheck() {
		return (LongConsumerX) this;
	}

	/** Dirty way, checked exception will propagate as it would be unchecked - there is no exception wrapping involved (at least not here). */
	default LongConsumer shove() {
		LongConsumerX<RuntimeException> exceptionCast = (LongConsumerX<RuntimeException>) this;
	...
	@Nonnull
	public static <X extends Exception, E extends Exception, Y extends Exception> LongConsumerX<Y> wrapException(@Nonnull final LongConsumerX<X> other, Class<E> exception, ExceptionHandler<E, Y> handler) {
		return (long l) -> {
			try {
				other.accept(l);
			} catch (Exception e) {
	...
	@Nonnull
	default <E extends Exception, Y extends Exception> LongConsumerX<Y> handleX(Class<E> exception, ExceptionHandler<E, Y> handler) {
	...
	@Nonnull
	default <Y extends Exception> LongConsumerX<Y> handleX(ExceptionHandler<Exception, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);
