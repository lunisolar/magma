...
public interface LBooleanToByteFunctionX<X extends Throwable> extends MetaFunction, PrimitiveCodomain<Object>, MetaInterface.Throwing<X> { // NOSONAR
	...
	@Nonnull
	default LBooleanToByteFunctionX<X> boolToByteFuncComposeBoolean(@Nonnull final LLogicalOperatorX<X> before1) {
	...
	@Nonnull
	default LBooleanToByteFunction nestingBoolToByteFunc() {
		return this::nestingDoApplyAsByte;
	}

	/** Converts to throwing variant (RuntimeException). */
	@Nonnull
	default LBooleanToByteFunctionX<RuntimeException> nestingBoolToByteFuncX() {
		return this::nestingDoApplyAsByte;
	}

	/** Converts to non-throwing variant that will propagate checked exception as it would be unchecked - there is no exception wrapping involved (at least not here). */
	default LBooleanToByteFunction shovingBoolToByteFunc() {
		return this::shovingDoApplyAsByte;
	}

	/** Converts to throwing variant (RuntimeException) that will propagate checked exception as it would be unchecked - there is no exception wrapping involved (at least not here). */
	default LBooleanToByteFunctionX<RuntimeException> shovingBoolToByteFuncX() {
		return this::shovingDoApplyAsByte;
	}
	...
	@Nonnull
	default LBooleanToByteFunction handleBoolToByteFunc(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
		return b -> this.handlingDoApplyAsByte(b, handling);
	}

	/** Converts to function that handles exceptions according to the instructions. */
	@Nonnull
	default <Y extends Throwable> LBooleanToByteFunctionX<Y> handleBoolToByteFuncX(@Nonnull HandlingInstructions<Throwable, Y> handling) {
		return b -> this.handlingDoApplyAsByte(b, handling);
	}
