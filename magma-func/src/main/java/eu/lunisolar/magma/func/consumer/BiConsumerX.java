...
/**
 * A consumer.
 * @see {@link eu.lunisolar.magma.func.consumer.BiConsumer}
 */
@FunctionalInterface
...
public interface BiConsumerX<T1, T2, X extends Exception> extends MetaConsumer, MetaThrowingInterface<X> {
	...
	@Nonnull
	public static <T1, T2, X extends Exception, Y extends Exception> BiConsumerX<T1, T2, Y> wrapException(@Nonnull final BiConsumerX<T1, T2, X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (T1 t1, T2 t2) -> {
			try {
				other.accept(t1, t2);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends Exception> BiConsumerX<T1, T2, Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
	...
	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> BiConsumerX<T1, T2, Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);
