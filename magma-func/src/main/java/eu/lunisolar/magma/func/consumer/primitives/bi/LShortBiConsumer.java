...
public interface LShortBiConsumer extends LShortBiConsumerX<RuntimeException>, MetaConsumer, MetaInterface.NonThrowing {
	...
	public static final String DESCRIPTION = "LShortBiConsumer: void accept(short s1,short s2)";
	...
	@Nonnull
	public static <X extends Exception, E extends Exception, Y extends RuntimeException> LShortBiConsumer wrapException(@Nonnull final LShortBiConsumer other, Class<E> exception, ExceptionHandler<E, Y> handler) {
		return (short s1, short s2) -> {
			try {
				other.accept(s1, s2);
			} catch (Exception e) {
	...
	@Nonnull
	default <E extends Exception, Y extends RuntimeException> LShortBiConsumer handle(Class<E> exception, ExceptionHandler<E, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LShortBiConsumer.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for any exception (including unchecked exception that might be different from X) will call handler function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> LShortBiConsumer handle(ExceptionHandler<Exception, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LShortBiConsumer.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
