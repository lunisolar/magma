...
/**
 *
 * @see {@link eu.lunisolar.magma.func.predicate.ObjIntPredicate}
 */
@FunctionalInterface
...
public interface ObjIntPredicateX<T, X extends Exception> extends MetaPredicate, PrimitiveCodomain<ObjIntPredicateX<T, X>>, MetaThrowingInterface<X> { // NOSONAR
	...
	@Nonnull
	public static <T, X extends Exception, Y extends Exception> ObjIntPredicateX<T, Y> wrapException(@Nonnull final ObjIntPredicateX<T, X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (T t, int i) -> {
			try {
	...
	@Nonnull
	default <Y extends Exception> ObjIntPredicateX<T, Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ObjIntPredicateX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> ObjIntPredicateX<T, Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ObjIntPredicateX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
