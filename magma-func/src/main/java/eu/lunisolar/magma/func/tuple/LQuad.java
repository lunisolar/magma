    ...
    public static  boolean tupleEquals(LQuad the, Object that) {
        return Null.equals(the, that, (one, two) -> {
            ...
            return  one.tupleSize() == other.tupleSize() &&
                    argEquals(one.first(), one.second(), one.third(), one.fourth(), other.first(), other.second(), other.third(), other.fourth());
        });
    }
...
@Immutable
    ...
    public static <T,C,R> Iterator<R> iterator(SequentialRead<C, ?, a<T>> sa, C source, LQuadFunction<T,T,T,T,R> factory) {
