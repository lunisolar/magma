...
public interface LFltTriple extends LTuple<Float>, Comparable<LFltTriple> {
	...
	public static LFltTriple immutableOf(float a1, float a2, float a3) {
		return new ImmFltTriple(a1, a2, a3);
	}

	public static LFltTriple immutableCopyOf(LFltTriple tuple) {
	...
	public static Iterator<LFltTriple.MutFltTriple> mutIterator(PrimitiveIterator.OfDouble items) {
		return iterator(items, LFltTriple::of);
	}

	public static Iterator<LFltTriple> immIterator(PrimitiveIterator.OfDouble items) {
		return iterator(items, LFltTriple::immutableOf);
	}
	...
	public static Stream<LFltTriple.MutFltTriple> mutStream(DoubleStream items) {
		return stream(items, LFltTriple::of);
	}

	public static Stream<LFltTriple> immStream(DoubleStream items) {
		return stream(items, LFltTriple::immutableOf);
	}
	...
	public static <C, R> Iterator<R> iterator(SequentialRead<C, ?, aFloat> sa_, C source, LTriFltFunction<R> factory) {

		var sa = (SequentialRead<C, Object, aFloat>) sa_;
		var iterator = SA.adapter(sa).apply(source);
		var testFunc = SA.tester(sa);
		var nextFunc = SA.fltSupplier(sa);
