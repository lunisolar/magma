...
public interface LBiFloatFunction<R> extends LBiFloatFunctionX<R, RuntimeException>, MetaFunction, MetaInterface.NonThrowing { // NOSONAR
	...
	@Nonnull
	static <R> LBiFloatFunction<R> safe() {
		return Function4U::produce;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <R> LSupplier<LBiFloatFunction<R>> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <R> LBiFloatFunction<R> safe(final @Nullable LBiFloatFunction<R> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <R> LSupplier<LBiFloatFunction<R>> safeSupplier(final @Nullable LSupplier<LBiFloatFunction<R>> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	/** Permutation of LBiFloatFunction for method references. */
	@FunctionalInterface
	interface V1<R> extends LBiFloatFunction<R> {
		@Nullable
		R doApplyV1(float a2, float a1);
