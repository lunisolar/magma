...
public interface LObjDoublePredicateX<T, X extends Throwable> extends MetaPredicate, MetaInterface.Throwing<X> { // NOSONAR
	...
	@Nonnull
	static <T, X extends Throwable> LObjDoublePredicateX<T, X> safe() {
		return Function4U::alwaysFalse;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <T, X extends Throwable, Y extends Throwable> LSupplierX<LObjDoublePredicateX<T, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <T, X extends Throwable> LObjDoublePredicateX<T, X> safe(final @Nullable LObjDoublePredicateX<T, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <T, X extends Throwable, Y extends Throwable> LSupplierX<LObjDoublePredicateX<T, X>, Y> safeSupplier(final @Nullable LSupplierX<LObjDoublePredicateX<T, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	default <V1> LObjDoublePredicateX<V1, X> objDoublePredComposeDouble(@Nonnull final LFunctionX<? super V1, ? extends T, X> before1, @Nonnull final LDoubleUnaryOperatorX<X> before2) {
		...
		return (V1 v1, double v2) -> this.doTest(before1.doApply(v1), before2.doApplyAsDouble(v2));
	}
	...
	/** Permutation of LObjDoublePredicateX for method references. */
	@FunctionalInterface
	interface V1<T, X extends Throwable> extends LObjDoublePredicateX<T, X> {

		boolean doTestV1(double a2, T a1) throws X;

		@Override
		default boolean doTest(T a1, double a2) throws X {
