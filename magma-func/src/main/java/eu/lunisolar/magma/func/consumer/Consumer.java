...
/**
 * A consumer.
 * @see {@link eu.lunisolar.magma.func.consumer.ConsumerX}
 */
@FunctionalInterface
...
public interface Consumer<T> extends java.util.function.Consumer<T>, MetaConsumer {
	...
	@Nonnull
	public static <T, X extends Exception, Y extends RuntimeException> Consumer<T> wrapException(@Nonnull final Consumer<T> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (T t) -> {
			try {
				other.accept(t);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> Consumer<T> handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
	...
	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> Consumer<T> handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);
