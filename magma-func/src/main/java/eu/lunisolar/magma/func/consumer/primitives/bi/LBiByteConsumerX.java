...
public interface LBiByteConsumerX<X extends Throwable> extends MetaConsumer, MetaInterface.Throwing<X> {
	...
	String DESCRIPTION = "LBiByteConsumerX: void doAccept(byte a1,byte a2) throws X";

	void doAccept(byte a1, byte a2) throws X;

	default LTuple.Void tupleAccept(LBytePair args) throws X {
	...
	default void shovingDoAccept(byte a1, byte a2) {
		((LBiByteConsumerX<RuntimeException>) this).doAccept(a1, a2);
	}

	/** Function call that handles exceptions according to the instructions. */
	default <Y extends Throwable> void handlingDoAccept(byte a1, byte a2, HandlingInstructions<Throwable, Y> handling) throws Y {
	...
	default LActionX<X> captureBiByteCons(byte a1, byte a2) {
	...
	static <X extends Throwable> void call(byte a1, byte a2, final @Nonnull LBiByteConsumerX<X> lambda) throws X {
		Null.nonNullArg(lambda, "lambda");
		lambda.doAccept(a1, a2);
	}

	static <X extends Throwable> void shoving(byte a1, byte a2, final @Nonnull LBiByteConsumerX<X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		lambda.shovingDoAccept(a1, a2);
	}

	static <X extends Throwable> void nesting(byte a1, byte a2, final @Nonnull LBiByteConsumerX<X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		lambda.nestingDoAccept(a1, a2);
	}

	static <X extends Throwable, Y extends Throwable> void handling(byte a1, byte a2, final HandlingInstructions<Throwable, Y> handling, final @Nonnull LBiByteConsumerX<X> lambda) throws Y {
		Null.nonNullArg(lambda, "lambda");
	...
	/** Safe instance. */
	@Nonnull
	static <X extends Throwable> LBiByteConsumerX<X> safe() {
		return Function4U::doNothing;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <X extends Throwable, Y extends Throwable> LSupplierX<LBiByteConsumerX<X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <X extends Throwable> LBiByteConsumerX<X> safe(final @Nullable LBiByteConsumerX<X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <X extends Throwable, Y extends Throwable> LSupplierX<LBiByteConsumerX<X>, Y> safeSupplier(final @Nullable LSupplierX<LBiByteConsumerX<X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	/** Combines two LBiByteConsumerX<X> together in a order. */
	@Nonnull
	...
	default LBiByteConsumer handleBiByteCons(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
		return (byte a1, byte a2) -> this.handlingDoAccept(a1, a2, handling);
	}
	...
	/** Permutation of LBiByteConsumerX for method references. */
	@FunctionalInterface
	interface V1<X extends Throwable> extends LBiByteConsumerX<X> {

		void doAcceptV1(byte a2, byte a1) throws X;

		@Override
		default void doAccept(byte a1, byte a2) throws X {
