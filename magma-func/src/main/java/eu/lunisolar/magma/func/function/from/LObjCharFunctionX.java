...
public interface LObjCharFunctionX<T, R, X extends Throwable> extends MetaFunction, MetaInterface.Throwing<X> { // NOSONAR
	...
	String DESCRIPTION = "LObjCharFunctionX: R doApply(T a1,char a2) throws X";

	@Nullable
	R doApply(T a1, char a2) throws X;

	default R tupleApply(LObjCharPair<T> args) throws X {
	...
	default R shovingDoApply(T a1, char a2) {
		return ((LObjCharFunctionX<T, R, RuntimeException>) this).doApply(a1, a2);
	}

	/** Function call that handles exceptions according to the instructions. */
	default <Y extends Throwable> R handlingDoApply(T a1, char a2, HandlingInstructions<Throwable, Y> handling) throws Y {
	...
	@Nonnull
	default R nonNullDoApply(T a1, char a2) throws X {
	...
	default LSupplierX<R, X> captureObjCharFunc(T a1, char a2) {
		return () -> this.doApply(a1, a2);
	}
	...
	static <T, R, X extends Throwable> R call(T a1, char a2, final @Nonnull LObjCharFunctionX<T, R, X> lambda) throws X {
		Null.nonNullArg(lambda, "lambda");
		return lambda.doApply(a1, a2);
	}

	static <T, R, X extends Throwable> R shoving(T a1, char a2, final @Nonnull LObjCharFunctionX<T, R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.shovingDoApply(a1, a2);
	}

	static <T, R, X extends Throwable> R nesting(T a1, char a2, final @Nonnull LObjCharFunctionX<T, R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.nestingDoApply(a1, a2);
	}

	static <T, R, X extends Throwable, Y extends Throwable> R handling(T a1, char a2, final HandlingInstructions<Throwable, Y> handling, final @Nonnull LObjCharFunctionX<T, R, X> lambda) throws Y {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	static <T, R, X extends Throwable> LObjCharFunctionX<T, R, X> safe() {
		return Function4U::produce;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <T, R, X extends Throwable, Y extends Throwable> LSupplierX<LObjCharFunctionX<T, R, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <T, R, X extends Throwable> LObjCharFunctionX<T, R, X> safe(final @Nullable LObjCharFunctionX<T, R, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <T, R, X extends Throwable, Y extends Throwable> LSupplierX<LObjCharFunctionX<T, R, X>, Y> safeSupplier(final @Nullable LSupplierX<LObjCharFunctionX<T, R, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	default LObjCharFunction<T, R> handleObjCharFunc(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
		return (T a1, char a2) -> this.handlingDoApply(a1, a2, handling);
	}
	...
	/** Permutation of LObjCharFunctionX for method references. */
	@FunctionalInterface
	interface V1<T, R, X extends Throwable> extends LObjCharFunctionX<T, R, X> {
		@Nullable
		R doApplyV1(char a2, T a1) throws X;

		@Override
		default R doApply(T a1, char a2) throws X {
