...
public interface LFunctionX<T, R, X extends Throwable> extends Function<T, R>, MetaFunction, MetaInterface.Throwing<X> { // NOSONAR
	...
	static <T, R, X extends Throwable> R call(T a1, final @Nonnull LFunctionX<T, R, X> lambda) throws X {
		Null.nonNullArg(lambda, "lambda");
		return lambda.doApply(a1);
	}

	static <T, R, X extends Throwable> R shoving(T a1, final @Nonnull LFunctionX<T, R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.shovingDoApply(a1);
	}

	static <T, R, X extends Throwable> R nesting(T a1, final @Nonnull LFunctionX<T, R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.nestingDoApply(a1);
	}

	static <T, R, X extends Throwable, Y extends Throwable> R handling(T a1, final HandlingInstructions<Throwable, Y> handling, final @Nonnull LFunctionX<T, R, X> lambda) throws Y {
		Null.nonNullArg(lambda, "lambda");
