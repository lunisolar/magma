...
public interface LObjDoubleFunction<T, R> extends LObjDoubleFunctionX<T, R, RuntimeException>, MetaFunction, MetaInterface.NonThrowing { // NOSONAR
	...
	String DESCRIPTION = "LObjDoubleFunction: R doApply(T a1,double a2)";

	@Nullable
	R doApply(T a1, double a2);

	default R tupleApply(LObjDoublePair<T> args) {
	...
	default LSupplier<R> captureObjDoubleFunc(T a1, double a2) {
		return () -> this.doApply(a1, a2);
	}
	...
	@Nonnull
	static <T, R> LObjDoubleFunction<T, R> apply1st(@Nonnull LFunction<T, R> func) {
		return (a1, a2) -> func.doApply(a1);
	}

	/** Captures single parameter function into this interface where only 2nd parameter will be used. */
	@Nonnull
	static <T, R> LObjDoubleFunction<T, R> apply2nd(@Nonnull LDoubleFunction<R> func) {
	...
	static <T, R> R call(T a1, double a2, final @Nonnull LObjDoubleFunction<T, R> lambda) {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	static <T, R> LObjDoubleFunction<T, R> safe() {
		return Function4U::produce;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <T, R> LSupplier<LObjDoubleFunction<T, R>> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <T, R> LObjDoubleFunction<T, R> safe(final @Nullable LObjDoubleFunction<T, R> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <T, R> LSupplier<LObjDoubleFunction<T, R>> safeSupplier(final @Nullable LSupplier<LObjDoubleFunction<T, R>> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	@Nonnull
	default <V1> LObjDoubleFunction<V1, R> objDoubleFuncComposeDouble(@Nonnull final LFunction<? super V1, ? extends T> before1, @Nonnull final LDoubleUnaryOperator before2) {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1, V2> LBiFunction<V1, V2, R> objDoubleFuncCompose(@Nonnull final LFunction<? super V1, ? extends T> before1, @Nonnull final LToDoubleFunction<? super V2> before2) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
	...
	@Nonnull
	default LObjDoubleFunction<T, R> nonNullObjDoubleFunc() {
		return this::nonNullDoApply;
	}

	// <editor-fold desc="interface variants">

	/** Permutation of LObjDoubleFunction for method references. */
	@FunctionalInterface
	interface V1<T, R> extends LObjDoubleFunction<T, R> {
		@Nullable
		R doApplyV1(double a2, T a1);
