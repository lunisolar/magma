...
public interface LObjIntLongFunction<T, R> extends MetaFunction, MetaInterface.NonThrowing, Codomain<a<R>>, Domain3<a<T>, aInt, aLong> { // NOSONAR
	...
	public static <M, K, V, R> R from(@Nonnull M container, LBiFunction<M, K, V> extractor, K key, int a2, long a3, @Nonnull LObjIntLongFunction<V, R> function) {
	...
	default LOiFunction<T, R> with(long a3) {
		return (a1, a2) -> apply(a1, a2, a3);
	}

	default LLongFunction<R> _with(T a1, int a2) {
		return a3 -> apply(a1, a2, a3);
	}

	default LFunction<T, R> with(int a2, long a3) {
	...
	final class M<T, R> implements LObjIntLongFunction<T, R> {
		...
		public R currentApply(T a1, int a2, long a3) {
			R x1 = lastBaseValue;
		...
		public R currentBaseValue(T a1, int a2, long a3) {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1, V2, V3> LTriFunction<V1, V2, V3, R> unboxingCompose(@Nonnull final LFunction<? super V1, ? extends T> before1, @Nonnull final LToIntFunction<? super V2> before2, @Nonnull final LToLongFunction<? super V3> before3) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
		Null.nonNullArg(before3, "before3");
		return (v1, v2, v3) -> this.apply(before1.apply(v1), before2.applyAsInt(v2), before3.applyAsLong(v3));
	...
	default LObjIntLongFunction<T, R> shoving() {

		return new LObjIntLongFunction<T, R>() {
			...
			public R applyX(T a1, int a2, long a3) throws Throwable {
				return LObjIntLongFunction.this.applyX(a1, a2, a3);
			}
