...
/**
 *
 * @see {@link eu.lunisolar.magma.func.operator.unary.LongUnaryOperatorX}
 */
@FunctionalInterface
@SuppressWarnings("UnusedDeclaration")
public interface LongUnaryOperator extends java.util.function.LongUnaryOperator, MetaOperator, PrimitiveCodomain<LongUnaryOperator> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> LongUnaryOperator wrapException(@Nonnull final LongUnaryOperator other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (long l) -> {
			try {
				return other.applyAsLong(l);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> LongUnaryOperator handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LongUnaryOperator.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> LongUnaryOperator handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LongUnaryOperator.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
