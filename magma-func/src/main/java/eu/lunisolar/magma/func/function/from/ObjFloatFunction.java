...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.from.ObjFloatFunctionX}
 */
@FunctionalInterface
@SuppressWarnings("UnusedDeclaration")
public interface ObjFloatFunction<T, R> extends MetaFunction { // NOSONAR

	public static final String DESCRIPTION = "ObjFloatFunction: R apply(T t, float f)";

	@Nullable
	public R apply(T t, float f);
	...
	@Nonnull
	public static <T, R, X extends Exception, Y extends RuntimeException> ObjFloatFunction<T, R> wrapException(@Nonnull final ObjFloatFunction<T, R> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (T t, float f) -> {
			try {
				return other.apply(t, f);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> ObjFloatFunction<T, R> handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ObjFloatFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> ObjFloatFunction<T, R> handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ObjFloatFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
