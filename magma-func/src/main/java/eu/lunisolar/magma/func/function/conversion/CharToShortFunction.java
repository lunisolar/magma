...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.CharToShortFunctionX}
 */
@FunctionalInterface
...
public interface CharToShortFunction extends MetaFunction, PrimitiveCodomain<CharToShortFunction> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> CharToShortFunction wrapException(@Nonnull final CharToShortFunction other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (char c) -> {
			try {
				return other.applyAsShort(c);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> CharToShortFunction handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return CharToShortFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> CharToShortFunction handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return CharToShortFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
