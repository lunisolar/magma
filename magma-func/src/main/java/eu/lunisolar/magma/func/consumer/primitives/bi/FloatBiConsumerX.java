...
/**
 * Throwing functional interface (lambda) FloatBiConsumerX for Java 8.
 *
 * Type: consumer
 *
...
public interface FloatBiConsumerX<X extends Exception> extends MetaConsumer, MetaInterface.Throwing<X> {
	...
	default ActionX<X> capture(float f1, float f2) {
	...
	default FloatBiConsumerX<RuntimeException> uncheck() {
		return (FloatBiConsumerX) this;
	}

	/** Dirty way, checked exception will propagate as it would be unchecked - there is no exception wrapping involved (at least not here). */
	default FloatBiConsumer shove() {
		FloatBiConsumerX<RuntimeException> exceptionCast = (FloatBiConsumerX<RuntimeException>) this;
	...
	@Nonnull
	public static <X extends Exception, E extends Exception, Y extends Exception> FloatBiConsumerX<Y> wrapException(@Nonnull final FloatBiConsumerX<X> other, Class<E> exception, ExceptionHandler<E, Y> handler) {
		return (float f1, float f2) -> {
			try {
				other.accept(f1, f2);
			} catch (Exception e) {
	...
	@Nonnull
	default <E extends Exception, Y extends Exception> FloatBiConsumerX<Y> handleX(Class<E> exception, ExceptionHandler<E, Y> handler) {
	...
	@Nonnull
	default <Y extends Exception> FloatBiConsumerX<Y> handleX(ExceptionHandler<Exception, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);
