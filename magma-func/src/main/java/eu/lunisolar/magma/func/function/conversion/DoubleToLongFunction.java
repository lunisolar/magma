...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.DoubleToLongFunctionX}
 */
@FunctionalInterface
...
public interface DoubleToLongFunction extends java.util.function.DoubleToLongFunction, MetaFunction, PrimitiveCodomain<DoubleToLongFunction> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> DoubleToLongFunction wrapException(@Nonnull final DoubleToLongFunction other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (double d) -> {
			try {
				return other.applyAsLong(d);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> DoubleToLongFunction handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return DoubleToLongFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> DoubleToLongFunction handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return DoubleToLongFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
