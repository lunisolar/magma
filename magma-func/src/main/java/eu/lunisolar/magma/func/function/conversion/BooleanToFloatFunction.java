...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.BooleanToFloatFunctionX}
 */
@FunctionalInterface
@SuppressWarnings("UnusedDeclaration")
public interface BooleanToFloatFunction extends MetaFunction, PrimitiveCodomain<BooleanToFloatFunction> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> BooleanToFloatFunction wrapException(@Nonnull final BooleanToFloatFunction other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (boolean b) -> {
			try {
				return other.applyAsFloat(b);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> BooleanToFloatFunction handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BooleanToFloatFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> BooleanToFloatFunction handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BooleanToFloatFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
