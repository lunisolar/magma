...
public interface LBiObjFloatFunction<T1, T2, R> extends LBiObjFloatFunctionX<T1, T2, R, RuntimeException>, MetaFunction, MetaInterface.NonThrowing { // NOSONAR
	...
	String DESCRIPTION = "LBiObjFloatFunction: R doApply(T1 a1,T2 a2,float a3)";

	@Nullable
	R doApply(T1 a1, T2 a2, float a3);

	default R tupleApply(LBiObjFloatTriple<T1, T2> args) {
	...
	default LSupplier<R> captureBiObjFloatFunc(T1 a1, T2 a2, float a3) {
		return () -> this.doApply(a1, a2, a3);
	}
	...
	@Nonnull
	static <T1, T2, R> LBiObjFloatFunction<T1, T2, R> apply1st(@Nonnull LFunction<T1, R> func) {
		return (a1, a2, a3) -> func.doApply(a1);
	}

	/** Captures single parameter function into this interface where only 2nd parameter will be used. */
	@Nonnull
	static <T1, T2, R> LBiObjFloatFunction<T1, T2, R> apply2nd(@Nonnull LFunction<T2, R> func) {
		return (a1, a2, a3) -> func.doApply(a2);
	}

	/** Captures single parameter function into this interface where only 3rd parameter will be used. */
	@Nonnull
	static <T1, T2, R> LBiObjFloatFunction<T1, T2, R> apply3rd(@Nonnull LFloatFunction<R> func) {
	...
	static <T1, T2, R> R call(T1 a1, T2 a2, float a3, final @Nonnull LBiObjFloatFunction<T1, T2, R> lambda) {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	static <T1, T2, R> LSupplier<LBiObjFloatFunction<T1, T2, R>> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <T1, T2, R> LBiObjFloatFunction<T1, T2, R> safe(final @Nullable LBiObjFloatFunction<T1, T2, R> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <T1, T2, R> LSupplier<LBiObjFloatFunction<T1, T2, R>> safeSupplier(final @Nullable LSupplier<LBiObjFloatFunction<T1, T2, R>> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	@Nonnull
	default <V1, V2> LBiObjFloatFunction<V1, V2, R> biObjFloatFuncComposeFloat(@Nonnull final LFunction<? super V1, ? extends T1> before1, @Nonnull final LFunction<? super V2, ? extends T2> before2, @Nonnull final LFloatUnaryOperator before3) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
		Null.nonNullArg(before3, "before3");
		return (v1, v2, v3) -> this.doApply(before1.doApply(v1), before2.doApply(v2), before3.doApplyAsFloat(v3));
	}

	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1, V2, V3> LTriFunction<V1, V2, V3, R> biObjFloatFuncCompose(@Nonnull final LFunction<? super V1, ? extends T1> before1, @Nonnull final LFunction<? super V2, ? extends T2> before2, @Nonnull final LToFloatFunction<? super V3> before3) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
		Null.nonNullArg(before3, "before3");
		return (v1, v2, v3) -> this.doApply(before1.doApply(v1), before2.doApply(v2), before3.doApplyAsFloat(v3));
	...
	@Nonnull
	default LBiObjFloatFunction<T1, T2, R> nonNullBiObjFloatFunc() {
