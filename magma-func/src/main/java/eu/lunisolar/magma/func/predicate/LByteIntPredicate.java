...
public interface LByteIntPredicate extends MetaPredicate, MetaInterface.NonThrowing, Codomain<aBool>, Domain2<aByte, aInt> { // NOSONAR
	 ...
	 */
	public static M memento(boolean initialValue, LByteIntPredicate baseFunction, LLogicalBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LByteIntPredicate.D)
	 */
	final class M implements LByteIntPredicate {
		...
		private M(boolean lastValue, LByteIntPredicate baseFunction, LLogicalBinaryOperator mementoFunction) {
