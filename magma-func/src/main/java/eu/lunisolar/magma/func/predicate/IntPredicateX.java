...
/**
 *
 * @see {@link eu.lunisolar.magma.func.predicate.IntPredicate}
 */
@FunctionalInterface
...
public interface IntPredicateX<X extends Exception> extends MetaPredicate, PrimitiveCodomain<IntPredicateX<X>>, MetaThrowingInterface<X> { // NOSONAR
	...
	/** Wraps JRE instance. */
	@Nonnull
	public static <X extends Exception> IntPredicateX<X> std(final java.util.function.IntPredicate other) {
		return other::test;
	}
	...
	@Nonnull
	public static <X extends Exception, Y extends Exception> IntPredicateX<Y> wrapException(@Nonnull final IntPredicateX<X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (int i) -> {
			try {
				return other.test(i);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends Exception> IntPredicateX<Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return IntPredicateX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> IntPredicateX<Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return IntPredicateX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
