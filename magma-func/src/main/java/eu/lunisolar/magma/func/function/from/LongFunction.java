...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.from.LongFunctionX}
 */
@FunctionalInterface
...
public interface LongFunction<R> extends java.util.function.LongFunction<R>, MetaFunction { // NOSONAR
	...
	@Nonnull
	public static <R, X extends Exception, Y extends RuntimeException> LongFunction<R> wrapException(@Nonnull final LongFunction<R> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (long l) -> {
			try {
				return other.apply(l);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> LongFunction<R> handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LongFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> LongFunction<R> handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LongFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
