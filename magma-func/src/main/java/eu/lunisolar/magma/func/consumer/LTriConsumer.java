...
public interface LTriConsumer<T1, T2, T3> extends LTriConsumerX<T1, T2, T3, RuntimeException>, MetaConsumer, MetaInterface.NonThrowing {
	...
	@Nonnull
	default <E extends Exception, Y extends RuntimeException> LTriConsumer<T1, T2, T3> handle(Class<E> exception, ExceptionHandler<E, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LTriConsumer.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for any exception (including unchecked exception that might be different from X) will call handler function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> LTriConsumer<T1, T2, T3> handle(ExceptionHandler<Exception, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return LTriConsumer.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
