...
public interface LObjIntBoolFunction<T, R> extends MetaFunction, MetaInterface.NonThrowing, Codomain<a<R>>, Domain3<a<T>, aInt, aBool> { // NOSONAR
	 ...
	 */
	public static <T, R> M<T, R> memento(R initialValue, LObjIntBoolFunction<T, R> baseFunction, LBinaryOperator<R> mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LObjIntBoolFunction.D)
	 */
	final class M<T, R> implements LObjIntBoolFunction<T, R> {
		...
		private M(R lastValue, LObjIntBoolFunction<T, R> baseFunction, LBinaryOperator<R> mementoFunction) {
