...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.FloatToByteFunction}
 */
@FunctionalInterface
...
public interface FloatToByteFunctionX<X extends Exception> extends MetaFunction, PrimitiveCodomain<FloatToByteFunctionX<X>>, MetaThrowingInterface<X> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends Exception> FloatToByteFunctionX<Y> wrapException(@Nonnull final FloatToByteFunctionX<X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (float f) -> {
			try {
	...
	@Nonnull
	default <Y extends Exception> FloatToByteFunctionX<Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return FloatToByteFunctionX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> FloatToByteFunctionX<Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return FloatToByteFunctionX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
