...
/**
 * A consumer.
 * @see {@link eu.lunisolar.magma.func.consumer.primitives.IntConsumer}
 */
@FunctionalInterface
...
public interface IntConsumerX<X extends Exception> extends MetaConsumer, MetaThrowingInterface<X> {
	...
	@Nonnull
	public static <X extends Exception, Y extends Exception> IntConsumerX<Y> wrapException(@Nonnull final IntConsumerX<X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (int i) -> {
			try {
				other.accept(i);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends Exception> IntConsumerX<Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
	...
	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> IntConsumerX<Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);
