...
/**
 *
 * @see {@link eu.lunisolar.magma.func.predicate.FloatPredicateX}
 */
@FunctionalInterface
...
public interface FloatPredicate extends MetaPredicate, PrimitiveCodomain<FloatPredicate> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> FloatPredicate wrapException(@Nonnull final FloatPredicate other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (float f) -> {
			try {
				return other.test(f);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> FloatPredicate handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return FloatPredicate.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> FloatPredicate handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return FloatPredicate.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
