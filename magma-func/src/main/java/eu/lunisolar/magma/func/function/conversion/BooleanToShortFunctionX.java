...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.BooleanToShortFunction}
 */
@FunctionalInterface
...
public interface BooleanToShortFunctionX<X extends Exception> extends MetaFunction, PrimitiveCodomain<BooleanToShortFunctionX<X>>, MetaThrowingInterface<X> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends Exception> BooleanToShortFunctionX<Y> wrapException(@Nonnull final BooleanToShortFunctionX<X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (boolean b) -> {
			try {
	...
	@Nonnull
	default <Y extends Exception> BooleanToShortFunctionX<Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BooleanToShortFunctionX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> BooleanToShortFunctionX<Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BooleanToShortFunctionX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
