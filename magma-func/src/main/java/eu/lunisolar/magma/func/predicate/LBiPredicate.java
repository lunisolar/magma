...
public interface LBiPredicate<T1, T2> extends BiPredicate<T1, T2>, MetaPredicate, MetaInterface.NonThrowing, Codomain<aBool>, Domain2<a<T1>, a<T2>> { // NOSONAR
	 ...
	 */
	public static <T1, T2> M<T1, T2> memento(boolean initialValue, LBiPredicate<T1, T2> baseFunction, LLogicalBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LBiPredicate.D)
	 */
	final class M<T1, T2> implements LBiPredicate<T1, T2> {
		...
		private M(boolean lastValue, LBiPredicate<T1, T2> baseFunction, LLogicalBinaryOperator mementoFunction) {
