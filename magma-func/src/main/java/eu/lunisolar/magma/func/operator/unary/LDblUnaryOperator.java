...
public interface LDblUnaryOperator extends DoubleUnaryOperator, MetaOperator, MetaInterface.NonThrowing, Codomain<aDouble>, Domain1<aDouble> { // NOSONAR
	 ...
	 */
	public static M memento(double initialValue, LDblUnaryOperator baseFunction, LDblBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LDblUnaryOperator.D)
	 */
	final class M implements LDblUnaryOperator {
		...
		private M(double lastValue, LDblUnaryOperator baseFunction, LDblBinaryOperator mementoFunction) {
