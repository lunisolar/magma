...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.BooleanToDoubleFunctionX}
 */
@FunctionalInterface
...
public interface BooleanToDoubleFunction extends MetaFunction, PrimitiveCodomain<BooleanToDoubleFunction> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> BooleanToDoubleFunction wrapException(@Nonnull final BooleanToDoubleFunction other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (boolean b) -> {
			try {
				return other.applyAsDouble(b);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> BooleanToDoubleFunction handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BooleanToDoubleFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> BooleanToDoubleFunction handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BooleanToDoubleFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
