...
public interface LObjLongFunction<T, R> extends MetaFunction, MetaInterface.NonThrowing, Codomain<a<R>>, Domain2<a<T>, aLong> { // NOSONAR
	 ...
	 */
	public static <T, R> M<T, R> memento(R initialValue, LObjLongFunction<T, R> baseFunction, LBinaryOperator<R> mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LObjLongFunction.D)
	 */
	final class M<T, R> implements LObjLongFunction<T, R> {
		...
		private M(R lastValue, LObjLongFunction<T, R> baseFunction, LBinaryOperator<R> mementoFunction) {
