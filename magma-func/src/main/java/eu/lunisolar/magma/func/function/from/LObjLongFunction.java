...
public interface LObjLongFunction<T, R> extends MetaFunction, MetaInterface.NonThrowing, Codomain<a<R>>, Domain2<a<T>, aLong> { // NOSONAR
	...
	static <T, R> R tryApply(T a1, long a2, LObjLongFunction<T, R> func, @Nonnull ExWMF<RuntimeException> exF, @Nonnull String newMessage, @Nullable Object... messageParams) {
		Null.nonNullArg(func, "func");
	...
	public static <R, M, K, V> R from(M container, LBiFunction<M, K, V> extractor, K key, long a2, LObjLongFunction<V, R> function) {
	...
	/**  */
	public static <T, R> LObjLongFunction<T, R> uncurry(LFunction<T, LLongFunction<R>> func) {
		return (T a1, long a2) -> func.apply(a1).apply(a2);
	}

	/** Cast that removes generics. */
	public default LObjLongFunction untyped() {
		return this;
	}

	/** Cast that replace generics. */
	public default <V2, V3> LObjLongFunction<V2, V3> cast() {
		return untyped();
	}
	...
	@Nonnull
	static <T, R> LObjLongFunction<T, R> objLongFunc(Class<T> c1, Class<R> c2, final @Nonnull LObjLongFunction<T, R> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda;
	}
