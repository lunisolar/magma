...
/**
 *
 * @see {@link eu.lunisolar.magma.func.predicate.CharPredicate}
 */
@FunctionalInterface
...
public interface CharPredicateX<X extends Exception> extends MetaPredicate, PrimitiveCodomain<CharPredicateX<X>>, MetaThrowingInterface<X> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends Exception> CharPredicateX<Y> wrapException(@Nonnull final CharPredicateX<X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (char c) -> {
			try {
				return other.test(c);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends Exception> CharPredicateX<Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return CharPredicateX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> CharPredicateX<Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return CharPredicateX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
