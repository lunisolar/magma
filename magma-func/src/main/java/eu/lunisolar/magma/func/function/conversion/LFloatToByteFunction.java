...
public interface LFloatToByteFunction extends LFloatToByteFunctionX<RuntimeException>, MetaFunction, MetaInterface.NonThrowing { // NOSONAR
	...
	String DESCRIPTION = "LFloatToByteFunction: byte doApplyAsByte(float a)";
	...
	default LByteSupplier captureFloatToByteFunc(float a) {
		return () -> this.doApplyAsByte(a);
	}
	...
	static byte call(float a, final @Nonnull LFloatToByteFunction lambda) {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	default LFloatToByteFunction floatToByteFuncComposeFloat(@Nonnull final LFloatUnaryOperator before) {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V> LToByteFunction<V> floatToByteFuncCompose(@Nonnull final LToFloatFunction<? super V> before) {
		Null.nonNullArg(before, "before");
		return v -> this.doApplyAsByte(before.doApplyAsFloat(v));
	}
