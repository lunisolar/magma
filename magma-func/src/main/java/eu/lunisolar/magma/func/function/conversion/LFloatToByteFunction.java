...
public interface LFloatToByteFunction extends LFloatToByteFunctionX<RuntimeException>, MetaFunction, MetaInterface.NonThrowing { // NOSONAR

	String DESCRIPTION = "LFloatToByteFunction: byte doApplyAsByte(float a1)";

	byte doApplyAsByte(float a1);

	default Byte tupleApplyAsByte(LFloatSingle args) {
	...
	default byte nonNullDoApplyAsByte(float a1) {
	...
	default LByteSupplier captureFloatToByteFunc(float a1) {
		return () -> this.doApplyAsByte(a1);
	}

	/** Creates function that always returns the same value. */
	static LFloatToByteFunction constant(byte r) {
		return a1 -> r;
	}

	/** Convenient method in case lambda expression is ambiguous for the compiler (that might happen for overloaded methods accepting different interfaces). */
	@Nonnull
	static LFloatToByteFunction l(final @Nonnull LFloatToByteFunction lambda) {
	...
	@Nonnull
	static <X extends Throwable> LFloatToByteFunction wrap(final @Nonnull LFloatToByteFunctionX<X> other) {
	...
	@Nonnull
	static LFloatToByteFunction safe() {
		return Function4U::produceByte;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static LSupplier<LFloatToByteFunction> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static LFloatToByteFunction safe(final @Nullable LFloatToByteFunction other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static LSupplier<LFloatToByteFunction> safeSupplier(final @Nullable LSupplier<LFloatToByteFunction> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	@Nonnull
	default LFloatToByteFunction floatToByteFuncComposeFloat(@Nonnull final LFloatUnaryOperator before1) {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1> LToByteFunction<V1> floatToByteFuncCompose(@Nonnull final LToFloatFunction<? super V1> before1) {
		Null.nonNullArg(before1, "before1");
		return v1 -> this.doApplyAsByte(before1.doApplyAsFloat(v1));
	}
	...
	/** Combines two functions together in a order. */
	@Nonnull
	default LFloatPredicate thenToBool(@Nonnull LBytePredicate after) {
		Null.nonNullArg(after, "after");
