...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.DoubleToShortFunction}
 */
@FunctionalInterface
...
public interface DoubleToShortFunctionX<X extends Exception> extends MetaFunction, PrimitiveCodomain<DoubleToShortFunctionX<X>>, MetaThrowingInterface<X> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends Exception> DoubleToShortFunctionX<Y> wrapException(@Nonnull final DoubleToShortFunctionX<X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (double d) -> {
			try {
	...
	@Nonnull
	default <Y extends Exception> DoubleToShortFunctionX<Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return DoubleToShortFunctionX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> DoubleToShortFunctionX<Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return DoubleToShortFunctionX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
