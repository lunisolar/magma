...
/**
 *
 * @see {@link eu.lunisolar.magma.func.predicate.BiObjIntPredicateX}
 */
@FunctionalInterface
...
public interface BiObjIntPredicate<T1, T2> extends MetaPredicate, PrimitiveCodomain<BiObjIntPredicate<T1, T2>> { // NOSONAR
	...
	@Nonnull
	public static <T1, T2, X extends Exception, Y extends RuntimeException> BiObjIntPredicate<T1, T2> wrapException(@Nonnull final BiObjIntPredicate<T1, T2> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (T1 t1, T2 t2, int i) -> {
			try {
				return other.test(t1, t2, i);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> BiObjIntPredicate<T1, T2> handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BiObjIntPredicate.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> BiObjIntPredicate<T1, T2> handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BiObjIntPredicate.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
