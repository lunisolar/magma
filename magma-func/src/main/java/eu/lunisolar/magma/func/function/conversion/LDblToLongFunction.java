...
public interface LDblToLongFunction extends DoubleToLongFunction, MetaFunction, MetaInterface.NonThrowing, Codomain<aLong>, Domain1<aDouble> { // NOSONAR
	 ...
	 */
	public static M memento(long initialValue, LDblToLongFunction baseFunction, LLongBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LDblToLongFunction.D)
	 */
	final class M implements LDblToLongFunction {
		...
		private M(long lastValue, LDblToLongFunction baseFunction, LLongBinaryOperator mementoFunction) {
	...
	@Nonnull
	static LDblToLongFunction safe() {
		return LDblToLongFunction::doNothing;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static LSupplier<LDblToLongFunction> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static LDblToLongFunction safe(final @Nullable LDblToLongFunction other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static LSupplier<LDblToLongFunction> safeSupplier(final @Nullable LSupplier<LDblToLongFunction> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
