...
public interface LByteToDoubleFunctionX<X extends Throwable> extends MetaFunction, MetaInterface.Throwing<X> { // NOSONAR
	...
	String DESCRIPTION = "LByteToDoubleFunctionX: double doApplyAsDouble(byte a1) throws X";

	double doApplyAsDouble(byte a1) throws X;
	...
	default double shovingDoApplyAsDouble(byte a1) {
		return ((LByteToDoubleFunctionX<RuntimeException>) this).doApplyAsDouble(a1);
	}

	/** Function call that handles exceptions according to the instructions. */
	default <Y extends Throwable> double handlingDoApplyAsDouble(byte a1, HandlingInstructions<Throwable, Y> handling) throws Y {
	...
	default double nonNullDoApplyAsDouble(byte a1) throws X {
	...
	default LDoubleSupplierX<X> captureByteToDoubleFunc(byte a1) {
		return () -> this.doApplyAsDouble(a1);
	}
	...
	static <X extends Throwable> double call(byte a1, final @Nonnull LByteToDoubleFunctionX<X> lambda) throws X {
		Null.nonNullArg(lambda, "lambda");
		return lambda.doApplyAsDouble(a1);
	}

	static <X extends Throwable> double shoving(byte a1, final @Nonnull LByteToDoubleFunctionX<X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.shovingDoApplyAsDouble(a1);
	}

	static <X extends Throwable> double nesting(byte a1, final @Nonnull LByteToDoubleFunctionX<X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.nestingDoApplyAsDouble(a1);
	}

	static <X extends Throwable, Y extends Throwable> double handling(byte a1, final HandlingInstructions<Throwable, Y> handling, final @Nonnull LByteToDoubleFunctionX<X> lambda) throws Y {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	default LByteToDoubleFunctionX<X> byteToDoubleFuncComposeByte(@Nonnull final LByteUnaryOperatorX<X> before1) {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1> LToDoubleFunctionX<V1, X> byteToDoubleFuncCompose(@Nonnull final LToByteFunctionX<? super V1, X> before1) {
		Null.nonNullArg(before1, "before1");
		return v1 -> this.doApplyAsDouble(before1.doApplyAsByte(v1));
	}
