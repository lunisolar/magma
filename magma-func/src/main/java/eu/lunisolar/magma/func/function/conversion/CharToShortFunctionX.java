...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.CharToShortFunction}
 */
@FunctionalInterface
...
public interface CharToShortFunctionX<X extends Exception> extends MetaFunction, PrimitiveCodomain<CharToShortFunctionX<X>>, MetaThrowingInterface<X> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends Exception> CharToShortFunctionX<Y> wrapException(@Nonnull final CharToShortFunctionX<X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (char c) -> {
			try {
	...
	@Nonnull
	default <Y extends Exception> CharToShortFunctionX<Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return CharToShortFunctionX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> CharToShortFunctionX<Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return CharToShortFunctionX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
