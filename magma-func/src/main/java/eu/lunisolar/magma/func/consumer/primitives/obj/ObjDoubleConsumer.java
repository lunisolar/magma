...
/**
 * A consumer.
 * @see {@link eu.lunisolar.magma.func.consumer.primitives.obj.ObjDoubleConsumerX}
 */
@FunctionalInterface
...
public interface ObjDoubleConsumer<T> extends java.util.function.ObjDoubleConsumer<T>, MetaConsumer {
	...
	@Nonnull
	public static <T, X extends Exception, Y extends RuntimeException> ObjDoubleConsumer<T> wrapException(@Nonnull final ObjDoubleConsumer<T> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (T t, double d) -> {
			try {
				other.accept(t, d);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> ObjDoubleConsumer<T> handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
	...
	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> ObjDoubleConsumer<T> handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);
