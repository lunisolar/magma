...
public interface LObjDblConsumer<T> extends ObjDoubleConsumer<T>, MetaConsumer, MetaInterface.NonThrowing, TeConsumer<T, aDouble>, Codomain<aVoid>, Domain2<a<T>, aDouble> {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1, V2> LBiConsumer<V1, V2> unboxingCompose(@Nonnull final LFunction<? super V1, ? extends T> before1, @Nonnull final LToDblFunction<? super V2> before2) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
		return (v1, v2) -> this.accept(before1.apply(v1), before2.applyAsDbl(v2));
	}
	...
	default LObjDblConsumer<T> shoving() {

		return new LObjDblConsumer<T>() {

			public void accept(T a1, double a2) {
				try {
					this.acceptX(a1, a2);
				} catch (Throwable e) { // NOSONAR
					Handling.handleErrors(e);
			...
			public void acceptX(T a1, double a2) throws Throwable {
				LObjDblConsumer.this.acceptX(a1, a2);
			}
