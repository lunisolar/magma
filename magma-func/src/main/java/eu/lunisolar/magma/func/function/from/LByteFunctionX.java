...
public interface LByteFunctionX<R, X extends Throwable> extends MetaFunction, MetaInterface.Throwing<X> { // NOSONAR
	...
	String DESCRIPTION = "LByteFunctionX: R doApply(byte a) throws X";

	@Nullable
	R doApply(byte a) throws X;
	...
	default R shovingDoApply(byte a) {
		return ((LByteFunctionX<R, RuntimeException>) this).doApply(a);
	}

	/** Function call that handles exceptions according to the instructions. */
	default <Y extends Throwable> R handlingDoApply(byte a, HandlingInstructions<Throwable, Y> handling) throws Y {
	...
	@Nonnull
	default R nonNullDoApply(byte a) throws X {
	...
	default LSupplierX<R, X> captureByteFunc(byte a) {
		return () -> this.doApply(a);
	}
	...
	static <R, X extends Throwable> R call(byte a, final @Nonnull LByteFunctionX<R, X> lambda) throws X {
		Null.nonNullArg(lambda, "lambda");
		return lambda.doApply(a);
	}

	static <R, X extends Throwable> R shoving(byte a, final @Nonnull LByteFunctionX<R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.shovingDoApply(a);
	}

	static <R, X extends Throwable> R nesting(byte a, final @Nonnull LByteFunctionX<R, X> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda.nestingDoApply(a);
	}

	static <R, X extends Throwable, Y extends Throwable> R handling(byte a, final HandlingInstructions<Throwable, Y> handling, final @Nonnull LByteFunctionX<R, X> lambda) throws Y {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	static <R, X extends Throwable> LByteFunctionX<R, X> safe() {
		return Function4U::produce;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static <R, X extends Throwable, Y extends Throwable> LSupplierX<LByteFunctionX<R, X>, Y> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static <R, X extends Throwable> LByteFunctionX<R, X> safe(final @Nullable LByteFunctionX<R, X> other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static <R, X extends Throwable, Y extends Throwable> LSupplierX<LByteFunctionX<R, X>, Y> safeSupplier(final @Nullable LSupplierX<LByteFunctionX<R, X>, Y> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	@Nonnull
	default LByteFunctionX<R, X> byteFuncComposeByte(@Nonnull final LByteUnaryOperatorX<X> before) {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V> LFunctionX<V, R, X> byteFuncCompose(@Nonnull final LToByteFunctionX<? super V, X> before) {
		Null.nonNullArg(before, "before");
		return v -> this.doApply(before.doApplyAsByte(v));
	}
