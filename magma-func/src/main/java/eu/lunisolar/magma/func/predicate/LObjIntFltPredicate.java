...
public interface LObjIntFltPredicate<T> extends MetaPredicate, MetaInterface.NonThrowing, Codomain<aBool>, Domain3<a<T>, aInt, aFloat> { // NOSONAR
	 ...
	 */
	public static <T> M<T> memento(boolean initialValue, LObjIntFltPredicate<T> baseFunction, LLogicalBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LObjIntFltPredicate.D)
	 */
	final class M<T> implements LObjIntFltPredicate<T> {
		...
		private M(boolean lastValue, LObjIntFltPredicate<T> baseFunction, LLogicalBinaryOperator mementoFunction) {
