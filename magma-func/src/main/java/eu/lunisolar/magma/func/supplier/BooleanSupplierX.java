...
/**
 *
 * @see {@link eu.lunisolar.magma.func.supplier.BooleanSupplier}
 */
@FunctionalInterface
...
public interface BooleanSupplierX<X extends Exception> extends MetaSupplier, PrimitiveCodomain<BooleanSupplierX<X>>, MetaThrowingInterface<X> {
	...
	@Nonnull
	public static <X extends Exception, Y extends Exception> BooleanSupplierX<Y> wrapException(@Nonnull final BooleanSupplierX<X> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return () -> {
			try {
				return other.getAsBoolean();
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends Exception> BooleanSupplierX<Y> handle(Class<? extends Exception> exception, ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BooleanSupplierX.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends Exception> BooleanSupplierX<Y> handle(ExceptionHandler<? super X, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return BooleanSupplierX.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
