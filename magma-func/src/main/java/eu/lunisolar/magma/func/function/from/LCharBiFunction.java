...
public interface LCharBiFunction<R> extends LCharBiFunctionX<R, RuntimeException>, MetaFunction, MetaInterface.NonThrowing { // NOSONAR
	...
	default LSupplier<R> captureCBiFunc(char c1, char c2) {
		return () -> this.doApply(c1, c2);
	}
	 ...
	 */
	@Nonnull
	default LCharBiFunction<R> cBiFuncFromChar(@Nonnull final LCharUnaryOperator before1, @Nonnull final LCharUnaryOperator before2) {
	 ...
	 */
	@Nonnull
	default <V1, V2> LBiFunction<V1, V2, R> cBiFuncFrom(@Nonnull final LToCharFunction<? super V1> before1, @Nonnull final LToCharFunction<? super V2> before2) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
	...
	@Nonnull
	default LCharBiFunction<R> nonNullCBiFunc() {
