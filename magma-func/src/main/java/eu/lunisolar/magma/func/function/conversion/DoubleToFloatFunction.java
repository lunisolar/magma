...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.DoubleToFloatFunctionX}
 */
@FunctionalInterface
...
public interface DoubleToFloatFunction extends MetaFunction, PrimitiveCodomain<DoubleToFloatFunction> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> DoubleToFloatFunction wrapException(@Nonnull final DoubleToFloatFunction other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (double d) -> {
			try {
				return other.applyAsFloat(d);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> DoubleToFloatFunction handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return DoubleToFloatFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> DoubleToFloatFunction handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return DoubleToFloatFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
