...
public interface LTieDblFunction<T> extends MetaFunction, MetaInterface.NonThrowing, TieFunction<T, aDouble>, Codomain<aInt>, Domain3<a<T>, aInt, aDouble> { // NOSONAR
	...
	final class M<T> implements LTieDblFunction<T> {
		...
		public int currentApplyAsInt(T a1, int a2, double a3) {
			int x1 = lastBaseValue;
		...
		public int currentBaseValue(T a1, int a2, double a3) {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V1, V2, V3> LToIntTriFunction<V1, V2, V3> unboxingCompose(@Nonnull final LFunction<? super V1, ? extends T> before1, @Nonnull final LToIntFunction<? super V2> before2, @Nonnull final LToDblFunction<? super V3> before3) {
		Null.nonNullArg(before1, "before1");
		Null.nonNullArg(before2, "before2");
		Null.nonNullArg(before3, "before3");
		return (v1, v2, v3) -> this.applyAsInt(before1.apply(v1), before2.applyAsInt(v2), before3.applyAsDbl(v3));
	...
	default LTieDblFunction<T> shoving() {

		return new LTieDblFunction<T>() {
			...
			public int applyAsIntX(T a1, int a2, double a3) throws Throwable {
				return LTieDblFunction.this.applyAsIntX(a1, a2, a3);
			}
