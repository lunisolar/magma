...
public interface LBoolToFloatFunction extends LBoolToFloatFunctionX<RuntimeException>, MetaFunction, MetaInterface.NonThrowing { // NOSONAR
	...
	String DESCRIPTION = "LBoolToFloatFunction: float doApplyAsFloat(boolean a)";

	float doApplyAsFloat(boolean a);
	...
	default float nonNullDoApplyAsFloat(boolean a) {
	...
	default LFloatSupplier captureBoolToFloatFunc(boolean a) {
		return () -> this.doApplyAsFloat(a);
	}
	...
	static float call(boolean a, final @Nonnull LBoolToFloatFunction lambda) {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	default LBoolToFloatFunction boolToFloatFuncComposeBool(@Nonnull final LLogicalOperator before) {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V> LToFloatFunction<V> boolToFloatFuncCompose(@Nonnull final LPredicate<? super V> before) {
		Null.nonNullArg(before, "before");
