...
public interface LObjIntDblFunction<T, R> extends MetaFunction, MetaInterface.NonThrowing, Codomain<a<R>>, Domain3<a<T>, aInt, aDouble> { // NOSONAR
	...
	public static <R, M, K, V> R from(M container, LBiFunction<M, K, V> extractor, K key, int a2, double a3, LObjIntDblFunction<V, R> function) {
	...
	/**  */
	public static <T, R> LObjIntDblFunction<T, R> uncurry(LFunction<T, LIntFunction<LDblFunction<R>>> func) {
	...
	@Nonnull
	static <T, R> LObjIntDblFunction<T, R> objIntDblFunc(Class<T> c1, Class<R> c2, final @Nonnull LObjIntDblFunction<T, R> lambda) {
		Null.nonNullArg(lambda, "lambda");
		return lambda;
	}
