...
public interface LByteToShortFunction extends LByteToShortFunctionX<RuntimeException>, MetaFunction, MetaInterface.NonThrowing { // NOSONAR
	...
	String DESCRIPTION = "LByteToShortFunction: short doApplyAsShort(byte a)";

	short doApplyAsShort(byte a);
	...
	default short nonNullDoApplyAsShort(byte a) {
	...
	default LShortSupplier captureByteToShortFunc(byte a) {
		return () -> this.doApplyAsShort(a);
	}
	...
	static short call(byte a, final @Nonnull LByteToShortFunction lambda) {
		Null.nonNullArg(lambda, "lambda");
	...
	@Nonnull
	static LByteToShortFunction safe() {
		return Function4U::produceShort;
	}

	/** Safe instance supplier. Returns supplier of safe() instance. */
	@Nonnull
	static LSupplier<LByteToShortFunction> safeSupplier() {
		return () -> safe();
	}

	/** Safe wrapping. Either argument function is returned (if it is not null) or safe() instance. */
	@Nonnull
	static LByteToShortFunction safe(final @Nullable LByteToShortFunction other) {
		if (other == null) {
			return safe();
		} else {
			return other;
	...
	@Nonnull
	static LSupplier<LByteToShortFunction> safeSupplier(final @Nullable LSupplier<LByteToShortFunction> supplier) {
		if (supplier == null) {
			return safeSupplier();
		} else {
			return supplier;
	...
	@Nonnull
	default LByteToShortFunction byteToShortFuncComposeByte(@Nonnull final LByteUnaryOperator before) {
	...
	/** Allows to manipulate the domain of the function. */
	@Nonnull
	default <V> LToShortFunction<V> byteToShortFuncCompose(@Nonnull final LToByteFunction<? super V> before) {
		Null.nonNullArg(before, "before");
		return v -> this.doApplyAsShort(before.doApplyAsByte(v));
	}
