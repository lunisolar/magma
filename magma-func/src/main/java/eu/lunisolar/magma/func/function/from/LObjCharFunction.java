...
public interface LObjCharFunction<T, R> extends MetaFunction, MetaInterface.NonThrowing, Codomain<a<R>>, Domain2<a<T>, aChar> { // NOSONAR
	 ...
	 */
	public static <T, R> M<T, R> memento(R initialValue, LObjCharFunction<T, R> baseFunction, LBinaryOperator<R> mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LObjCharFunction.D)
	 */
	final class M<T, R> implements LObjCharFunction<T, R> {
		...
		private M(R lastValue, LObjCharFunction<T, R> baseFunction, LBinaryOperator<R> mementoFunction) {
