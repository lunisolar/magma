...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.to.ToDoubleFunctionX}
 */
@FunctionalInterface
...
public interface ToDoubleFunction<T> extends java.util.function.ToDoubleFunction<T>, MetaFunction, PrimitiveCodomain<ToDoubleFunction<T>> { // NOSONAR
	...
	@Nonnull
	public static <T, X extends Exception, Y extends RuntimeException> ToDoubleFunction<T> wrapException(@Nonnull final ToDoubleFunction<T> other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (T t) -> {
			try {
				return other.applyAsDouble(t);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> ToDoubleFunction<T> handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ToDoubleFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> ToDoubleFunction<T> handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return ToDoubleFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
