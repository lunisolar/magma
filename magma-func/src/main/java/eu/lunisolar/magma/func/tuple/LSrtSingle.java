...
public interface LSrtSingle extends LTuple<Short>, Comparable<LSrtSingle> {
	...
	interface Mut<SELF extends Mut<SELF>> extends LSrtSingle {
		...
		default SELF setValueIfCurrent(short value, LSrtPredicate predicate) {//1
		...
		default SELF setValueIfNew(short value, LSrtPredicate predicate) {//1
		...
		default SELF setValueIf(short value, LBiSrtPredicate predicate) {//2
	...
	final class AtomicSrtSingle extends AbstractSrtSingle implements Mut<AtomicSrtSingle> {
		...
		static {
			try {
				vh = MethodHandles.lookup().in(AtomicSrtSingle.class).findVarHandle(AtomicSrtSingle.class, "value", short.class);
		...
		/** Sets value if predicate(current) is true */
		public @Override AtomicSrtSingle setValueIfCurrent(short value, LSrtPredicate predicate) {
