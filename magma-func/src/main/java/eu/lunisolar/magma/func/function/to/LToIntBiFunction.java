...
public interface LToIntBiFunction<T1, T2> extends ToIntBiFunction<T1, T2>, MetaFunction, MetaInterface.NonThrowing, Codomain<aInt>, Domain2<a<T1>, a<T2>> { // NOSONAR
	 ...
	 */
	public static <T1, T2> M<T1, T2> memento(int initialValue, LToIntBiFunction<T1, T2> baseFunction, LIntBinaryOperator mementoFunction) {
		return new M(initialValue, baseFunction, mementoFunction);
	}

	/**
	 * Implementation that allows to create derivative functions (do not ). Very short name is intended to be used with parent (LToIntBiFunction.D)
	 */
	final class M<T1, T2> implements LToIntBiFunction<T1, T2> {
		...
		private M(int lastValue, LToIntBiFunction<T1, T2> baseFunction, LIntBinaryOperator mementoFunction) {
