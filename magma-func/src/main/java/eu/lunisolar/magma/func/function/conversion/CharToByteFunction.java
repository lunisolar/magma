...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.CharToByteFunctionX}
 */
@FunctionalInterface
...
public interface CharToByteFunction extends MetaFunction, PrimitiveCodomain<CharToByteFunction> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> CharToByteFunction wrapException(@Nonnull final CharToByteFunction other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (char c) -> {
			try {
				return other.applyAsByte(c);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> CharToByteFunction handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return CharToByteFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> CharToByteFunction handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return CharToByteFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
