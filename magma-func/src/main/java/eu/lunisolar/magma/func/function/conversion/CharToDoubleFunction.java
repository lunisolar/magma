...
/**
 *
 * @see {@link eu.lunisolar.magma.func.function.conversion.CharToDoubleFunctionX}
 */
@FunctionalInterface
...
public interface CharToDoubleFunction extends MetaFunction, PrimitiveCodomain<CharToDoubleFunction> { // NOSONAR
	...
	@Nonnull
	public static <X extends Exception, Y extends RuntimeException> CharToDoubleFunction wrapException(@Nonnull final CharToDoubleFunction other, Class<? extends Exception> exception, ExceptionHandler<Exception, Y> rethrower) {
		return (char c) -> {
			try {
				return other.applyAsDouble(c);
			} catch (Exception e) {
	...
	@Nonnull
	default <Y extends RuntimeException> CharToDoubleFunction handle(Class<? extends Exception> exception, ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(exception, Function4U.VALIDATION_MESSAGE_EXCEPTION);
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return CharToDoubleFunction.wrapException(this, exception, (ExceptionHandler) handler);
	}

	/** Wraps with exception handling that for argument exception class will call function to determine the final exception. */
	@Nonnull
	default <Y extends RuntimeException> CharToDoubleFunction handle(ExceptionHandler<? super RuntimeException, Y> handler) {
		Objects.requireNonNull(handler, Function4U.VALIDATION_MESSAGE_HANDLER);

		return CharToDoubleFunction.wrapException(this, Exception.class, (ExceptionHandler) handler);
	}
