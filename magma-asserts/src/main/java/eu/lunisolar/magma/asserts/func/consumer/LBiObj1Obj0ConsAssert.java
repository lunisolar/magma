...
public interface LBiObj1Obj0ConsAssert<S extends LBiObj1Obj0ConsAssert<S, A, T3, T2, T1>, A extends LTriConsumer.LBiObj1Obj0Cons<T3, T2, T1>, T3, T2, T1> extends Assert<S, A>, FunctionalAssert.Simple<S, LTriConsumer<T3, T2, T1>, A> {
	...
	@Nonnull
	SemiEvaluation<S, LTriConsumer<T3, T2, T1>, A> doesAccept(T3 a3, T2 a2, T1 a1);

	/** Convenience implementation - if you want instantiate not to extend (uses one less generic parameter). */
	final class The<A extends LTriConsumer.LBiObj1Obj0Cons<T3, T2, T1>, T3, T2, T1> extends Base<The<A, T3, T2, T1>, A, T3, T2, T1> {
	...
	class Base<S extends Base<S, A, T3, T2, T1>, A extends LTriConsumer.LBiObj1Obj0Cons<T3, T2, T1>, T3, T2, T1> extends FunctionalAssert.Simple.Base<S, LTriConsumer<T3, T2, T1>, A> implements LBiObj1Obj0ConsAssert<S, A, T3, T2, T1> {
		...
		@Nonnull
		public SemiEvaluation<S, LTriConsumer<T3, T2, T1>, A> doesAccept(T3 a3, T2 a2, T1 a1) {

			return evaluation(() -> String.format("(%s,%s,%s)", a3, a2, a1), pc -> {
				if (pc != null) {
					pc.accept(a3, a2, a1);
				}
				actual.acceptBiObj1Obj0(a3, a2, a1);
				return null;
			});
		}
