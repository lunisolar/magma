...
public final class LObjIntSrtPredicateBuilder<T> extends PerCaseBuilderWithBoolProduct.Base<LObjIntSrtPredicateBuilder<T>, LObjIntSrtPredicate<T>, LObjIntSrtPredicate<T>> {
	...
	public LObjIntSrtPredicateBuilder() {
		this(null);
	}

	/** One of ways of creating builder. In most cases (considering all _functional_ builders) it requires to provide generic parameters (in most cases redundantly) */
	@Nonnull
	public static <T> LObjIntSrtPredicateBuilder<T> objIntSrtPredicate() {
		return new LObjIntSrtPredicateBuilder();
	}
	...
	@Nonnull
	public static <T> LObjIntSrtPredicateBuilder<T> objIntSrtPredicate(Consumer<LObjIntSrtPredicate<T>> consumer) {
		return new LObjIntSrtPredicateBuilder(consumer);
	}

	/** One of ways of creating builder. In most cases (considering all _functional_ builders) it requires to provide generic parameters (in most cases redundantly) */
	@Nonnull
	public final LObjIntSrtPredicateBuilder<T> withHandling(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
	...
	public <V extends T> LObjIntSrtPredicateBuilder<T> forValue(T v1, int v2, short v3, LObjIntSrtPredicate<V> function) {
	...
	/** Allows to specify additional cases for a specific values of arguments (matched by equals).*/
	public <V extends T> PartialCaseWithBoolProduct.The<LObjIntSrtPredicateBuilder<T>, LObjIntSrtPredicate<T>, LObjIntSrtPredicate<T>> forValue(T v1, int v2, short v3) {
		return partialCaseFactoryMethod((a1, a2, a3) -> Objects.equals(a1, v1) && a2 == v2 && a3 == v3);
	}

	/** Allows to specify additional cases for a specific type of generic arguments (matched by instanceOf). Null classes can be provided in case of arguments that do not matter. */
	@Nonnull
	public <V extends T> LObjIntSrtPredicateBuilder<T> casesOf(Class<V> argC1, Consumer<LObjIntSrtPredicateBuilder<V>> pcpConsumer) {
	...
	@Nonnull
	public <V extends T> LObjIntSrtPredicateBuilder<T> forClass(Class<V> argC1, LObjIntSrtPredicate<V> function) {
		PartialCaseWithBoolProduct.The pc = partialCaseFactoryMethod((a1, a2, a3) -> (argC1 == null || argC1.isInstance(a1)));
		pc.evaluate(function);
		return fluentCtx();
	}
	...
	/** Adds full new case for the argument that are of specific classes (matched by instanceOf, null is a wildcard). */
	@Nonnull
	public <V extends T> PartialCaseWithBoolProduct.The<LObjIntSrtPredicateBuilder<T>, LObjIntSrtPredicate<T>, LObjIntSrtPredicate<T>> forClass(Class<V> argC1) {
	...
	public final LObjIntSrtPredicate<T> build(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
