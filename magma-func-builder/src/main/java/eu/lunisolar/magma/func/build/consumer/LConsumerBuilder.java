...
public final class LConsumerBuilder<T> extends PerCaseBuilder.Base<LConsumerBuilder<T>, LPredicate<T>, LConsumer<T>> {
	...
	public static final LConsumer OTHERWISE_THROW = LConsumer.cons(a -> {
		throw new IllegalStateException("There is no case configured for the arguments (if any).");
	});

	public LConsumerBuilder(@Nullable Consumer<LConsumer<T>> consumer) {
		super(OTHERWISE_THROW, () -> new LConsumerBuilder(null));
	...
	/** One of ways of creating builder. In most cases (considering all _functional_ builders) it requires to provide generic parameters (in most cases redundantly) */
	@Nonnull
	public static <T> LConsumerBuilder<T> consumer() {
		return new LConsumerBuilder();
	}

	/** One of ways of creating builder. This is possibly the least verbose way where compiler should be able to guess the generic parameters. */
	@Nonnull
	public static <T> LConsumer<T> consumerFrom(Consumer<LConsumerBuilder<T>> buildingFunction) {
		LConsumerBuilder builder = new LConsumerBuilder();
	...
	/** One of ways of creating builder. In most cases (considering all _functional_ builders) it requires to provide generic parameters (in most cases redundantly) */
	@Nonnull
	public final LConsumerBuilder<T> withHandling(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
	...
	public <V extends T> LConsumerBuilder<T> forValue(T v, LConsumer<V> function) {
		PartialCase.The pc = partialCaseFactoryMethod(a -> Objects.equals(a, v));
		pc.evaluate(function);
		return fluentCtx();
	}

	/** Allows to specify additional cases for a specific values of arguments (matched by equals).*/
	public <V extends T> PartialCase.The<LConsumerBuilder<T>, LPredicate<T>, LConsumer<T>> forValue(T v) {
	...
	@Nonnull
	public <V extends T> LConsumerBuilder<T> forClass(Class<V> argC, LConsumer<V> function) {
		PartialCase.The pc = partialCaseFactoryMethod(a -> (argC == null || argC.isInstance(a)));
		pc.evaluate(function);
		return fluentCtx();
	}
	...
	/** Adds full new case for the argument that are of specific classes (matched by instanceOf, null is a wildcard). */
	@Nonnull
	public <V extends T> PartialCase.The<LConsumerBuilder<T>, LPredicate<T>, LConsumer<T>> forClass(Class<V> argC) {
	...
	public final LConsumer<T> build() {
		...
		final LConsumer<T> otherwiseFinal = this.otherwise;
	...
	public final LConsumer<T> build(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
