...
public final class LToShortFunctionBuilder<T> extends PerCaseBuilderWithShortProduct.Base<LToShortFunctionBuilder<T>, LPredicate<T>, LToShortFunction<T>> {
	...
	public static final LToShortFunction EVENTUALLY_THROW = LToShortFunction.l((Object a1) -> {
		String message;
		try {
			message = String.format("No case specified for: %s  as function %s.", a1, LToShortFunction.DESCRIPTION);
	...
	public LToShortFunctionBuilder(@Nullable Consumer<LToShortFunction<T>> consumer) {
		super(EVENTUALLY_THROW, LToShortFunction::constant, () -> new LToShortFunctionBuilder(null));
	...
	/** One of ways of creating builder. In most cases (considering all _functional_ builders) it requires to provide generic parameters (in most cases redundantly) */
	@Nonnull
	public static <T> LToShortFunctionBuilder<T> toShortFunction() {
		return new LToShortFunctionBuilder();
	}

	/** One of ways of creating builder. This might be the only way (considering all _functional_ builders) that might be utilize to specify generic params only once. */
	@Nonnull
	public static <T> LToShortFunctionBuilder<T> toShortFunction(Consumer<LToShortFunction<T>> consumer) {
		return new LToShortFunctionBuilder(consumer);
	}

	/** One of ways of creating builder. In most cases (considering all _functional_ builders) it requires to provide generic parameters (in most cases redundantly) */
	@Nonnull
	public final LToShortFunctionBuilder<T> withHandling(@Nonnull HandlingInstructions<RuntimeException, RuntimeException> handling) {
	...
	@Nonnull
	public <E1 extends T> LToShortFunctionBuilder<T> casesOf(Class<E1> argC1, Consumer<LToShortFunctionBuilder<E1>> pcpConsumer) {
	...
	@Nonnull
	public <E1 extends T> LToShortFunctionBuilder<T> aCase(Class<E1> argC1, LToShortFunction<E1> function) {
		PartialCaseWithShortProduct.The pc = partialCaseFactoryMethod(a1 -> (argC1 == null || argC1.isInstance(a1)));
	...
	public final LToShortFunction<T> build(@Nonnull HandlingInstructions<RuntimeException, RuntimeException> handling) {
