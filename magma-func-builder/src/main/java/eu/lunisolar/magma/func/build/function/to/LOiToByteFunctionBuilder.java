...
public final class LOiToByteFunctionBuilder<T> extends PerCaseBuilderWithByteProduct.Base<LOiToByteFunctionBuilder<T>, LObjIntPredicate<T>, LOiToByteFunction<T>> {
	...
	/** One of ways of creating builder. In most cases (considering all _functional_ builders) it requires to provide generic parameters (in most cases redundantly) */
	@Nonnull
	public final LOiToByteFunctionBuilder<T> withHandling(@Nonnull HandlingInstructions<RuntimeException, RuntimeException> handling) {
	...
	@Nonnull
	public <V extends T> LOiToByteFunctionBuilder<T> aCase(Class<V> argC1, LOiToByteFunction<V> function) {
		PartialCaseWithByteProduct.The pc = partialCaseFactoryMethod((a1, a2) -> (argC1 == null || argC1.isInstance(a1)));
	...
	public final LOiToByteFunction<T> build(@Nonnull HandlingInstructions<RuntimeException, RuntimeException> handling) {
