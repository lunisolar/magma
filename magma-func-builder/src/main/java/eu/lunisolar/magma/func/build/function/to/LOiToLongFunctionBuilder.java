...
public final class LOiToLongFunctionBuilder<T> extends PerCaseBuilderWithLongProduct.Base<LOiToLongFunctionBuilder<T>, LObjIntPredicate<T>, LOiToLongFunction<T>> {
	...
	/** One of ways of creating builder. In most cases (considering all _functional_ builders) it requires to provide generic parameters (in most cases redundantly) */
	@Nonnull
	public final LOiToLongFunctionBuilder<T> withHandling(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
	...
	public <V extends T> LOiToLongFunctionBuilder<T> forValue(T v1, int v2, LOiToLongFunction<V> function) {
		PartialCaseWithLongProduct.The pc = partialCaseFactoryMethod((a1, a2) -> Objects.equals(a1, v1) && a2 == v2);
		pc.evaluate(function);
		return fluentCtx();
	}

	/** Allows to specify additional cases for a specific values of arguments (matched by equals).*/
	public <V extends T> PartialCaseWithLongProduct.The<LOiToLongFunctionBuilder<T>, LObjIntPredicate<T>, LOiToLongFunction<T>> forValue(T v1, int v2) {
	...
	@Nonnull
	public <V extends T> LOiToLongFunctionBuilder<T> forClass(Class<V> argC1, LOiToLongFunction<V> function) {
		PartialCaseWithLongProduct.The pc = partialCaseFactoryMethod((a1, a2) -> (argC1 == null || argC1.isInstance(a1)));
		pc.evaluate(function);
		return fluentCtx();
	}
	...
	/** Adds full new case for the argument that are of specific classes (matched by instanceOf, null is a wildcard). */
	@Nonnull
	public <V extends T> PartialCaseWithLongProduct.The<LOiToLongFunctionBuilder<T>, LObjIntPredicate<T>, LOiToLongFunction<T>> forClass(Class<V> argC1) {
	...
	public final LOiToLongFunction<T> build(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
