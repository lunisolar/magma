...
public final class LTieDblFunctionBuilder<T> extends PerCaseBuilderWithIntProduct.Base<LTieDblFunctionBuilder<T>, LObjIntDblPredicate<T>, LTieDblFunction<T>> {
	...
	public static final LTieDblFunction OTHERWISE_THROW = LTieDblFunction.tieDblFunc((a1, a2, a3) -> {
		throw new IllegalStateException("There is no case configured for the arguments (if any).");
	});

	public LTieDblFunctionBuilder(@Nullable Consumer<LTieDblFunction<T>> consumer) {
		super(OTHERWISE_THROW, LTieDblFunction::constant, () -> new LTieDblFunctionBuilder(null));
	...
	/** One of ways of creating builder. In most cases (considering all _functional_ builders) it requires to provide generic parameters (in most cases redundantly) */
	@Nonnull
	public final LTieDblFunctionBuilder<T> withHandling(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
	...
	public <V extends T> LTieDblFunctionBuilder<T> forValue(T v1, int v2, double v3, LTieDblFunction<V> function) {
	...
	/** Allows to specify additional cases for a specific values of arguments (matched by equals).*/
	public <V extends T> PartialCaseWithIntProduct.The<LTieDblFunctionBuilder<T>, LObjIntDblPredicate<T>, LTieDblFunction<T>> forValue(T v1, int v2, double v3) {
	...
	@Nonnull
	public <V extends T> LTieDblFunctionBuilder<T> forClass(Class<V> argC1, LTieDblFunction<V> function) {
		PartialCaseWithIntProduct.The pc = partialCaseFactoryMethod((a1, a2, a3) -> (argC1 == null || argC1.isInstance(a1)));
		pc.evaluate(function);
		return fluentCtx();
	}
	...
	/** Adds full new case for the argument that are of specific classes (matched by instanceOf, null is a wildcard). */
	@Nonnull
	public <V extends T> PartialCaseWithIntProduct.The<LTieDblFunctionBuilder<T>, LObjIntDblPredicate<T>, LTieDblFunction<T>> forClass(Class<V> argC1) {
	...
	public final LTieDblFunction<T> build() {
		...
		final LTieDblFunction<T> otherwiseFinal = this.otherwise;
	...
	public final LTieDblFunction<T> build(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
