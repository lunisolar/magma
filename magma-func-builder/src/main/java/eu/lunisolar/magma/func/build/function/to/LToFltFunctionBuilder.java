...
public final class LToFltFunctionBuilder<T> extends PerCaseBuilderWithFltProduct.Base<LToFltFunctionBuilder<T>, LPredicate<T>, LToFltFunction<T>> {
	...
	public <V extends T> LToFltFunctionBuilder<T> forValue(T v, LToFltFunction<V> function) {
		PartialCaseWithFltProduct.The pc = partialCaseFactoryMethod(a -> Objects.equals(a, v));
		pc.evaluate(function);
		return fluentCtx();
	}

	/** Allows to specify additional cases for a specific values of arguments (matched by equals).*/
	public <V extends T> PartialCaseWithFltProduct.The<LToFltFunctionBuilder<T>, LPredicate<T>, LToFltFunction<T>> forValue(T v) {
	...
	@Nonnull
	public <V extends T> LToFltFunctionBuilder<T> forClass(Class<V> argC, LToFltFunction<V> function) {
		PartialCaseWithFltProduct.The pc = partialCaseFactoryMethod(a -> (argC == null || argC.isInstance(a)));
		pc.evaluate(function);
		return fluentCtx();
	}
	...
	/** Adds full new case for the argument that are of specific classes (matched by instanceOf, null is a wildcard). */
	@Nonnull
	public <V extends T> PartialCaseWithFltProduct.The<LToFltFunctionBuilder<T>, LPredicate<T>, LToFltFunction<T>> forClass(Class<V> argC) {
