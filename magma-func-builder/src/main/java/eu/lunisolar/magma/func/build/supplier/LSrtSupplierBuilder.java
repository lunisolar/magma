...
public final class LSrtSupplierBuilder extends PerCaseBuilderWithSrtProduct.Base<LSrtSupplierBuilder, LBoolSupplier, LSrtSupplier> {
	...
	@Nonnull
	public static LSrtSupplier srtSupplierFrom(Function<LSrtSupplierBuilder, LSrtSupplier> buildingFunction) {
		return buildingFunction.apply(new LSrtSupplierBuilder());
	}
	...
	/** One of ways of creating builder. In most cases (considering all _functional_ builders) it requires to provide generic parameters (in most cases redundantly) */
	@Nonnull
	public final LSrtSupplierBuilder withHandling(@Nonnull HandlingInstructions<RuntimeException, RuntimeException> handling) {
			...
			try {
				for (Case<LBoolSupplier, LSrtSupplier> aCase : casesArray) {
					if (aCase.casePredicate().doGetAsBool()) {
						return aCase.caseFunction().doGetAsSrt();
					}
				}

				return eventuallyFinal.doGetAsSrt();
	...
	public final LSrtSupplier build(@Nonnull HandlingInstructions<RuntimeException, RuntimeException> handling) {
