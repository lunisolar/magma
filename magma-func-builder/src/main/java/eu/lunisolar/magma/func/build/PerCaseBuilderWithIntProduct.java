 ...
 */
public abstract class PerCaseBuilderWithIntProduct<PCB extends PerCaseBuilderWithIntProduct<PCB, P, F>, P, F> implements Fluent<PCB> {
	...
	/** Sets the function to evaluate _eventually_ when input data do not match any case. */
	public final PCB eventuallyProduce(@Nonnull int directValue) {
	...
	public final class PartialCase implements FluentSubcontext<PartialCase, PCB> {
		...
		public final PCB evaluate(@Nonnull F caseFunction) {
			PerCaseBuilderWithIntProduct.this.addCase(Case.of(casePredicate, caseFunction));
			return PerCaseBuilderWithIntProduct.this.self();
		}

		/** Finalize the case build by providing second required value for the Case. */
		public final PCB produce(@Nonnull int directValue) {
			PerCaseBuilderWithIntProduct.this.addCase(Case.of(casePredicate, directToFunction.apply(directValue)));
			return PerCaseBuilderWithIntProduct.this.self();
		}
