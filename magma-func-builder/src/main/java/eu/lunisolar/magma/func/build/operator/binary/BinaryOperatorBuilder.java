...
public final class BinaryOperatorBuilder<T> extends PerCaseBuilderWithProduct.Base<BinaryOperatorBuilder<T>, BiPredicate<T, T>, BinaryOperator<T>, T> {

	private Consumer<BinaryOperator<T>> consumer;

	public static final BinaryOperator EVENTUALLY_THROW = BinaryOperator.l((Object t1, Object t2) -> {
		String message;
		try {
			message = String.format("No case specified for: %s ,%s  as function %s.", t1, t2, BinaryOperator.DESCRIPTION);
	...
	@Nonnull
	public static final <T> BinaryOperatorBuilder<T> binaryOperator(Consumer<BinaryOperator<T>> consumer) {
	...
	public final BinaryOperator<T> build() {
		...
		if (cases.isEmpty()) {
			...
			final Case<BiPredicate<T, T>, BinaryOperator<T>>[] casesArray = cases.toArray(new Case[cases.size()]);
			retval = BinaryOperator.l((T t1, T t2) -> {
				for (Case<BiPredicate<T, T>, BinaryOperator<T>> aCase : casesArray) {
