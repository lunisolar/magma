...
public final class LToSrtFunctionBuilder<T> extends PerCaseBuilderWithSrtProduct.Base<LToSrtFunctionBuilder<T>, LPredicate<T>, LToSrtFunction<T>> {
	...
	public <V extends T> LToSrtFunctionBuilder<T> forValue(T v, LToSrtFunction<V> function) {
		PartialCaseWithSrtProduct.The pc = partialCaseFactoryMethod(a -> Objects.equals(a, v));
		pc.evaluate(function);
		return fluentCtx();
	}

	/** Allows to specify additional cases for a specific values of arguments (matched by equals).*/
	public <V extends T> PartialCaseWithSrtProduct.The<LToSrtFunctionBuilder<T>, LPredicate<T>, LToSrtFunction<T>> forValue(T v) {
	...
	@Nonnull
	public <V extends T> LToSrtFunctionBuilder<T> forClass(Class<V> argC, LToSrtFunction<V> function) {
		PartialCaseWithSrtProduct.The pc = partialCaseFactoryMethod(a -> (argC == null || argC.isInstance(a)));
		pc.evaluate(function);
		return fluentCtx();
	}
	...
	/** Adds full new case for the argument that are of specific classes (matched by instanceOf, null is a wildcard). */
	@Nonnull
	public <V extends T> PartialCaseWithSrtProduct.The<LToSrtFunctionBuilder<T>, LPredicate<T>, LToSrtFunction<T>> forClass(Class<V> argC) {
