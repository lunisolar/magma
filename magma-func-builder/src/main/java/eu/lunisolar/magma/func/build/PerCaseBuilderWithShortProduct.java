 ...
 */
public abstract class PerCaseBuilderWithShortProduct<PCB extends PerCaseBuilderWithShortProduct<PCB, P, F>, P, F> implements Fluent<PCB> {
	...
	/** Sets the function to evaluate _eventually_ when input data do not match any case. */
	public final PCB eventuallyProduce(@Nonnull short directValue) {
	...
	public final class PartialCase implements FluentSubcontext<PartialCase, PCB> {
		...
		public final PCB evaluate(@Nonnull F caseFunction) {
			PerCaseBuilderWithShortProduct.this.addCase(Case.of(casePredicate, caseFunction));
			return PerCaseBuilderWithShortProduct.this.self();
		}

		/** Finalize the case build by providing second required value for the Case. */
		public final PCB produce(@Nonnull short directValue) {
			PerCaseBuilderWithShortProduct.this.addCase(Case.of(casePredicate, directToFunction.apply(directValue)));
			return PerCaseBuilderWithShortProduct.this.self();
		}
