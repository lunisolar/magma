 ...
 */
public final class LObjIntPredicateBuilder<T> extends PerCaseBuilderWithBoolProduct.Base<LObjIntPredicateBuilder<T>, LObjIntPredicate<T>, LObjIntPredicate<T>> {
	...
	public static final LObjIntPredicate OTHERWISE_THROW = LObjIntPredicate.objIntPred((a1, a2) -> {
		throw new IllegalStateException("There is no case configured for the arguments (if any).");
	});

	public LObjIntPredicateBuilder(@Nullable Consumer<LObjIntPredicate<T>> consumer) {
		super(OTHERWISE_THROW, LObjIntPredicate::constant, () -> new LObjIntPredicateBuilder(null));
	...
	/** One of ways of creating builder. In most cases (considering all _functional_ builders) it requires to provide generic parameters (in most cases redundantly) */
	@Nonnull
	public final LObjIntPredicateBuilder<T> withHandling(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
	...
	public <V extends T> LObjIntPredicateBuilder<T> forValue(T v1, int v2, LObjIntPredicate<V> function) {
	...
	/** Allows to specify additional cases for a specific values of arguments (matched by equals).*/
	public <V extends T> PartialCaseWithBoolProduct.The<LObjIntPredicateBuilder<T>, LObjIntPredicate<T>, LObjIntPredicate<T>> forValue(T v1, int v2) {
		return partialCaseFactoryMethod((a1, a2) -> Objects.equals(a1, v1) && a2 == v2);
	}

	/** Allows to specify additional cases for a specific type of generic arguments (matched by instanceOf). Null classes can be provided in case of arguments that do not matter. */
	@Nonnull
	public <V extends T> LObjIntPredicateBuilder<T> casesOf(Class<V> argC1, Consumer<LObjIntPredicateBuilder<V>> pcpConsumer) {
	...
	@Nonnull
	public <V extends T> LObjIntPredicateBuilder<T> forClass(Class<V> argC1, LObjIntPredicate<V> function) {
		PartialCaseWithBoolProduct.The pc = partialCaseFactoryMethod((a1, a2) -> (argC1 == null || argC1.isInstance(a1)));
		pc.evaluate(function);
		return fluentCtx();
	}
	...
	/** Adds full new case for the argument that are of specific classes (matched by instanceOf, null is a wildcard). */
	@Nonnull
	public <V extends T> PartialCaseWithBoolProduct.The<LObjIntPredicateBuilder<T>, LObjIntPredicate<T>, LObjIntPredicate<T>> forClass(Class<V> argC1) {
	...
	public final LObjIntPredicate<T> build() {
		...
		final LObjIntPredicate<T> otherwiseFinal = this.otherwise;
	...
	public final LObjIntPredicate<T> build(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
