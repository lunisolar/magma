...
public final class LOiFunctionBuilder<T, R> extends PerCaseBuilderWithProduct.Base<LOiFunctionBuilder<T, R>, LObjIntPredicate<T>, LOiFunction<T, R>, R> {
	...
	@Nonnull
	public static <T, R> LOiFunction<T, R> oiFunctionFrom(Function<LOiFunctionBuilder<T, R>, LOiFunction<T, R>> buildingFunction) {
		return buildingFunction.apply(new LOiFunctionBuilder());
	}
	...
	/** One of ways of creating builder. In most cases (considering all _functional_ builders) it requires to provide generic parameters (in most cases redundantly) */
	@Nonnull
	public final LOiFunctionBuilder<T, R> withHandling(@Nonnull HandlingInstructions<RuntimeException, RuntimeException> handling) {
	...
	public final LOiFunction<T, R> build(@Nonnull HandlingInstructions<RuntimeException, RuntimeException> handling) {
