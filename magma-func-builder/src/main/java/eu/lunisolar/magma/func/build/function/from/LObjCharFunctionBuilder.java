...
public final class LObjCharFunctionBuilder<T, R> extends PerCaseBuilderWithProduct.Base<LObjCharFunctionBuilder<T, R>, LObjCharPredicate<T>, LObjCharFunction<T, R>, R> {
	...
	public static final LObjCharFunction OTHERWISE_THROW = LObjCharFunction.objCharFunc((a1, a2) -> {
		throw new IllegalStateException("There is no case configured for the arguments (if any).");
	});

	public LObjCharFunctionBuilder(@Nullable Consumer<LObjCharFunction<T, R>> consumer) {
		super(OTHERWISE_THROW, LObjCharFunction::constant, () -> new LObjCharFunctionBuilder(null));
	...
	@Nonnull
	public static <T, R> LObjCharFunction<T, R> objCharFunctionFrom(Consumer<LObjCharFunctionBuilder<T, R>> buildingFunction) {
		LObjCharFunctionBuilder builder = new LObjCharFunctionBuilder();
	...
	/** One of ways of creating builder. In most cases (considering all _functional_ builders) it requires to provide generic parameters (in most cases redundantly) */
	@Nonnull
	public final LObjCharFunctionBuilder<T, R> withHandling(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
	...
	public <V extends T> LObjCharFunctionBuilder<T, R> forValue(T v1, char v2, LObjCharFunction<V, R> function) {
	...
	/** Allows to specify additional cases for a specific values of arguments (matched by equals).*/
	public <V extends T> PartialCaseWithProduct.The<LObjCharFunctionBuilder<T, R>, LObjCharPredicate<T>, LObjCharFunction<T, R>, R> forValue(T v1, char v2) {
		return partialCaseFactoryMethod((a1, a2) -> Objects.equals(a1, v1) && a2 == v2);
	}

	/** Allows to specify additional cases for a specific type of generic arguments (matched by instanceOf). Null classes can be provided in case of arguments that do not matter. */
	@Nonnull
	public <V extends T> LObjCharFunctionBuilder<T, R> casesOf(Class<V> argC1, Consumer<LObjCharFunctionBuilder<V, R>> pcpConsumer) {
	...
	@Nonnull
	public <V extends T> LObjCharFunctionBuilder<T, R> forClass(Class<V> argC1, LObjCharFunction<V, R> function) {
		PartialCaseWithProduct.The pc = partialCaseFactoryMethod((a1, a2) -> (argC1 == null || argC1.isInstance(a1)));
		pc.evaluate(function);
		return fluentCtx();
	}
	...
	/** Adds full new case for the argument that are of specific classes (matched by instanceOf, null is a wildcard). */
	@Nonnull
	public <V extends T> PartialCaseWithProduct.The<LObjCharFunctionBuilder<T, R>, LObjCharPredicate<T>, LObjCharFunction<T, R>, R> forClass(Class<V> argC1) {
	...
	public final LObjCharFunction<T, R> build() {
		...
		final LObjCharFunction<T, R> otherwiseFinal = this.otherwise;
		...
		retval = LObjCharFunction.<T, R> objCharFunc((a1, a2) -> {
			try {
	...
	public final LObjCharFunction<T, R> build(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
