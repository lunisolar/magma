...
public final class LObjFltFunctionBuilder<T, R> extends PerCaseBuilderWithProduct.Base<LObjFltFunctionBuilder<T, R>, LObjFltPredicate<T>, LObjFltFunction<T, R>, R> {
	...
	public static final LObjFltFunction OTHERWISE_THROW = LObjFltFunction.objFltFunc((a1, a2) -> {
		throw new IllegalStateException("There is no case configured for the arguments (if any).");
	});

	public LObjFltFunctionBuilder(@Nullable Consumer<LObjFltFunction<T, R>> consumer) {
		super(OTHERWISE_THROW, LObjFltFunction::constant, () -> new LObjFltFunctionBuilder(null));
	...
	@Nonnull
	public static <T, R> LObjFltFunction<T, R> objFltFunctionFrom(Consumer<LObjFltFunctionBuilder<T, R>> buildingFunction) {
		LObjFltFunctionBuilder builder = new LObjFltFunctionBuilder();
	...
	/** One of ways of creating builder. In most cases (considering all _functional_ builders) it requires to provide generic parameters (in most cases redundantly) */
	@Nonnull
	public final LObjFltFunctionBuilder<T, R> withHandling(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
	...
	public <V extends T> LObjFltFunctionBuilder<T, R> forValue(T v1, float v2, LObjFltFunction<V, R> function) {
	...
	/** Allows to specify additional cases for a specific values of arguments (matched by equals).*/
	public <V extends T> PartialCaseWithProduct.The<LObjFltFunctionBuilder<T, R>, LObjFltPredicate<T>, LObjFltFunction<T, R>, R> forValue(T v1, float v2) {
	...
	@Nonnull
	public <V extends T> LObjFltFunctionBuilder<T, R> forClass(Class<V> argC1, LObjFltFunction<V, R> function) {
		PartialCaseWithProduct.The pc = partialCaseFactoryMethod((a1, a2) -> (argC1 == null || argC1.isInstance(a1)));
		pc.evaluate(function);
		return fluentCtx();
	}
	...
	/** Adds full new case for the argument that are of specific classes (matched by instanceOf, null is a wildcard). */
	@Nonnull
	public <V extends T> PartialCaseWithProduct.The<LObjFltFunctionBuilder<T, R>, LObjFltPredicate<T>, LObjFltFunction<T, R>, R> forClass(Class<V> argC1) {
	...
	public final LObjFltFunction<T, R> build() {
		...
		final LObjFltFunction<T, R> otherwiseFinal = this.otherwise;
	...
	public final LObjFltFunction<T, R> build(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
