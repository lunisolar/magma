...
public final class LOiToDblFunctionBuilder<T> extends PerCaseBuilderWithDblProduct.Base<LOiToDblFunctionBuilder<T>, LObjIntPredicate<T>, LOiToDblFunction<T>> {
	...
	/** One of ways of creating builder. In most cases (considering all _functional_ builders) it requires to provide generic parameters (in most cases redundantly) */
	@Nonnull
	public final LOiToDblFunctionBuilder<T> withHandling(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
	...
	public <V extends T> LOiToDblFunctionBuilder<T> forValue(T v1, int v2, LOiToDblFunction<V> function) {
		PartialCaseWithDblProduct.The pc = partialCaseFactoryMethod((a1, a2) -> Objects.equals(a1, v1) && a2 == v2);
		pc.evaluate(function);
		return fluentCtx();
	}

	/** Allows to specify additional cases for a specific values of arguments (matched by equals).*/
	public <V extends T> PartialCaseWithDblProduct.The<LOiToDblFunctionBuilder<T>, LObjIntPredicate<T>, LOiToDblFunction<T>> forValue(T v1, int v2) {
	...
	@Nonnull
	public <V extends T> LOiToDblFunctionBuilder<T> forClass(Class<V> argC1, LOiToDblFunction<V> function) {
		PartialCaseWithDblProduct.The pc = partialCaseFactoryMethod((a1, a2) -> (argC1 == null || argC1.isInstance(a1)));
		pc.evaluate(function);
		return fluentCtx();
	}
	...
	/** Adds full new case for the argument that are of specific classes (matched by instanceOf, null is a wildcard). */
	@Nonnull
	public <V extends T> PartialCaseWithDblProduct.The<LOiToDblFunctionBuilder<T>, LObjIntPredicate<T>, LOiToDblFunction<T>> forClass(Class<V> argC1) {
	...
	public final LOiToDblFunction<T> build(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
