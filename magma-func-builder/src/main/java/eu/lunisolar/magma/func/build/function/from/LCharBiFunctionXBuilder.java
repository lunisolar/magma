...
public final class LCharBiFunctionXBuilder<R, X extends Throwable> extends PerCaseBuilderWithProduct.Base<LCharBiFunctionXBuilder<R, X>, LBiCharPredicateX<X>, LCharBiFunctionX<R, X>, R> {
	...
	/** One of ways of creating builder. In most cases (considering all _functional_ builders) it requires to provide generic parameters (in most cases redundantly) */
	@Nonnull
	public static final <R, X extends Throwable> LCharBiFunctionXBuilder<R, X> charBiFunctionX() {
		return new LCharBiFunctionXBuilder();
	}

	/** One of ways of creating builder. This might be the only way (considering all _functional_ builders) that might be utilize to specify generic params only once. */
	@Nonnull
	public static final <R, X extends Throwable> LCharBiFunctionXBuilder<R, X> charBiFunctionX(Consumer<LCharBiFunctionX<R, X>> consumer) {
		return new LCharBiFunctionXBuilder(consumer);
	}

	/** One of ways of creating builder. In most cases (considering all _functional_ builders) it requires to provide generic parameters (in most cases redundantly) */
	@Nonnull
	public final LCharBiFunctionXBuilder<R, X> withHandling(@Nonnull HandlingInstructions<X, X> handling) {
	...
	public final LCharBiFunctionX<R, X> build() {
		...
		retval = LCharBiFunctionX.<R, X> lX((char c1, char c2) -> {
			try {
	...
	public final LCharBiFunctionX<R, X> build(@Nonnull HandlingInstructions<X, X> handling) {
