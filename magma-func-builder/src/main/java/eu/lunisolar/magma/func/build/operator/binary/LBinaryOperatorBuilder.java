...
public final class LBinaryOperatorBuilder<T> extends PerCaseBuilderWithProduct.Base<LBinaryOperatorBuilder<T>, LBiPredicate<T, T>, LBinaryOperator<T>, T> {
	...
	public static final LBinaryOperator OTHERWISE_THROW = LBinaryOperator.binaryOp((a1, a2) -> {
		throw new IllegalStateException("There is no case configured for the arguments (if any).");
	});

	public LBinaryOperatorBuilder(@Nullable Consumer<LBinaryOperator<T>> consumer) {
		super(OTHERWISE_THROW, LBinaryOperator::constant, () -> new LBinaryOperatorBuilder(null));
	...
	@Nonnull
	public static <T> LBinaryOperator<T> binaryOperatorFrom(Consumer<LBinaryOperatorBuilder<T>> buildingFunction) {
		LBinaryOperatorBuilder builder = new LBinaryOperatorBuilder();
	...
	@Nonnull
	public static <T> LBinaryOperatorBuilder<T> binaryOperator(Consumer<LBinaryOperator<T>> consumer) {
		return new LBinaryOperatorBuilder(consumer);
	}

	/** One of ways of creating builder. In most cases (considering all _functional_ builders) it requires to provide generic parameters (in most cases redundantly) */
	@Nonnull
	public final LBinaryOperatorBuilder<T> withHandling(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
	...
	public <V extends T> LBinaryOperatorBuilder<T> forValue(T v1, T v2, LBinaryOperator<V> function) {
	...
	/** Allows to specify additional cases for a specific values of arguments (matched by equals).*/
	public <V extends T> PartialCaseWithProduct.The<LBinaryOperatorBuilder<T>, LBiPredicate<T, T>, LBinaryOperator<T>, T> forValue(T v1, T v2) {
	...
	@Nonnull
	public <V extends T> LBinaryOperatorBuilder<T> forClass(Class<V> argC1, Class<V> argC2, LBinaryOperator<V> function) {
		PartialCaseWithProduct.The pc = partialCaseFactoryMethod((a1, a2) -> (argC1 == null || argC1.isInstance(a1)) && (argC2 == null || argC2.isInstance(a2)));
		pc.evaluate(function);
		return fluentCtx();
	}
	...
	/** Adds full new case for the argument that are of specific classes (matched by instanceOf, null is a wildcard). */
	@Nonnull
	public <V extends T> PartialCaseWithProduct.The<LBinaryOperatorBuilder<T>, LBiPredicate<T, T>, LBinaryOperator<T>, T> forClass(Class<V> argC1, Class<V> argC2) {
	...
	public final LBinaryOperator<T> build() {
		...
		final LBinaryOperator<T> otherwiseFinal = this.otherwise;
	...
	public final LBinaryOperator<T> build(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
