...
public final class LTieCharFunctionBuilder<T> extends PerCaseBuilderWithIntProduct.Base<LTieCharFunctionBuilder<T>, LObjIntCharPredicate<T>, LTieCharFunction<T>> {
	...
	/** One of ways of creating builder. In most cases (considering all _functional_ builders) it requires to provide generic parameters (in most cases redundantly) */
	@Nonnull
	public final LTieCharFunctionBuilder<T> withHandling(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
	...
	public <V extends T> LTieCharFunctionBuilder<T> forValue(T v1, int v2, char v3, LTieCharFunction<V> function) {
	...
	/** Allows to specify additional cases for a specific values of arguments (matched by equals).*/
	public <V extends T> PartialCaseWithIntProduct.The<LTieCharFunctionBuilder<T>, LObjIntCharPredicate<T>, LTieCharFunction<T>> forValue(T v1, int v2, char v3) {
	...
	@Nonnull
	public <V extends T> LTieCharFunctionBuilder<T> forClass(Class<V> argC1, LTieCharFunction<V> function) {
		PartialCaseWithIntProduct.The pc = partialCaseFactoryMethod((a1, a2, a3) -> (argC1 == null || argC1.isInstance(a1)));
		pc.evaluate(function);
		return fluentCtx();
	}
	...
	/** Adds full new case for the argument that are of specific classes (matched by instanceOf, null is a wildcard). */
	@Nonnull
	public <V extends T> PartialCaseWithIntProduct.The<LTieCharFunctionBuilder<T>, LObjIntCharPredicate<T>, LTieCharFunction<T>> forClass(Class<V> argC1) {
	...
	public final LTieCharFunction<T> build(@Nonnull HandlingInstructions<Throwable, RuntimeException> handling) {
