...
import static eu.lunisolar.magma.func.build.std.IntConsumerBuilder.intConsumer;
import static eu.lunisolar.magma.func.build.std.IntConsumerBuilder.intConsumerFrom;

public class IntConsumerBuilderTest{

    @Test
    public void testOtherwiseThrow()  {

        try {
            IntConsumer function = intConsumerFrom(b-> b
                .build()
            );
    ...
    public void testBuild()  {
        ...
        IntConsumer function = intConsumerFrom( b -> b
            ...
            .inCase(a -> a > 0 && a < 10).evaluate(a -> externalEffect.set(1))
            .inCase(a -> a > 10 && a < 20).evaluate(a -> externalEffect.set(2))
            .otherwise(a -> externalEffect.set(99))
            .build()
        );


        FuncAttests.attestIntCons(function)
            .doesAccept(0).when(null).soThat(() -> Checks.attest(externalEffect.get()).mustEx(Be::equalEx, 0))
            .doesAccept(5).when(null).soThat(() -> Checks.attest(externalEffect.get()).mustEx(Be::equalEx, 1))
            .doesAccept(15).when(null).soThat(() -> Checks.attest(externalEffect.get()).mustEx(Be::equalEx, 2))
            .doesAccept(10).when(null).soThat(() -> Checks.attest(externalEffect.get()).mustEx(Be::equalEx, 99))
        ;
